...
**Companion to `PHASE_2_FINAL_ARCHITECTURE_AUDIT.md`.** The consolidated, ranked register of every architectural risk the Phase 2C board surfaced, merged with the still-open items from the prior review (O1–O5, 15.A–15.D). Each row: severity, the defect, the source, the proposed additive remedy (correction ID), the gate that blocks on it, reversibility, and owner. Design-validation only — remedies are proposed for a later, separately-approved ratification pass; nothing here is applied to the frozen constitution.
