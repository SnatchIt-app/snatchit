...
BEGIN
  ...
  v_party_only :=
    ...
    AND (NEW.buyer_id  = OLD.buyer_id  OR NEW.buyer_id  = v_sentinel)
    AND (NEW.seller_id = OLD.seller_id OR NEW.seller_id = v_sentinel)
    AND NEW.status IS NOT DISTINCT FROM OLD.status
    AND NEW.amount = OLD.amount AND NEW.buyer_fee = OLD.buyer_fee AND NEW.seller_fee = OLD.seller_fee
    AND NEW.total = OLD.total AND NEW.mode = OLD.mode AND NEW.listing_id = OLD.listing_id
