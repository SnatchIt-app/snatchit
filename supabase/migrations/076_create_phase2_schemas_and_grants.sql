...
-- lands", extended to notify by OR-12/F-P1-7). Issued for the migration role
-- (postgres — the creator of every later Phase-2 table), so a later package
-- that forgets its per-table REVOKE still ships a deny-by-default table.
...
--
-- Both are TRIGGER functions: never EXECUTEd by a principal, fired as the
-- table owner. Owned by postgres, search_path pinned (frozen Tests row).
...
create function kernel.set_updated_at()
returns trigger
language plpgsql
set search_path = ''
as $$
...
create function kernel.raise_append_only()
returns trigger
language plpgsql
set search_path = ''
as $$
...
create index outbox_drain_order_idx
...
--     sequence), which is the contracted backstop (warning for BE, raise for
--     REQUIRED);
...
create function notify.emit_event(
  p_event_type     text,
...
create function notify.emit_event_required(
  p_event_type     text,
  p_aggregate_kind text,
  p_aggregate_id   uuid,
  p_event_key      text,
