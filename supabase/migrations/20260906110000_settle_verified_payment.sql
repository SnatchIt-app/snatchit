...
--                     mark its row failed.)
--   not_external_rail the row is a kernel-rail payment (093: mode 'native_primary',
--                     listing_id NULL) — NO writes, no review row; terminal for
--                     every caller (review round 2, MAJOR-1).
...
BEGIN
  ...
  -- ── 1b. Rail. This contract settles the EXTERNAL (resale) rail only.
  --       Production (093) admits mode 'native_primary' rows with listing_id
  --       and seller_id NULL; they belong to the kernel money rail and are
  --       never touched here — no writes, no review row (review round 2,
  --       MAJOR-1). Callers treat the outcome as terminal.
  IF v_p.listing_id IS NULL OR v_p.mode NOT IN ('buy_now', 'auction') THEN
    outcome := 'not_external_rail';
    RETURN NEXT; RETURN;
  END IF;
  ...
  IF coalesce(p_amount_refunded, 0) >= v_p.total THEN
    ...
    IF v_refund_fn IS NOT NULL AND nullif(p_stripe_refund_id, '') IS NOT NULL THEN
      ...
      -- INCREMENT, not the cumulative figure (review round 2, MINOR-4):
      -- p_amount_refunded is Stripe's cumulative amount_refunded and
      -- p_stripe_refund_id the NEWEST refund (refunds.data is newest-first).
      -- Ledgering the cumulative figure under that id would double-count
      -- earlier refunds the charge.refunded branch already ledgered. The
      -- per-refund ledger row is therefore (cumulative - already recorded),
      -- and nothing is written when nothing new was refunded.
      EXECUTE 'SELECT coalesce(p.amount_refunded_cents, 0) FROM public.payments p WHERE p.id = $1'
         INTO v_already USING v_p.id;
      v_inc := least(p_amount_refunded, v_p.total) - v_already;
      IF v_inc > 0 THEN
        EXECUTE 'SELECT public.record_payment_refund($1, $2, NULL, $3, $4)'
          USING p_payment_intent_id, p_stripe_refund_id, v_inc, 'dashboard';
      END IF;
    ELSE
