-- ============================================================================
-- ROLLBACK of 20260924120000_payout_decisions_buyer_confirmed_truth.sql (registry 149)
-- Restores both bodies to 20260906120000's (the applied pre-149 text), guarded:
-- refuses unless the live bodies are 149's, and raises unless the restored
-- bodies hash to the pre-149 values. Not a migration: lives outside
-- supabase/migrations/ (a rollback there is parsed as a duplicate version).
-- ============================================================================
BEGIN;
DO $g$
BEGIN
  IF (SELECT md5(prosrc) FROM pg_proc WHERE oid = 'public.record_payout_attempt_result(uuid,text,text,jsonb)'::regprocedure) <> '62f747287fa1c47d7bea06b686426ae3'
  OR (SELECT md5(prosrc) FROM pg_proc WHERE oid = 'public.flag_payout_reversal_required(uuid,text,jsonb)'::regprocedure) <> '03ea4589c46624b72fa4d48e466ac977' THEN
    RAISE EXCEPTION 'ROLLBACK_149_REFUSED: live bodies are not 149''s';
  END IF;
END $g$;
...
DO $v$
BEGIN
  IF (SELECT md5(prosrc) FROM pg_proc WHERE oid = 'public.record_payout_attempt_result(uuid,text,text,jsonb)'::regprocedure) <> '6a8372b4ee470d7a6ab9c1e0754765da'
  OR (SELECT md5(prosrc) FROM pg_proc WHERE oid = 'public.flag_payout_reversal_required(uuid,text,jsonb)'::regprocedure) <> 'd86c2b36f40c83835624bbacee716d38' THEN
    RAISE EXCEPTION 'ROLLBACK_149_VERIFY_FAILED: restored bodies do not hash to pre-149';
  END IF;
END $v$;
COMMIT;
