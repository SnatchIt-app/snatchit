-- ============================================================================
-- ROLLBACK for 122_transfers_profiles_fk_parity.sql.
--
-- Returns both constraints to auth.users(id) — the CHAIN's pre-122 shape, which
-- is NOT production's shape. Running this against production would CREATE the
-- drift 122 exists to remove; it is here for an environment built from the
-- chain (sandbox, CI, local rehearsal) where 122 was applied and must be undone.
--
...
begin
  ...
  if v_orphan > 0 then
    raise exception '122 rollback REFUSED — % transfers row(s) reference a buyer or seller with no auth.users row', v_orphan;
  end if;
