...
SELECT is((SELECT count(*)::int FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
            WHERE n.nspname IN ('kernel','venue','catalog','market','notify')), 304,
  ...
  'B2: the five phase-2 schemas hold exactly 304 routines [139 +1 notify: claim_report_delivery; 135 +3 notify: issue_push_token_challenge, get_push_token_challenge, record_push_token_challenge_delivery; 131 +4 kernel: invalidate_push_bindings_for, trg_push_bindings_on_password_change, trg_push_bindings_on_sessions_gone, push_session_predates_epoch] (153+87+17+22+17 — plus 108''s four venue scan cores/machine entrypoints, 109''s force_close_session_manifests (kernel) and tg_session_terminal_force_close (catalog), 110''s guard_signing_key_insert (kernel), 113''s manifest core/machine entrypoint (venue), 114''s M1 door read/manifest signing context (venue))');
