...
SELECT is((SELECT count(*)::int FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
  ...
  'P2: GATE-2 functions=99 — 096 adds no public function (70 through 20260902003623; +1 from 119''s guard_listing_seller_not_blocked; +1 P1, +2 P2, +12 P3, +1 20260906130000; +1 from 20260909000000''s public.get_my_tickets; +1 from 127: release_reservation_for_payment; +4 from 128: register_push_token, unbind_push_token, guard_push_token_secret_hash, guard_push_token_rebind_epoch; +1 from 129: revoke_push_token; +2 from 130: claim_checkout_supersede, release_checkout_supersede; +3 from 132: claim_checkout_group, release_checkout_group, record_checkout_attempt; pgtap''s own extension-owned functions excluded)');
...
SELECT is((SELECT count(*)::int FROM pg_trigger t JOIN pg_class c ON c.oid=t.tgrelid JOIN pg_namespace n ON n.oid=c.relnamespace
            WHERE n.nspname='public' AND NOT t.tgisinternal), 35,
  'P4: GATE-2 triggers=35 — 096 adds no public trigger (26 through 109; +1 from 119''s trg_guard_listing_seller_not_blocked; +6 from 20260906120000 guards; +2 from 128: trg_guard_push_token_secret_hash, trg_guard_push_token_rebind_epoch)');
