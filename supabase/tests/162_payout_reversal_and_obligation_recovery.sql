...
SELECT is((SELECT count(*)::int FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
            WHERE n.nspname='public'
              AND NOT EXISTS (SELECT 1 FROM pg_depend d WHERE d.objid=p.oid AND d.deptype='e')), 99,
  'P2: GATE-2 functions=99 — 096 adds no public function (70 through 20260902003623; +1 from 119''s guard_listing_seller_not_blocked; +1 P1, +2 P2, +12 P3, +1 20260906130000; +1 from 20260909000000''s public.get_my_tickets; +1 from 127: release_reservation_for_payment; +4 from 128: register_push_token, unbind_push_token, guard_push_token_secret_hash, guard_push_token_rebind_epoch; +1 from 129: revoke_push_token; +2 from 130: claim_checkout_supersede, release_checkout_supersede; +3 from 131: revoke_all_push_bindings, guard_push_token_session_stmt, guard_push_token_session_row; pgtap''s own extension-owned functions excluded)');
