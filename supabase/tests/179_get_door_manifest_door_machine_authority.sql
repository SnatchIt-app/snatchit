...
SELECT is((SELECT count(*)::int FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace WHERE n.nspname IN ('kernel','venue','catalog','market','notify')), 306,
  'A9: five-schema routine census 306 (141 +1 notify: retire_account_deletion_pending (F-NOTICE-1, 2026-09-17); 292 post-111 + 113''s two + 114''s two = 296; 131 +4 kernel: invalidate_push_bindings_for, trg_push_bindings_on_password_change, trg_push_bindings_on_sessions_gone, push_session_predates_epoch; 135 +3 notify: issue_push_token_challenge, get_push_token_challenge, record_push_token_challenge_delivery; 139 +2 notify: claim_report_delivery, release_report_delivery)');
