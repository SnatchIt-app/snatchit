...
SELECT is((SELECT count(*)::int FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace
  ...
  'A1: kernel holds EXACTLY THIRTY-TWO tables (111''s signing_key_recovery_approval + 27 post-088 + 091''s reserve stub + 094''s organization_obligation + 096''s payout_reversal/organization_obligation_recovery)');
...
SELECT is((SELECT count(*)::int FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
  ...
  'A32: kernel holds EXACTLY 153 functions (111''s three + 109 post-090 + 093''s sixteen + 095''s seven + 094''s four + 096''s nine + 099''s one + 102''s one + 105''s one + 109''s one + 110''s one)');
SELECT is((SELECT count(*)::int FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
