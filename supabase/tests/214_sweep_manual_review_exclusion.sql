...
-- Negative control: on the chain without 147 exactly A2, A3, A5, A6, A7 and A11 fail (A2/A3/A5/A6/A7 list
-- a payment under review; A11 is the comment).
BEGIN;
SELECT plan(11);
...
UPDATE public.listings SET sold_at = now() - interval '50 days', auction_status = 'cancelled' WHERE id = tap._id214(1);
...
-- P1's transfer: buyer confirmed, payout already recorded (the incident's exact shape).
