...
SELECT is((SELECT new_c -> 'ret' FROM t210_dj), (SELECT old_c -> 'ret' FROM t210_dj),
  'B4: detect_jobs returns what the applied body returns');
SELECT is((SELECT new_c -> 'cases' FROM t210_dj), (SELECT old_c -> 'cases' FROM t210_dj),
  'B5: detect_jobs opens exactly the applied body''s job_failure cases, with the same text');
SELECT is((SELECT new_c -> 'alerts' FROM t210_dj), (SELECT old_c -> 'alerts' FROM t210_dj),
  'B6: detect_jobs fires exactly the applied body''s job_failure alerts, with the same payloads');
...
SELECT ok(NOT ((SELECT coalesce(array_agg(x ->> 'ref'), '{}') FROM t210_dj, jsonb_array_elements(new_c -> 'cases') x) @> array['t210-hung8d']),
  'D3: unchanged meaning — a job with no run started in the last 7 days is not a detect_jobs case');
