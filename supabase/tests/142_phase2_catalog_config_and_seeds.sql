...
SELECT is((SELECT count(*)::int FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
  ...
  -- 2026-09-17 (package 138): 17 -> 18. catalog.bootstrap_venue (A3, console-only; PFA-33). Named in A4 below.
  ...
  'A3: catalog holds EXACTLY eighteen functions — no helper the closed world does not carry');
...
SELECT bag_eq(
  ...
  $$VALUES ('create_venue'),('approve_venue'),('update_venue'),('create_event'),
           ...
           ('tg_door_open_at_is_ledger_head'),('cancel_event'),('tg_session_terminal_force_close'),('bootstrap_venue')$$,
  'A4: the eighteen catalog function names are exactly the frozen set (bootstrap_venue by 138; publish_event by 081; four door fns by 086; cancel_event by 088; tg_session_terminal_force_close by 109)');
...
SELECT is((SELECT count(*)::int FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
  ...
  'K3: kernel holds 159 functions [138 +2 kernel: bootstrap_organization, invite_bootstrap_owner] [131 +4 kernel: invalidate_push_bindings_for, trg_push_bindings_on_password_change, trg_push_bindings_on_sessions_gone, push_session_predates_epoch] — — 111 added three; 109 post-090 plus 093''s sixteen plus 095''s seven plus 094''s four plus 096''s nine plus 099''s one plus 102''s one plus 105''s one plus 109''s one plus 110''s guard_signing_key_insert (141 A14 pins the exact figure and the re-created-not-added set)');
SELECT is((SELECT count(*)::int FROM pg_policy p JOIN pg_class c ON c.oid = p.polrelid
