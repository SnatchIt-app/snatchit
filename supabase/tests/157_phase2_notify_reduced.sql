...
SELECT is((SELECT count(*)::int FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname IN ('kernel','venue','catalog','market','notify')), 308,
  ...
  'A46: five-schema routines 308 (138''s kernel.bootstrap_organization, kernel.invite_bootstrap_owner and catalog.bootstrap_venue + 139''s notify.claim_report_delivery and release_report_delivery + 135''s 3 notify: issue_push_token_challenge, get_push_token_challenge, record_push_token_challenge_delivery + 131''s 4 kernel: invalidate_push_bindings_for, trg_push_bindings_on_password_change, trg_push_bindings_on_sessions_gone, push_session_predates_epoch + 111''s 3 kernel + 228 + 092''s 15 + 093''s 16 + 095''s 7 + 094''s 4 + 096''s 9 + 099''s 1 + 102''s 1 + 105''s 1 all kernel + 108''s 4 venue + 109''s 1 kernel + 1 catalog + 110''s 1 kernel + 113''s 2 venue + 114''s 2 venue)');
