...
SELECT is((SELECT count(*)::int FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='notify' AND c.relkind='r'), 9,
  'A7: notify holds exactly 9 tables — 076''s outbox + the six (NO schedule, NO announcement: OR-5 [C]) + 135''s push_token_challenges + 139''s report_delivery_claim');
...
SELECT is((SELECT count(*)::int FROM pg_proc WHERE pronamespace='notify'::regnamespace), 23,
  'A14: notify holds 23 routines — 141''s retire_account_deletion_pending (F-NOTICE-1, 2026-09-17) + 076''s emit pair + 092''s fifteen (the reduced 16 minus emit_event; no announcement RPC, no sweep_scheduled) + 135''s issue_push_token_challenge, get_push_token_challenge and record_push_token_challenge_delivery + 139''s claim_report_delivery');
...
SELECT is((SELECT count(*)::int FROM pg_proc p WHERE p.pronamespace='notify'::regnamespace AND p.proname NOT LIKE 'emit_event%'
             ...
             AND 'search_path=""' = ANY(p.proconfig)), 21,
  'A16: all fifteen + 135''s three + 139''s one + 141''s retire_account_deletion_pending (F-NOTICE-1, 2026-09-17) are SECURITY DEFINER, owned by postgres, search_path = '''' (067 discipline)');
...
SELECT is((SELECT count(*)::int FROM notify.notification_type), 32, 'A22: registry seeds exactly the 31 reduced IN types + 135''s security_device_rebound (the 31 reduced IN types (ODR-3 §1 + OR-15 + OR-17)');
SELECT is((SELECT count(*)::int FROM notify.notification_type WHERE delivery_class='mandatory'), 30, 'A23: 30 MANDATORY (135''s security_device_rebound + 29: 29 MANDATORY (21 ODR-3 + 6 N3 refund/approval + the OR-15 ninth + the OR-17 pair)');
...
SELECT is((SELECT string_agg(type_key, ',' ORDER BY type_key COLLATE "C") FROM notify.notification_type),
  'account_deletion_completed,account_deletion_pending,event_cancelled,event_postponed,event_time_changed,event_venue_changed,ownership_changed,payout_failed,payout_on_hold,payout_released,payout_request_pending_approval,promoter_commission_accrued,purchase_confirmed,purchase_failed,refund_completed,refund_failed,refund_request_approved,refund_request_cancelled,refund_request_denied,refund_request_expired,refund_request_parked,refund_requested,refund_submitted,security_device_rebound,security_org_role_granted,security_org_role_revoked,security_password_changed,security_payout_destination_changed,security_payout_method_added,staff_payout_failed,ticket_ready,wallet_pass_available',
...
SELECT is((SELECT count(*)::int FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='kernel' AND c.relkind='r'), 32, 'A43: kernel tables 32 — 111''s signing_key_recovery_approval, 094''s kernel.organization_obligation and 096''s payout_reversal/organization_obligation_recovery are the only relations added since 091');
...
SELECT is((SELECT count(*)::int FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname IN ('kernel','venue','catalog','market','notify') AND c.relkind IN ('r','p','v','m','S','f')), 81, 'A45: five-schema relations 81 (139''s notify.report_delivery_claim + 135''s notify.push_token_challenges + 111''s kernel.signing_key_recovery_approval + 69 post-091 + 092''s 6 + 094''s kernel.organization_obligation + 096''s two)');
...
SELECT is((SELECT count(*)::int FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname IN ('kernel','venue','catalog','market','notify')), 306,
  ...
  'A46: five-schema routines 306 (141''s notify.retire_account_deletion_pending (F-NOTICE-1) + 139''s notify.claim_report_delivery and release_report_delivery + 135''s 3 notify: issue_push_token_challenge, get_push_token_challenge, record_push_token_challenge_delivery + 131''s 4 kernel: invalidate_push_bindings_for, trg_push_bindings_on_password_change, trg_push_bindings_on_sessions_gone, push_session_predates_epoch + 111''s 3 kernel + 228 + 092''s 15 + 093''s 16 + 095''s 7 + 094''s 4 + 096''s 9 + 099''s 1 + 102''s 1 + 105''s 1 all kernel + 108''s 4 venue + 109''s 1 kernel + 1 catalog + 110''s 1 kernel + 113''s 2 venue + 114''s 2 venue)');
...
SELECT is((SELECT count(*)::int FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='notify' AND c.relkind='r' AND c.relrowsecurity), 9, 'B13: RLS enabled on all nine notify tables (incl. 135''s push_token_challenges and 139''s report_delivery_claim)');
