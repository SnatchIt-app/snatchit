...
-- Fixture: D becomes a releasable, paid, live obligation with no payout yet.
UPDATE public.payments SET status = 'succeeded', paid_at = now(), stripe_livemode = true WHERE id = tap.payment_d();
...
VALUES ('cccccccc-0000-0000-0000-0000000000d1', tap.listing_d(), tap.payment_d(), tap.seller(), tap.buyer(), 'mobile_transfer', 'auto_released', now(), now(), NULL, now() + interval '72 hours');
CREATE TEMP TABLE _d1 AS SELECT * FROM public.claim_payout_attempt('cccccccc-0000-0000-0000-0000000000d1', 'edge:test');
SELECT is((SELECT state FROM public.payout_attempts WHERE id = (SELECT attempt_id FROM _d1)), 'claimed', 'R2 fixture: D attempt claimed');
-- MINOR-5: the last-moment re-check — a refund landing between claim and POST stops the POST.
SELECT is((public.record_payment_refund('pi_fixture_d', 're_d_part', NULL, 500, 'dashboard'))->>'amount_refunded_cents', '500',
  'R2 fixture: a partial refund lands after the claim');
SELECT is(public.mark_payout_requested((SELECT attempt_id FROM _d1)), false,
  'R2 MINOR-5: mark_payout_requested refuses (nothing is sent) once the payment is partially refunded');
SELECT is((SELECT state FROM public.payout_attempts WHERE id = (SELECT attempt_id FROM _d1)), 'claimed',
  'R2 MINOR-5: the attempt stays claimed (the next claim reconciles it)');
SELECT is((public.reconcile_payout_attempt((SELECT attempt_id FROM _d1), NULL))->>'state', 'failed', 'R2 fixture: closed as failed');
-- MINOR-3: a partially refunded payment is never paid automatically.
SELECT throws_ok(
  $$ SELECT * FROM public.claim_payout_attempt('cccccccc-0000-0000-0000-0000000000d1', 'edge:test') $$,
  'P0001', 'PAYMENT_PARTIALLY_REFUNDED',
  'R2 MINOR-3: claim refuses a partially refunded payment (operator decision, DAY5 playbook)');
-- MAJOR-2: a refund AFTER payout flags the paid transfer for reversal.
SELECT is((public.record_payment_refund('pi_fixture_a', 're_a_after', NULL, 3000, 'dashboard'))->>'status', 'succeeded',
  'R2 fixture: a partial refund on A after its payout (tr_y)');
SELECT is((SELECT count(*) FROM public.payout_decisions
            WHERE transfer_id = tap.transfer_a() AND decision = 'manual_review' AND 'PARTIAL_REFUND_AFTER_PAYOUT' = ANY(reason_codes)), 1::bigint,
