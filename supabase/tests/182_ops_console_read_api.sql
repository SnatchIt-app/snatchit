...
-- Amended 2026-09-25 (migration 150): + refund_state_detection_enabled (seeded false) + refund_state_pending_hours.
SELECT is(jsonb_array_length(ops.settings()), 14, 'B31: settings() lists the 14 seeded console settings (8 from 115 + actions_enabled from 118 + refund_resolution_detector_enabled from 144 + cron_job_first_seen from 145 + alert_delivery_enabled from 146 + refund_state_detection_enabled, refund_state_pending_hours from 150) for a platform_admin');
SELECT tap.logout();
