...
SELECT is(jsonb_array_length(ops.settings()), 12, 'B31: settings() lists the 12 seeded console settings (8 from 115 + actions_enabled from 118 + refund_resolution_detector_enabled from 144 + cron_job_first_seen from 145 + alert_delivery_enabled from 146) for a platform_admin');
SELECT tap.logout();
