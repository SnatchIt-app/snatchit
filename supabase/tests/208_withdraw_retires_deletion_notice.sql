...
--
-- Every assertion here fails on 077's body; the negative controls fail if 141's
-- predicate is widened. Runs as postgres inside BEGIN … ROLLBACK like every suite.
-- =============================================================================
BEGIN;
SELECT plan(19);
...
SELECT is((SELECT count(*)::int FROM notify.notification
            WHERE (read_at IS NOT NULL OR dismissed_at IS NOT NULL)), 1,
  '208 C3 [blast radius]: exactly ONE notification row in the database is retired');
