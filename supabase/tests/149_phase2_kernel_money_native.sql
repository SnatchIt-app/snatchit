...
SELECT is((SELECT count(*)::int FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace
  ...
  'A1: kernel holds 32 tables — 111''s signing_key_recovery_approval + 22 post-084 + the four money ledgers + 088''s dispute_native + 091''s reserve stub + 094''s organization_obligation + 096''s payout_reversal/organization_obligation_recovery');
...
SELECT is((SELECT count(*)::int FROM pg_policy p JOIN pg_class c ON c.oid=p.polrelid
  ...
  'A17: 085 added ZERO policies (money tables deny-all, GP-3a); register now 72 after 092''s five notify owner policies');
