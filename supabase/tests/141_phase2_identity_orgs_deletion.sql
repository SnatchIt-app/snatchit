...
SELECT is(
  ...
  159, '077 A14: exactly 153 kernel  [138 +2 kernel: bootstrap_organization, invite_bootstrap_owner] [131 +4 kernel: invalidate_push_bindings_for, trg_push_bindings_on_password_change, trg_push_bindings_on_sessions_gone, push_session_predates_epoch]functions (111''s three recovery fns + 109 post-090 + 093''s sixteen + 095''s seven payout-state-machine + 094''s four obligation ones + 096''s nine payout-reversal/obligation-recovery ones + 099''s check_signing_key_invariants + 102''s get_ticket_signing_context + 105''s force_close_key_manifests + 109''s force_close_session_manifests + 110''s guard_signing_key_insert (106''s revoke un-park and 108''s two scan re-creates were REPLACED, not added); settlement_royalty_lines, get_payout_execution_context, close_settlement, resolve_organization_obligation, org_outstanding_obligation_minor, settlement_primary_lines, organization_obligation_guard, settlement_payout_maturity, settlement_maturity_hold_codes, record_dispute_native, mark_dispute_state, settlement_commission_lines, pay_promoter_commission and mark_payout_transfer_state were replaced, not added)');
