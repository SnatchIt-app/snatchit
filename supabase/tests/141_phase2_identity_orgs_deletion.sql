...
SELECT is(
  ...
  149, '077 A14: exactly 149 kernel functions (109 post-090 + 093''s sixteen + 095''s seven payout-state-machine + 094''s four obligation ones + 096''s nine payout-reversal/obligation-recovery ones + 099''s check_signing_key_invariants + 102''s get_ticket_signing_context + 105''s force_close_key_manifests + 109''s force_close_session_manifests (106''s revoke un-park and 108''s two scan re-creates were REPLACED, not added); settlement_royalty_lines, get_payout_execution_context, close_settlement, resolve_organization_obligation, org_outstanding_obligation_minor, settlement_primary_lines, organization_obligation_guard, settlement_payout_maturity, settlement_maturity_hold_codes, record_dispute_native, mark_dispute_state, settlement_commission_lines, pay_promoter_commission and mark_payout_transfer_state were replaced, not added)');
