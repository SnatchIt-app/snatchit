...
/**
 * LEGACY-AWARE pre-flight (release review, Agent 3 §2 finding 2). Transfers
 * created by the pre-ledger edges carried `metadata[transfer_id]` but NO
 * `transfer_group`, so `findTransferByAttempt` cannot see a legacy payout whose
 * `record_transfer_payout` write failed ("Payout sent but record update
 * failed"). Before the FIRST POST of any attempt we list the destination's
 * recent transfers and match `metadata.transfer_id` — a match is money already
 * moved for this obligation and is reconciled, never re-sent. One GET per
 * payout (three pages max); a listing failure means "do not POST this run".
 */
export async function findTransferByLegacyMetadata(
  destination: string,
  transferId: string,
  opts: { createdAfterEpoch?: number; maxPages?: number } = {},
): Promise<FindTransferResult> {
  const maxPages = opts.maxPages ?? 3;
  let startingAfter: string | null = null;
  const unmatched: string[] = [];
  for (let page = 0; page < maxPages; page++) {
    let q = `/transfers?destination=${encodeURIComponent(destination)}&limit=100`;
    if (opts.createdAfterEpoch) q += `&created[gte]=${Math.floor(opts.createdAfterEpoch)}`;
    if (startingAfter) q += `&starting_after=${encodeURIComponent(startingAfter)}`;
    let res: { ok: boolean; status: number; data: unknown };
    try {
      res = await stripeFetchRaw(q);
    } catch (err) {
      return { ok: false, error: err instanceof Error ? err.message : String(err) };
    }
    if (!res.ok) {
    ...
    const body = res.data as { data?: Array<{ id: string; metadata?: Record<string, string>; transfer_group?: string | null }>; has_more?: boolean };
    const list = body?.data ?? [];
    for (const tr of list) {
      if (tr?.metadata?.transfer_id === transferId || tr?.transfer_group === transferId) {
        return { ok: true, transfer: { id: tr.id }, unmatched };
      }
    }
    if (!body?.has_more || list.length === 0) break;
    startingAfter = list[list.length - 1].id;
  }
  return { ok: true, transfer: null, unmatched };
}
...
export type PayoutAttemptOutcome =
  ...
  | { kind: 'in_progress' }
  | { kind: 'reconciled';        attemptId: string; state: string; stripeTransferId: string | null; found: boolean; legacy?: boolean }
...
export interface ExecutePayoutArgs {
  ...
  actor:      string;
  /** transfers.created_at as epoch seconds — bounds the legacy pre-flight listing (optional) */
  createdAfterEpoch?: number;
}
...
export async function executePayoutAttempt(db: PayoutDb, args: ExecutePayoutArgs): Promise<PayoutAttemptOutcome> {
  ...
  // ── 2b. Legacy-aware pre-flight: money may already have moved for this
  //        obligation under the pre-ledger edges (metadata.transfer_id, no
  //        transfer_group). Found ⇒ reconcile this attempt to it, never POST.
  //        Listing failure ⇒ do not POST this run (the attempt stays claimed
  //        under its lease; the next claim reconciles it and re-evaluates).
  const legacy = await findTransferByLegacyMetadata(att.destination, args.transferId, {
    createdAfterEpoch: args.createdAfterEpoch ? args.createdAfterEpoch - 86400 : undefined,
  });
  if (!legacy.ok) {
    return { kind: 'reconcile_pending', attemptId: att.attempt_id, error: `legacy pre-flight: ${legacy.error}`, unmatched: [] };
  }
  if (legacy.transfer) {
    const rec = await db.rpc('reconcile_payout_attempt', {
      p_attempt_id: att.attempt_id, p_found_transfer_id: legacy.transfer.id,
    });
    if (rec.error) {
      return { kind: 'db_error', stage: 'reconcile', error: rec.error.message, attemptId: att.attempt_id,
               stripeTransferId: legacy.transfer.id };
    }
    const r = (rec.data ?? {}) as { state?: string; stripe_transfer_id?: string | null };
    return { kind: 'reconciled', attemptId: att.attempt_id, state: r.state ?? 'unknown',
             stripeTransferId: r.stripe_transfer_id ?? legacy.transfer.id, found: true, legacy: true };
  }
