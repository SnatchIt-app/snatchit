...
| **AN-2** | Refund amounts (known) | Today every refund amount is unknown (migration 120) | Blocked on A's ops-console partial-refund exactness migration (`ops.refund_facts`; number and contract live in A's `docs/release/MIGRATION_NUMBER_REGISTRY.md`, not here). Nothing shipped on this branch depends on it. The UI already renders known / unknown / mixed; no change to `120` semantics requested. |
