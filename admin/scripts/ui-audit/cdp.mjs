// Minimal Chrome DevTools Protocol client (copied from venue/scripts/acceptance/lib/cdp.mjs; adds viewport/key/send) (Node >= 22 built-in WebSocket; no dependencies).
  ...
  async function newPage({ width = 1280, height = 900 } = {}) {
    ...
    const page = {
      ...
      async screenshot(file) {
        const [w, h] = await evaluate("[document.documentElement.clientWidth, Math.min(document.documentElement.scrollHeight, 3000)]");
        const { data } = await s("Page.captureScreenshot", { format: "png", captureBeyondViewport: true, clip: { x: 0, y: 0, width: w, height: h, scale: 1 } });
