...
describe('payout race simulations (attempt protocol)', () => {
  ...
  it('LEGACY ORPHAN (release review A3 §2.2): a pre-ledger transfer with metadata.transfer_id and NO transfer_group is found by the destination listing and reconciled — no second POST', async () => {
    const w = world(); w.ledger.seedReleasable({ destination: 'acct_A' }); w.ledger.applyAutoRelease('t1');
    // the deployed edge POSTed this before the ledger existed and its record_transfer_payout write failed
    w.stripe.createTransfer('payout_t1_acct_A_src', { amount: 9000, destination: 'acct_A', transfer_group: null,
      metadata: { transfer_id: 't1', payment_id: 'p1', seller_id: 'seller-1', source: 'confirm-and-release' } });
    expect(w.stripe.created).toBe(1);
    expect(w.row().stripe_transfer_id).toBeNull();
    const out = await w.pay();
    expect(out).toMatchObject({ kind: 'reconciled', found: true, legacy: true, stripeTransferId: 'tr_1' });
    expect(w.stripe.created).toBe(1);                             // NOT re-sent
    expect(w.stripe.posts).toBe(0);                               // the new code never POSTed
    expect(w.row().stripe_transfer_id).toBe('tr_1');
    expect((await w.pay()).kind).toBe('already_released');
  });

  it('LEGACY pre-flight listing failure ⇒ no POST this run (fail closed); the next run reconciles and proceeds', async () => {
    const w = world(); w.ledger.seedReleasable({ destination: 'acct_A' }); w.ledger.applyAutoRelease('t1');
    w.stripe.listFault = { kind: 'network' };
    expect(await w.pay()).toMatchObject({ kind: 'reconcile_pending' });
    expect(w.stripe.posts).toBe(0);
    expect(w.ledger.openAttempt('t1')?.state).toBe('claimed');
    w.ledger.advance(LEASE_MS + 1);
    // claim → needs_reconcile → group search (empty) → failed_not_created → new attempt → legacy listing (empty) → POST
    expect((await w.pay()).kind).toBe('reconciled');
    expect((await w.pay()).kind).toBe('succeeded');
    expect(w.stripe.created).toBe(1);
    expect(w.stripe.posts).toBe(1);
  });

  it('a foreign transfer at the same destination (other obligation) does not match the legacy pre-flight', async () => {
    const w = world(); w.ledger.seedReleasable({ destination: 'acct_A' }); w.ledger.applyAutoRelease('t1');
    w.stripe.createTransfer('payout_t9_acct_A_src', { amount: 5000, destination: 'acct_A', transfer_group: null, metadata: { transfer_id: 't9' } });
    expect((await w.pay()).kind).toBe('succeeded');
    expect(w.stripe.created).toBe(2);
    expect(w.stripe.posts).toBe(1);
  });
