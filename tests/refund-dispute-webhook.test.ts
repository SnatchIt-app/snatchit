...
describe('charge.dispute.created / transfer.created / transfer.reversed', () => {
  ...
  it('dispute.created freeze failure ⇒ non-2xx, no dispute upsert this delivery (review round 2 MINOR-5)', async () => {
    const h = await load({ rpcFail: { freeze_transfer_for_dispute: 'connection reset' } });
    const res = await h.send('charge.dispute.created', { id: 'dp_2', charge: 'ch_1', payment_intent: 'pi_1', amount: 11000, currency: 'usd', reason: 'fraudulent', status: 'needs_response' });
    expect(res.status).toBeGreaterThanOrEqual(500);
    expect(h.rpcNames()).toContain('freeze_transfer_for_dispute');
    expect(h.rpcNames()).toContain('fail_stripe_webhook_event');
    expect(h.sb.queries.filter((q) => q.table === 'disputes' && q.op === 'upsert')).toHaveLength(0);
  });
