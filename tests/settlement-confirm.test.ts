...
describe('confirm-payment — kernel-rail payments are refused (review round 2 MAJOR-1)', () => {
  it('outcome not_external_rail ⇒ 409 with code, no success flag', async () => {
    const s = await scenario({ outcome: 'not_external_rail' });
    expect(s.res.status).toBe(409);
    expect(s.body).toEqual({ error: 'This payment is not a marketplace resale payment.', code: 'not_external_rail' });
