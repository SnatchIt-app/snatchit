...
precheck_common() {
  "${PSQL[@]}" -tA <<'SQL'
select 'server_ref_ok='||(current_setting('server_version')<>'')::text||' role='||current_user;
...
case "$MODE" in
  ...
  url)
    precheck_common; require_logging_safe; require_name_absent project_url
...
SQL
  ...
  key)
    precheck_common; require_logging_safe; require_name_absent service_role_key
