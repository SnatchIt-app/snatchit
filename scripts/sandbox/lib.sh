...
stripe_mode_guard(){ local lm; lm=$(stripe balance retrieve 2>/dev/null | jq -r '.livemode // "unknown"'); [ "$lm" = "false" ] || { echo "REFUSING: stripe CLI is not in test mode (livemode=$lm)"; exit 3; }; }
