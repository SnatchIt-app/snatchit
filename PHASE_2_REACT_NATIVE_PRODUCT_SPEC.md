...
**Binding inputs:** `SPEC_FOUNDATION.md` (shared vocabulary; §9 product language is a hard rule), `SNATCH_IT_DOMAIN_ARCHITECTURE.md` (two rails, venue ops), `SNATCH_IT_CANONICAL_DATA_MODEL.md` (object states), `PHASE_2_IMPLEMENTATION_ROADMAP.md` (build order), `PHASE_2_FINAL_ARCHITECTURE_AUDIT.md` (product findings). Companion deliverables: schema (1), migration (2), RLS (3), RPC (4), edge (5), this file (6), review (7).
...
Every backend state the UI references, and where it lives (for reviewer cross-check against schema/RPC specs). Exact enum labels to be pinned against `SNATCH_IT_CANONICAL_DATA_MODEL.md` §1.
