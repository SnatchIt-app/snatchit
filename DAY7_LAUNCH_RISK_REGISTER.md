...
| **Mitigation** | (1) Stripe webhook retries (up to 3 attempts over 24h). (2) Daily monitoring query to find payments without matching transfers (see DAY7_DAILY_MONITORING_CHECKLIST.md). (3) Manual recovery: admin creates transfer row via SQL using DAY5_ADMIN_SQL_PACK.sql. |
...
| **Mitigation** | (1) Edge function logs refund failures with full error details. (2) Daily monitoring query checks for expired transfers without a corresponding Stripe refund. (3) Admin uses DAY5_MANUAL_REFUND_PLAYBOOK.md to issue manual refund via Stripe Dashboard. |
| **Owner** | Admin / Ops |
