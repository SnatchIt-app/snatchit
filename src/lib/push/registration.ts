...
export type RegistrationErrorKind =
  ...
  | 'session_stale'      // 131 (provisional): 42501 'session predates a credential change' — terminal until this device re-authenticates
...
export function decideRegistration(i: DecisionInput): Decision {
  ...
  if (failure) {
    ...
    // 131: only a new session changes the answer; the hook signs this device
...
export function classifyRegistrationError(err: ErrorLike | null | undefined): RegistrationErrorKind {
  ...
  if (code === '42501') {
    ...
    // 131 (provisional): the session predates the user's binding epoch (credential change or sign-out everywhere), or its row is gone.
