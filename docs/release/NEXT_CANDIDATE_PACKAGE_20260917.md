...
|---|---|
| **Build** | **Hold condition met at `f412d10`:** the owner held the build until C's Keep editing fix (F-NAV-1) passed D's review and was integrated. D PASSED 2ba9e3a (product) and 76b8622 (the required test additions); A integrated it at `f412d10`. **CI 35237352892 on `f412d10`: SUCCESS on all five jobs** (Typecheck/Lint/Unit tests; Migrations apply cleanly on a fresh DB; Admin console; Web build; Deno type-check). **Line 1 is issued once D's merge-preservation gate on `f412d10` is green** |
...
| Tag line | will name **`f412d10`** (= `db16e1a` + `frontend/unsaved-guard-native-dismiss @ 76b8622`, --no-ff; tree `0409138f`, identical to 76b8622's tree). `candidate/2026-09-18-build-c3` does not exist |
