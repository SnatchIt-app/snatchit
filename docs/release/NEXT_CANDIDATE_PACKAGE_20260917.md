...
|---|---|
| **Build** | **HELD.** The owner's hold came first through C and then directly to A: no build until C's Keep editing fix (**F-NAV-1**) has passed D's review and been integrated |
...
| Tag line | will name **`db16e1a` + F-NAV-1**, head not yet sent by C. `candidate/2026-09-18-build-c3` does not exist |
...
| **F-NAV-1** | C; head not yet sent | D review when sent | Keep editing keeps Edit listing open after swipe-back and after the back button; behavioural test + mutants per path; gated-surface and `supabase/` diff (requirements sent to C) |
...
| Row | Class | Exactly |
|---|---|---|
| **F-NAV-1 re-check** | N | Edit listing on a bid-free listing (Phone P1 `c343406e-be85-49c1-9951-ac08bb1daab2`; Save changes is never tapped): change a field → swipe back → **Keep editing** → still on Edit listing **with the change visible**; repeat with the header back button; then Discard → My Listings |
