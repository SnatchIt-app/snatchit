...
| **A-12** | A | **133** config-driven functions URL (Vault `project_url`; 4 bodies + 5 crons; queue purge at apply) — owner item 6 permanent fix. Written, rehearsed (150/150, census unchanged, rollback md5-identical, 200 24/24 + negative control 16 not ok; 165 C2/C3 updated) | D review; CI run 34980799558 | Wed | registry row; D's review |
| **A-13** | A | **A-131-K2** amendment on 131 (D's K2-S1 MEDIUM + K2-S2 LOW): session-stamped bindings, per-session revoke in the sessions trigger, deleted-session JWT fails closed. Rehearsed: 198 56/56, full suite 5030/5030, two negative controls, rollback identity | D re-review; CI run 34980745488 | Wed | 131 branch `f72e2d3` |
...
| **B-3** | B | **132** (REQUIRED before production, owner 2026-09-15) — Option B pre-mint group record (`checkout_group_claim`), PR #70 @ `acbd5dd`, CI green; **D: NOT PASSED — F-132-1 HIGH cross-mode double mint (key includes mode), F-132-2 HIGH cross-buyer retire is best-effort**; fix on the branch, D re-reviews the full battery at the fixed head | D | Wed | PR #70; D's review log |
