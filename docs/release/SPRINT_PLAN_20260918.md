...
| **A-17** | A | **135** proof of possession: challenge table + verbs + `challenge_required` branch + tombstoning client DELETE + proof supersession + in-app notice; pgTAP 202 with negative controls per route; rollback | A-16 | Fri 19 | branch `fix/135-push-proof-of-possession` |
| **B-4** | B | `send-push` challenge kind (silent + visible code), rate-limit re-check, nonce never logged, vitest incl. a log-the-nonce mutant; support runbook draft | A-16 (contract); 135 for integration only | Thu 18 | branch `fix/b2-send-push-challenge` |
