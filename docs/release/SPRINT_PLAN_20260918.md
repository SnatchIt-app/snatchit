...
| C-5 ◐ | C + owner | Handset session 1 on build 17: Block 0 ✔ (registration recovered via Path B, no mutation); Block 1 rows 1–8 PASS (owner, 2026-09-16; labels/timings not captured for every row); rows 9–14 in progress; rows 15–18 on C's triggers with A's read-backs. **F-FE-1 (owner): seller form — the action bar sits far above the keyboard (large gap), the heading crowds the sandbox banner → C fixes in the next candidate's frontend workstream; build 17 preserved as the tested artifact** | A read-backs | Thu | plan rows; manifest §10 |
