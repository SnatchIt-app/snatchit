...
| A-17 ◐ | A | **135 written** `fix/135-push-proof-of-possession @ b49a012` (CI 35050777765): challenge table + verbs + `challenge_required` + tombstoning client DELETE and support unbind (RB-1) + proof supersession + stale-nonce generation + in_app notice; 202 57/57; full suite 5180/5180; rollback identity. Disclosure: two red pushes before it, corrected | D's C1–C5 matrix | Wed 17 | registry row |
| B-4 ◐ | B | `send-push` challenge kind PR #71 @ d710772 (D PASSED C6's edge half; SP-1 fixed); one change pending: read the challenge via `notify.get_push_token_challenge` (no notify table grant) | A-16 done; re-check by D | Wed 17 | PR #71 |
