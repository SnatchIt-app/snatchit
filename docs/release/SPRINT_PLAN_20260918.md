...
| A-15 ✔ | A | **134** = `20260916000000_processing_sweep_arm.sql` + edge Phase 0 branch (B's disclosed 132 residual; D: not blocking) — `fix/134-processing-sweep-arm @ 5146040`, CI 35046284427 green; **D PASSED**; **integrated: stack `d61970b` = 131 + 132 + 133 + 134** (153/153, 32|102|37|37, 5123/5123; CI 35046570214). Versioned by timestamp (registry rule) | — | done | registry row |
...
| C-5 ◐ | C + owner | Handset session 1 on build 17: Block 0 ✔ (registration recovered via Path B through normal app-driven writes, no support override); Block 1 rows 1–8 PASS (owner, 2026-09-16; labels/timings not captured for every row); row 9 (Reduced Motion) PASS; row 11 (large text) PASS; row 14 offline PASS (F-OFF-1 LOW wording difference, next candidate); row 10 VoiceOver UNTESTED at the owner's request (A11Y-1 follow-up); rows 12–13 and row 14's filtered/empty checks pending (not inferred); rows 15–18 on C's triggers with A's read-backs. **F-FE-1 (owner): seller form — the action bar sits far above the keyboard (large gap), the heading crowds the sandbox banner → C fixes in the next candidate's frontend workstream; build 17 preserved as the tested artifact** | A read-backs | Thu | plan rows; manifest §10 |
...
| **B-3** | B | **132** (REQUIRED before production, owner 2026-09-15) — Option B pre-mint group record (`checkout_group_claim`), PR #70; D's F-132-1 HIGH (cross-mode) and F-132-2 HIGH (cross-buyer best-effort retire) **fixed at `ebbd1c0`** (group key (listing, buyer); edge reads across modes, fails closed on other buyers' processing/uncancellable intents), **D PASSED 132 at `9d82247` (2026-09-16)** after B's `record_checkout_attempt` (FOR SHARE), F-132-3 sweep, and a concurrent-withdraw fix; processing residual NOT BLOCKING → 134 proposed. **Integrated: production-gate stack = 131 + 132 + 133** — `6b058d2` (D incremental PASS: harness 25/0/2, reverse-order rollback identity, CI 35045310911) → **`cb68811`** after B's final 74a4371 (D's N-132-1, edge/tests only, D passed the delta; CI 35045923123). N-132-2 disclosed residual (availability, ms window; no change wanted by D or B) | — | done | PR #70; D's review log |
...
| **A-16** | A | **Contract v3** (`PUSH_TOKEN_CONTRACT_V3.md`, draft 2026-09-16) — the text B and C code against; D reviews first | — | Wed | D's review |
| A-17 ✔ | A | **135** `fix/135-push-proof-of-possession @ 38b1e02` — **D Gate 1 PASS** (C1–C6 through real writers; rollback exact; two LOW fixed: bigint code generator, in-app comments). Disclosure: two red pushes before b49a012, corrected. Next: merge 135 → send-push onto the stack on D's delta ack | D delta ack | Wed 17 | registry row |
| B-4 ✔ | B | `send-push` challenge kind PR #71 @ `c92d7c7` (CI green incl. Deno): reads via `notify.get_push_token_challenge`, ownership check (body user_id = requesting_user, else 409), rate key token_id + requester from the row, neither field in payload/logs (A21), seven mutants dead; **D re-checked: passes**. Merges after 135 | — | done | PR #71 |
| C-7 ◐ | C | **Client v3** `frontend/push-proof-v3 @ 0eea9c3` (all four rulings applied: only `rebound` binds, `stale_nonce` neutral, P3-1 re-request, final reply list; 21/21 RED-first; no gated file) — with D for re-review; merges after 135 + send-push. **On the stack already (CI green at `06d414f`, run 35050897533):** K-2/131 delta (`27c0b7d`), F-SELL-1 (`2b9559e`), the owner-requested Premium StateView unification (`bf8b9ba`, D-approved with SV-1/SV-3), all with a zero gated diff; typecheck 0, vitest 1996/1996 | D | Wed 17 | stack head `06d414f` |
| **D-8** | D | Contract v3 review (today) → 135 C1–C6 matrix → send-push tests → C's branches (second read) → combined stack at one commit → device rows review | each piece as it lands | Mon 22 | review log |
| A-18 ✔ | A | **PINNED 2026-09-16: tag `candidate/2026-09-18-pin-b2` = `release/production-gate-20260918 @ 9bef640`** (combined b2 commit `0e8de77` + C's test-only classifier-migration guard). CI at the pin 35053607616 green 5/5 (pgTAP files=85, tests_ran=5186 PASS, masking gate 0). D Gate 3 PASS (D log `26d69d9`): merge chain byte-checked against the passed heads; rehearsal 27/0/WARN 3; census 32\|105\|37\|38; reverse-order rollback 0 lines differing; C1–C6 0 errors, negative controls flip; push/auth 97/97; twelve client-keyed raise texts verified with comments stripped; guard fails on three mutations. `candidate/2026-09-18-pin` (`aabe029`) kept, untouched. D-135-5 (LOW, B's `SUPPORT_RUNBOOK_PUSH_TOKEN_UNBIND.md` still described the pre-RB-1 DELETE) → B fixed docs-only (`5225557` D-135-5, `84ddd9a` D-135-6, both D PASS), integrated at `5e8b0f4` and `e6d9f2e` on top of the pin (one docs file, applied bytes unchanged, tag stays). **Owner APPROVED the sandbox-only package in principle 2026-09-16** (conditions met: D's runbook review complete, tag + hash published, package names `9bef640`, CI + Gate 3 tied to it; notice stays in-app only; build only after application + edge verification; exclusions recorded in package §9). **Open before the window:** the ruling's order "133 → 134 → 135" must be confirmed by the owner as the canonical `133 → 135 → 20260916000000` (D's point; package §3). Pre-flight run 04:10Z: ledger 136, flags false, L-1 0, census 31\|97\|37\|34, push_tokens 1, sandbox Vault empty (no `service_role_key`, no `project_url`), production-host drift 0/0 (sandbox bodies name the sandbox host, 133 replaces them) | owner: order confirmation + "open" | Thu 17 window / Fri 18 build | packet, package **WINDOW EXECUTED 2026-09-16 04:35–04:56Z:** Vault `project_url` + 131 → 132 → 133 → 135 → 20260916000000 applied (ledger 141, each md5 = pinned bytes), three edges deployed with byte parity (`stripe-webhook` unchanged), five refused 401 ticks and zero drift, D closing read PASS; b2 device verification DEFERRED (option (b)); manifest §11 |
...
| O-9 production-gate pin + build | **Pin DONE 2026-09-16:** `candidate/2026-09-18-pin-b2` = `9bef640` (131+K2, 132, 133, 134, 135, their edges, C's client v3 and reviewed client work). **NEEDED from the owner:** (a) `SANDBOX_APPLICATION_PACKAGE_B2.md` as written, (b) the pin, (c) the sandbox window, (d) §8 decision on the in-app-only `security_device_rebound` notice (accept, or authorize an email row). The one combined build was authorized 2026-09-16 "after the combined commit passes required reviews and CI" — met at the pin; A recommends C cuts it from the tag after the sandbox application completes, owner's call **WINDOW DONE 2026-09-16 (manifest §11).** NEEDED next: the owner's word to C to cut the one build from `candidate/2026-09-18-pin-b2` (or a new tag if the sign-out fix `8dc4cec` is included — owner's word to A required), and the row-17 handset instruction **BUILD SOURCE TAGGED 2026-09-17: `candidate/2026-09-18-build-b2` = `aad5f75`** (9bef640 + runbook docs + C's 8dc4cec sign-in/sign-out deadlock fix + a609cbc push-token fetch visibility, both D PASS, owner ruling 2026-09-17; D verified the union byte-identical; CI 35175523163 5/5; tsc clean, lint 0 err, vitest 2077/2077 alone). `candidate/2026-09-18-pin-b2` = 9bef640 stays the sandbox application pin. C cutting the one authorized combined build from the tag; installation link to the owner on EAS finish **BUILD 18 FINISHED 2026-09-17** (C, from the tag in a clean worktree, `aad5f752ef69…`): EAS iOS preview/internal `dcbf20e0-76dd-4b18-a48a-20c203ba0175`, SDK 54.0.0, app 1.0.0, started 02:48:54Z; installation page https://expo.dev/accounts/jdt_inc/projects/snatchit/builds/dcbf20e0-76dd-4b18-a48a-20c203ba0175 — the one authorized combined build; link forwarded to the owner; session 2 block S2-1 (DV-AUTH-1) ready on the owner's install |
