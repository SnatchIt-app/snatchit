...
| Production payments split by `stripe_livemode`: 49 `false`, 2026-03-25 to 2026-07-29 19:51:05Z; then 8 `true`, 2026-08-04 01:00:22Z to 2026-09-03 14:40:20Z; no interleaving. Live sample: 2 succeeded, 2 refunded, 3 pending, 1 failed | D's read at about 04:50Z on 2026-09-24; historical | [D-PROD 04:50Z] |
...
- **The server code deployed 2026-09-23/24:** `create-payment-intent` v48, `confirm-payment` v37,
  ...
  `stripe-webhook` v42. No live payment has run through them. As of D's read at about 04:50Z on 2026-09-24, the newest payment of any kind was 2026-09-03 [D-PROD 04:50Z].
...
|---|---|---|---|---|
| V2 | Server key is live **and** of the same account as the build's `pk_live` | G4's method: the owner compares the Supabase digest of `STRIPE_SECRET_KEY` with the live key from the Stripe Dashboard using `scripts/owner/secret_digest_compare.zsh` (hidden input; the key never reaches a command line, shell history, process arguments, a file or a log). A match is conclusive; G4 says what a mismatch can and cannot mean. Alternatively V6 proves it | owner | no |
...
| "Payouts go straight to your bank through Stripe" | **reword**: "Sellers are paid through Stripe Connect once the sale completes." | legacy path paid 23 transfers, including a live one 2026-08-04 [REC `SPRINT_STATUS:1405,1412`]; the payout code deployed 2026-09-23 has not yet run in production [REC: `payout_attempts` 0 at A's authorised 149 P3 read, 2026-09-24 20:30:20Z, `evidence/149_20260924/A/out/P3_prestate.txt`; REC SPRINT_STATUS:1412] |
| "Confirm receipt and the sale completes" | **keep** | [SRC] `confirm-and-release` |
| "open a dispute and our team steps in" | **reword**: "If the tickets don't arrive, report it in the app; the seller's payout is frozen." Stop there (D's review): a review process is not evidenced, since `dispute_resolutions` held 0 rows and 5 disputes were open at A's authorised 149 P3 read, 2026-09-24 20:30:20Z [REC `evidence/149_20260924/A/out/P3_prestate.txt`]. | [SRC] `buyer_dispute_transfer` (0550:207-225) sets `disputed_at`, which every payout gate checks. "Our team steps in" is **omitted**: operator alerts are undelivered (`alert_delivery_enabled` false, no dispatcher; admin push to `admin_users` only, 0 of 2 admins with an active push token [REC SPRINT_STATUS:1377]; email off: `EMAIL_ENABLED` absent from the 24 secret names [REC: A's authorised read, 2026-09-17, `SIGNING_MONITOR_ALERT_CHECK_CLEAN_RUN_20260917.md` R7]), and the review promise has no operating process (§5b) |
...
- **Tables record** [D-PROD 05:08Z, as of that read. It is D's second read; it corrected D's 04:50Z count of 5]: 7 payments `refunded` (5 test-mode, 2 live-mode). 4 carry a `stripe_refund_id`
...
- **History establishes:** real Stripe refunds occurred in test mode. The two live-mode sibling orders of 2026-08-04
  ...
  **2** transfers are `reversed`, matching the 2 live-mode refunded payments of 2026-08-04 [D-PROD 05:15Z].
...
- **Affected rows: 0** at A's authorised 149 P3 read, 2026-09-24 20:30:20Z. That read counted rows in the seller-win
  shape: `status 'buyer_confirmed'`, `buyer_confirmed_at` NULL and `dispute_resolution 'resolved_seller_paid'` [REC
  `evidence/149_20260924/A/out/P3_prestate.txt`]. D's wider read at 05:15Z found all 19 `'buyer_confirmed'` transfers
  carrying a timestamp [D-PROD 05:15Z]. The zero exists only because **no dispute has been resolved**: at the same P3
  read `public.dispute_resolutions` held 0 rows and **5 disputes were open** (`dispute_resolved_at` NULL) [REC]. D's
  05:15Z read matched those 5 to 5 `dispute_open` ops cases [D-PROD 05:15Z]. The first seller-win resolution
  would have hit this path. D confirmed the three source anchors independently.
- **Fixed in production 2026-09-24:** #92 (148 + `enforce-transfer-expiry` v41, 16:55–16:57Z) pays seller-wins with
  holds respected; 149 + `confirm-and-release` v38 (20:31Z) keeps their audit rows truthful (R1). The 5 disputes remain
  unresolved; no action on them is authorised.
...
- **Production inventory, the hardest blocker.** As of D's read at about 05:15Z on 2026-09-24 there were 113 listings,
  **exactly 1** active and unexpired: III Points Saturday GA, ends 2026-10-18, Buy Now **$300** (so a reviewer could
  complete a $330 charge). 66 more were active with an end time already past [D-PROD 05:15Z]. These counts are
  volatile: an authorised read must re-measure them before any action relies on them. At that read a reviewer
  would have seen one purchasable item. Owner action O8;
...
| G3 | **Live webhook.** Stripe Dashboard → switch to **Live** → Developers → **Workbench** → **Webhooks** tab (`dashboard.stripe.com/webhooks`) → select the destination `https://hqycwntpfoztoinemqns.supabase.co/functions/v1/stripe-webhook` → **Overview**. On the older Developers dashboard: Developers → Webhooks → the endpoint. The docs don't give the exact Overview labels. **What a yes shows: configuration only.** Enabled and the event list show what Stripe is set to send. They are **not** evidence that an event is delivered, that its signature verifies against `STRIPE_WEBHOOK_SECRET`, or that handler v42 processes it. Only a delivered live event shows that (E1 or V6, each needing its own permission) | enabled yes/no; the event list, and specifically **is `payment_intent.canceled` there?** Expected set (11): `payment_intent.succeeded/.payment_failed/.canceled`, `charge.refunded`, `charge.dispute.created/.closed`, `account.updated`, `transfer.created/.reversed`, `payout.paid/.failed` | V3; whether E1/V6 can prove delivery |
| G4 | **Secret digests, compared without exposing the secret.** *What the digest is known to be* (A, 2026-09-24, a read-only read of the **sandbox** secrets list, `scratchpad digest_probe/result.txt`): every listed value is 64 hex characters, and `SUPABASE_URL`'s is exactly SHA-256 of the URL's bytes with no trailing newline. The discriminators did not match: the value plus a newline, SHA-1, and the other control's value. *Not known:* that secrets set by hand, as the Stripe ones are, are digested the same way. A second control, the anon key from `eas.json`, did **not** match. The likely cause is that the platform injects a different key, which A could not test without reading keys, so it is recorded as unexplained. A's test of a hand-set secret was stopped by a permission check because it guessed candidate values, and A did not pursue it. **So step 1 is a control on the production page itself.** The tool: `zsh ~/snatchit-converge/scripts/owner/secret_digest_compare.zsh` (sha256 `a4f1a636…0866`). It asks for the digest (visible), then the secret with echo **off**. The secret passes only through a zsh builtin into a pipe to `/usr/bin/shasum`, so it never reaches a command line, shell history, `ps`, a file or a log. The tool unsets it, prints only its kind (live or test key, signing secret) and length, and clears the clipboard. Clipboard-history apps keep their own copy; clear it there too. (1) Supabase Dashboard → project `hqycwntpfoztoinemqns` → Edge Functions → **Secrets**. **Control:** run the tool with the `SUPABASE_URL` row's digest and, as the value, `https://hqycwntpfoztoinemqns.supabase.co`. It must say MATCH; the expected digest is `3551ab7a…dcdc2ae`, computed locally. If it does not, **stop**: the page's digest is not directly comparable, and you should use E2 instead. (2) Stripe **Live** → `dashboard.stripe.com/apikeys` → Standard keys → **Reveal live key**. Only Stripe-created keys can be revealed; if yours cannot, say so and use E2 instead, and **never rotate for this**. Run the tool with the `STRIPE_SECRET_KEY` digest. (3) The signing secret: G3's endpoint → **Reveal secret**; run the tool with the `STRIPE_WEBHOOK_SECRET` digest. **Reading the result:** MATCH is conclusive. MATCH WITH A TRAILING NEWLINE means the same value, stored with a newline; report it. NO MATCH after a passing control most likely means a different value: another or rolled key, a test key, another account, or another endpoint's signing secret. It is **not proof**, because hand-set secrets are unverified. One sandbox write would settle that: set and then remove a throwaway secret with a known, non-secret value. That needs the owner's authorisation. Never rotate on a mismatch; report it | control MATCH yes/no; for the key and for the signing secret: MATCH / newline / NO MATCH / not revealable, with the kind line | V2 (server key live and of the build's account) |
| G5 | **Apple Pay certificate.** Apple Developer → Certificates, Identifiers & Profiles → **Identifiers** → top-right filter **Merchant IDs** → `merchant.com.snatchit` → **Apple Pay Payment Processing Certificate**: status and expiry (valid 25 months). Then Stripe **Live** → `dashboard.stripe.com/settings/ios_certificates` (a support article gives `…/settings/payments/apple_pay` for the same page): the certificate is listed. **Stripe does not show expiry.** If renewal is ever needed: new CSR from Stripe → Apple → upload the `.cer` to Stripe **before** Activate at Apple. Not authorised here. **What this shows: configuration only.** A valid certificate listed in Stripe is a precondition. It is **not** evidence that an Apple Pay payment works: V4 shows only that PaymentSheet offers Apple Pay, and only a completed Apple Pay payment would show that it works. None is authorised | expiry date; listed in Stripe yes/no | any Apple Pay line in the review notes (V4) |
...
any of them. D has been asked to confirm or correct this list from its own session, without new reads.
...
checklist cites none of them.

| # | Reported (UTC, 2026-09-24) | Purpose | Scope read (read-only) | Figures it supports, and where | Already measured by an authorised read? | A's recommendation |
|---|---|---|---|---|---|---|
| R1 | ~04:53Z | Answer the owner's live/test-mismatch question and what the submission may claim | `payments` counts by `stripe_livemode` and status, with first and last `created_at`; refund counts (`payment_refunds`, `amount_refunded_cents`); `payout_attempts` count and transfers with a Stripe transfer id; listings with `quantity > 1`; production edge-function secret **names** (24; no values) | §2: the livemode split 49/8 with its dates and no interleaving; the live sample 2/2/3/1; the newest payment 2026-09-03 (§2 "does not cover"). R1's refund count (5) was wrong and R2 corrected it | payout_attempts 0: yes (149 P3, 20:30:20Z). Email off (`EMAIL_ENABLED` absent): yes (A, 2026-09-17, R7). The split, sample and newest date: no | **Retain** the split and the newest-payment date, dated. They are historical facts that later rows cannot change, and they corroborate §2's [REC] account without carrying it. **Drop** the 2/2/3/1 sample: nothing depends on it. Not cited: the quantity>1 counts |
| R2 | ~05:08Z | Correct R1's refund count; the owner's three-way refund account | Every `payments` row with status `refunded`: livemode, whether `stripe_refund_id` is present, `refunded_at`, `amount_refunded_cents`; the `payment_refunds` count | §4 "Tables record": 7 refunded (5 test, 2 live); 4 with a refund id (dates); 3 without (dates, including the two live rows at 17:20:05Z and 17:20:19Z on 2026-08-04); amount NULL on all 7; `payment_refunds` 0 | payment_refunds 0: yes (09-23 window, REC SPRINT_STATUS:1412). The rest: no | **Retain**, dated. It is the only table-level leg of the account the owner asked for. No submission claim depends on it, because refund wording is omitted |
