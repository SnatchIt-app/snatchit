...
- **DV-ST2b — PREPARED, NOT STARTED (owner 2026-09-17: "Prepare DV-ST2b using the existing purchase rows, with A executing and D witnessing, but wait for my readiness and authorization before starting another restriction window").** No capture that changes anything and no revoke until the owner's "ready" arrives through C **and** the owner authorizes this restriction window.
  ...
  - **Build, and the expected result per build (D's review of F-BIDS-1, 2026-09-17):** record which build the handset runs. **Build 18 or Build 19 (c3; `bids.tsx` identical, F-BIDS-1 not included): the rows stay with NO notice.** **A build carrying F-BIDS-1 (C's `1ad216f`, a later candidate): the rows stay WITH "Couldn't refresh your bids and purchases. Showing what loaded earlier."** A result is judged against the expectation for the build actually installed, never the other one.
  - **Authorization line drafted for the owner (nothing runs until it is spoken):** "Run the DV-ST2b restriction window on the sandbox `ofaidukbieeekqaboscm` on C's trigger after my 'ready': A captures the `public.bids` grants, revokes SELECT from authenticated only after D's before-read, restores and verifies immediately after my one pull-to-refresh observation or on any interruption or the 360 s watchdog, and D's after-read must equal the before-read. At least 10 minutes after the window closes, A runs one read-only query of that project's API request logs, limited to `GET /rest/v1/bids` between the revoke time and the restore time, returning path, status and timestamp only, and **[D runs the same read independently | D reviews A's exact query, time range and raw output]**. PASS needs a denied response in that range plus 'rows stayed'; otherwise INCONCLUSIVE."
  - **D's witness terms (2026-09-17):** a separate witness pair (tag `st2b`; the ST2a files untouched, md5 `466fd2d8…` both). D reads nothing before C's trigger and A's capture. If the owner's line covers D's identical log read, D runs it and sends the rows; otherwise D reviews A's exact query text, time range and raw output and states that it reviewed rather than read. Rows outside the revoke→restore range, or for another endpoint, do not count.
...
### Step 2 — W-C3: EXECUTED AND CLOSED (V0 16:19:21Z → D's closing read 16:29:26Z)
Exec tree: detached worktree at `f412d10a11310167fc0227fe58ea189822bec625`, 0 dirty, unlinked; all ten package §3 sha256 values matched (PF1). Handset paused by C from 12:18 EDT (PF5). A's reads: scratchpad `wc3/wc3.log` (md5 `ec2872fb7c8a5fd5e02c81534d2440b4`).
| Point | A | D (independent) |
|---|---|---|
| V0 | 16:19:21Z: ledger 141 · census 32\|106\|37\|37 · notify 8 · new objects none · sandbox deltas 1\|1\|0\|0 · `mark_transfer_sent` void `bab0d402…`/955 and `c3281f0a…`/1042 · business 49/51/33/1/104/9/18, queue 0 · Vault `project_url` only · executors false · 2xx 0 | 16:20:00Z: agrees (md5 `2f14ffd9…`) |
| PF6 | functions list: notify-report v3 `90dc5e9b`, stripe-webhook v4 `897283ef`, verify_jwt false. Capture of both deployed bundles: **notify-report v3 is byte-identical to 9bef640's closure** (its previously unrecorded parity is now recorded), and stripe-webhook v4 is too, so both edge rollbacks redeploy from 9bef640 | pre-S4a list agrees |
| S1 136 | preflight OK; applied 16:20:53Z (CREATE FUNCTION ×2, INSERT 0 1); verify OK, ledger md5 `735b9920…`; read 16:21:32Z: ledger 142 · census 32\|108\|37\|37 · notify 8 · new true\|true\|false\|false · both notice functions EXECUTE anon/auth/svc false/true/false. **Disclosure:** A's first wrapper ran 136's read-only preflight, then broke on a zsh PIPESTATUS bug before apply; apply and verify then ran once under bash | 16:23:14Z: matches the written row (md5 `8b15ead8…`) |
| S2 139 | preflight OK 16:23:41Z; applied 16:23:55Z (table, index, RLS, 2 functions); verify OK, md5 `f36734e2…`; read 16:24:29Z: ledger 143 · census 32\|108\|37\|37 · notify 9 · new true\|true\|false\|true · `notify.report_delivery_claim` RLS on, no API-role SELECT | 16:24:55Z: matches (md5 `b7a32a28…`) |
| S3 140 | preflight OK 16:25:16Z; applied 16:25:32Z (DROP FUNCTION ×2, CREATE FUNCTION ×3); verify OK, md5 `f6296870…`; read 16:26:07Z: ledger 144 · **census 32\|109\|37\|37** · both `mark_transfer_sent` jsonb `d816c53e…`/86 and `17453329…`/2560 · `attach_transfer_evidence` jsonb `67615b89…`/2690, EXECUTE false/true/false | 16:26:36Z: **census stop condition met exactly**; bodies equal D's local replay at f412d10 (md5 `3871342a…`) |
| S4a stripe-webhook | `functions deploy … --no-verify-jwt --use-api` 16:27:02–08Z; uploaded index.ts, _shared/stripe.ts, _shared/sentry.ts; **v5**, verify_jwt false, ezbr `d33f3889…`; downloaded v5 **byte-identical** to the exec tree on all three files | 16:28:07Z list: only stripe-webhook changed |
| S4b notify-report | deploy 16:28:29–32Z; uploaded index.ts, _shared/sentry.ts; **v4**, verify_jwt false, ezbr `4a51f193…`; download byte-identical. **Limit:** `esm.sh/@supabase/supabase-js@2` floats and the resolved version is not observable from the CLI, so this is source parity only | 16:29:25Z list: exactly two functions changed since V0 |
| Close | 16:28:49Z: ledger 144 · census 32\|109\|37\|37 · notify 9 · new all true · deltas 1\|1\|0\|0 · the five transfers unchanged · business counts = V0 · Vault `project_url` only · executors false · 2xx 0 · production host refs in routines 0 · cron jobs 22, 0 with a `*.supabase.co` literal | 16:29:26Z: agrees, zero drift (md5 `bda8ae8b…`); a Line 3 baseline at 16:29:40Z: the five transfers as described, dedupe counts 0 for the three pending and 1 for each of the two sent, 7 triggers enabled, folder 0, **PC3: all seven `pg_get_functiondef` md5s equal D's local replay at f412d10** |
**W-C3 CLOSED.** Rollback (only on the owner's order): edges redeploy from 9bef640; migrations 140 → 139 → 136 from the exec tree's rollbacks, each with its ledger row removed. A rollback restores code, not data. D witnessed state and function lists, not the commands or the bundles. The 2xx figure is the `net._http_response` table, not an edge-log check.

### Step 3 — staged security notice (temporary): EXECUTED AND CLOSED (16:30:58Z → D's cleanup read 16:58:13Z)
- **Hold and restatement:** C stopped the write before it happened (nothing written). On Build 19 the owner already saw an older banner, "Account deletion requested". A's read-only answers (`notice/notice_c_questions.txt`):
  - no staged row existed;
  - the buyer's only security-set row was `f3abe550…` (`account_deletion_pending`, 2026-09-14T04:28Z, unread, undismissed, 2 pre-existing delivery rows);
  - the buyer is ACTIVE with no pending deletion (identity_ext deletion_requested_at null; 0 `public.account_deletions` rows). **F-NOTICE-1** was recorded (sprint table).
  - C corrected the controls: a rebound notice shows "Sign out of all devices" and "Dismiss", and Dismiss marks only the displayed id.
  - A corrected DV-N-1's expected text: `get_inbox` renders the highest template version, and 136's v2 has no `{{device_name}}`.
- **Before:** A 16:30:58Z and D 16:32:40Z: buyer rows 1, staged dedupe 0, notification total 9, delivery 18, queue 0; the type is mandatory with `allowed_channels []`. A's fresh pre-count 16:42:47Z was identical, and the role was postgres.
- **Write, 16:42:48Z (once):** `notify.enqueue('919d511e…', 'security_device_rebound', 'account_security', '140fcb44-4920-4c32-8333-1a551879d36b', {"device_name":"iPhone (DV staged)"}, 'dv-notice:140fcb44:2026-09-17')` → `fb23b20a-72fa-4fa7-968c-3a4191136a3d`. After (A 16:42:49Z, D 16:43:21Z): delivery rows for it 0, buyer rows 2, totals 10/18/0, 2xx 0, f3abe550 unchanged.
- **DV-N-1 PASS** (owner, 12:44 EDT, Build 19, DV buyer): title "A device stopped receiving your notifications"; body "A device that was getting notifications for this account is now registered to a different account. If that was you signing in to another account, there's nothing to do. If not, sign out of all devices."; buttons SIGN OUT OF ALL DEVICES and DISMISS. No device name, so v2 rendered. Nothing tapped.
- **DV-N-2 PASS** (Dismiss tapped once at 12:46 EDT). A's read-back at 16:47:20Z: fb23b20a `read_at` 16:46:11.195Z, `dismissed_at` null; f3abe550 unchanged; no session change (newest session is the owner's 16:32:39Z sign-in), push row 140fcb44 active and bound, push_binding_epoch unchanged; delivery 0 for the staged row.
...
- **Reading caveat (B, 2026-09-17; A agrees):** `account_deletion_pending` is registered mandatory/account_security, which is exactly the set `public.get_my_security_notices()` returns, so the buyer's stale F-NOTICE-1 row **is** returned by that RPC and the client shows the newest unread of any returned type. DV-N-3's evidence is therefore "the STAGED notice is gone", not "no banner is shown" — a pre-existing "Account deletion requested" banner may legitimately take its place. The recorded run matches that reading (the owner saw only the expected deletion notice), and future observers record the notice **by title or id, never by presence**. **Sequencing:** if F-NOTICE-1's §4 cleanup is ever authorized, that write moves the buyer's notification totals, so it must land before or after — never inside — a staged-notice window or Line 3's inbox-delta reads.
...
- **D's post-read 18:08:24.956Z** (`d_rep_post.txt`, md5 `00f88172c603eb7186491ec9285710fd`): agrees. D's own line-by-line diff of its pre-read against its post-read, ignoring the read time and the pg_net line, shows exactly two changed lines — the `reports` counters 1 → 0 and the target's keyed match 1 → 0 with `row_md5` → none. Everything else is identical, including D's extra `reports_other` line (0, md5 none), D7, the buyer, the notices and totals, the claim and queue, the cron list md5, ledger 144 and the absent `ops` schema. pg_net advanced only on its cadence (1127@18:04Z, 1128@18:06Z, 1129@18:08Z, all 401) with **no request at 18:07Z and no 2xx**, so the delete made no outbound call.
- **Evidence limit, stated by D:** D did not observe A's transaction. D's independent statement is that exactly one matching report existed before the commit and none after, with everything else it read unchanged. The guards, the NOTICE and the single-row count are A's evidence.
- **INCIDENT CLOSED 18:08:24.956Z.** Nothing outbound ever left the system; one row was created accidentally and that one row was removed.

## 15. DV-ST2b — bids read-access window on Build 19, 2026-09-17 18:36:23Z–18:38:56Z (A executes, D witnesses, C guides; sandbox `ofaidukbieeekqaboscm` only)

**Authorization.** The owner authorized the window separately to each session in that session's own conversation: to A ("A: go — run the DV-ST2b window as C described"), to D ("D: go — take the before and after witness reads for the DV-ST2b window") and to C, with the bound "up to six minutes" and "restore access immediately after the refresh". A and D each refused the relayed form first and waited for their own line; C's relay of the owner's *sequencing* (DV-ST2b before Line 3) was honoured throughout.

**Before.** A's capture 18:35:20Z (`st2_bids_acl_capture.txt`, md5 `536505054bd9a3fe2148b1e6401d315f`): relacl `{postgres=arwdDxtm/postgres,anon=arwdm/postgres,authenticated=arwdm/postgres,service_role=arwdDxtm/postgres}`; authenticated select/insert/update/delete true; anon SELECT true; owner = grantor = current_user = postgres; **per-column ACLs 0**; policies 3; RLS on. Restore statement pinned in advance: `grant select on table public.bids to authenticated;` (md5 `0836055427283986b2eb4d62373aa76f`). D's before-read 18:35:23Z (`d_st2b_before.txt`, md5 `466fd2d8db214b2a60ea4e779344252f`), byte-identical to the ST2a baseline.

**One apparent disagreement, resolved before the revoke.** D read "column ACLs 50" where A read 0. Each checked it in their own read-only transaction and they measure different things: `pg_attribute.attacl` non-null = 0 (the real per-column ACL, and A's restore precondition), while `information_schema.column_privileges` holds 70 rows, 50 excluding postgres, because that view expands TABLE-level grants across 5 columns and 4 grantees. A table-level revoke therefore does cut the read. The derived 50 drops to 45 during the window and must return to 50 after.

**Window.** REVOKED 18:36:23Z (authenticated SELECT false; insert/update/delete unchanged; anon untouched). The owner pulled to refresh Bids once at ~18:38Z — no Retry, no tab switch, no backgrounding, no force-quit. RESTORED 18:38:56Z **by A**, on C's report; the watchdog never fired and logged itself idle. VERIFY 18:39:07Z: relacl matches the capture exactly, anon SELECT still true, derived count back to 50, policies 3, RLS on.

**Observation, with the owner's own recording limit.** The purchase rows **REMAINED VISIBLE**. Whether any message or banner appeared, and whether the screen changed to an error or empty state, is **NOT EXPLICITLY CAPTURED** — the owner asked that this not be inferred as "none". So the "rows stay" half is observed and the "no message" half is not evidence either way. Build 19 does not carry F-BIDS-1.

**F-ST2B-1 (defect, recorded as a defect at D's request, not a footnote).** The script started its watchdog — and later ran its verify — by bare command name with no path, so the shell could not find it. Two occurrences, one root cause, three self-invocations of which two were unreachable.
- **Occurrence 1, 18:36:23Z–18:36:51Z (28 s):** the window ran with **no automatic restore** while appearing armed. The owner's six-minute bound is enforced by that control; for those 28 seconds it rested on A staying responsive, which is exactly what the watchdog exists not to rely on. A noticed immediately, fixed the call and armed a corrected watchdog **against the original revoke timestamp**, so the bound stayed 18:42:23Z.
- **Occurrence 2, 18:38:56Z:** VERIFY did not run automatically after the restore. A ran it explicitly at 18:39:07Z and it passed.
- **Class:** the same one this sprint keeps finding — a guard believed armed that was not. A silent arming failure looks identical to a working one.
- **Fix, stated so it cannot regress (D's wording):** invoke by absolute path, and **verify the arm** — the revoke step now checks the watchdog pid is alive and, if it is not, restores immediately rather than hold a window with no bound. "Arming that is not verified is not arming." Script md5 now `1d2be8c3221fe57a2943e6e6448fd808`, zero bare self-invocations.
- **It does not invalidate the window:** the revoke behaved normally, the bound was unchanged, the restore was A's and was verified, and nothing else in the sequence depended on the watchdog.

**D's after-read 18:39:55Z:** output md5 `466fd2d8db214b2a60ea4e779344252f` — byte-identical to D's before-read and to the ST2a baseline. relacl exactly as captured; authenticated select/insert/update/delete true; anon SELECT true; derived column count back to exactly 50 excluding postgres; RLS true; 3 policies. **D's stated evidence limit:** D did not observe the revoke or the restore. D's evidence is that the grant state before and after is identical; that it was A and not the watchdog rests on A's log.

**D's addition to F-ST2B-1, adopted:** the second occurrence is the more serious of the two. The first removed a safety net; the second removed the evidence that the restore was correct, producing a window that *looks* verified. What stands in its place is named deliberately: A's explicit verify at 18:39:07Z and D's independent after-read at 18:39:55Z. Alongside "arming that is not verified is not arming" the rule is **"a verify that did not run is not a verify"**.

**API-log check (A, read-only, sandbox project only, run 18:49:10Z — after the ~10-minute ingestion lag).** Source `edge_logs`, window 18:30:00Z–18:45:00Z, filtered to `/rest/v1/bids`. The access token is not in the sandbox env, so this ran through the Supabase MCP logs tool with the sandbox project id passed explicitly; no production project was queried.
- **Six denied reads inside the window**, all `GET /rest/v1/bids` → **403**, `proxy_status "PostgREST; error=42501"` (insufficient_privilege), authenticated as the DV buyer `919d511e`, UA `SnatchIt/19`: 18:37:12.060Z, 18:37:18.526Z, 18:37:25.576Z, 18:37:29.258Z, 18:37:35.513Z, 18:37:48.711Z. All fall between the revoke (18:36:23Z) and the restore (18:38:56Z).
- **Before the window**, the same request returned **200** at 18:32:24.174Z, 18:32:24.176Z, 18:32:27.196Z, 18:33:05.084Z and 18:34:34.097Z. No 403 outside the window; no bids read is recorded after the restore, because the owner did not refresh again.
- **DV-ST2b: PASS on the server half.** The revoke demonstrably denied the signed-in user's bids read, and the purchase rows stayed on screen throughout.

**Two things the log adds that the handset could not show.**
1. **One pull-to-refresh produced six bids requests over 36 seconds.** The owner refreshed once. This is the client's own retry/refetch behaviour under a failing read, not six user actions. Recorded as an observation for C, not a defect, and not something this window was designed to measure.
2. **The pre-window 200s carry `content_length: 2`** — an empty JSON array. The DV buyer has **no bids at all**, so the Bids tab's visible rows come entirely from merged purchases. This is the same fact that withdrew the bid-fixture premise (Line 2), now confirmed from the server side. It also sharpens what DV-ST2b proved: the list survived a **failing bids read** because purchases populate it, which is exactly the behaviour under test — but it does **not** exercise a failed read on a list whose rows come from bids, because no such rows exist on this account.

**Row status: PASS (server half) with the client half split** — "rows remained visible" observed; "no message or banner appeared" NOT EXPLICITLY CAPTURED and not inferred. F-ST2B-1 stands as recorded. **Window CLOSED 18:49:10Z.**

**DV-ST2b CLOSED BY THE OWNER (relayed through C, 2026-09-17).** The recorded result, in the owner's terms: on Build 19 the buyer's existing purchase rows stayed visible during a failed bids refresh; **banner and error-message behaviour was NOT captured and is not claimed either way**. The owner directs **no repeat and no further log check**. A's six 403s inside the window and D's matching before/after reads are the evidence; nothing further is to be added to this row.

**Line 3 readiness, re-verified read-only at 20:17:58.915Z** (A; scratchpad `line3/pc_pre_line3.log`), after the window closed and at C's request: every line identical to A's 18:30:38Z pre-check and to the 18:50:58Z post-window read — PC1–PC8 all PASS, the seven PC3 bodies unchanged, the bids ACL back to `{postgres=arwdDxtm/postgres,anon=arwdm/postgres,authenticated=arwdm/postgres,service_role=arwdDxtm/postgres}` with anon SELECT untouched, executors false, vault `project_url` only, net queue 0 and 2xx 0, the five transfers unchanged with dedupe 0/1/1/0/0, folder 0/0, buyer inbox 41. Nothing of A's is running against the sandbox: no revoke flag, no watchdog process.

**Line 3 remains PAUSED.** C is running handset SETUP only, which writes nothing: switch to the DV seller (also the F-SELL-2 sign-in screen check), prepare synthetic images with no real codes or personal information, Camera location off before DV-IMG-9. Step 0 begins only on the owner's explicit confirmation that they are back and available with the phone — the owner's away message authorizes no handset action — and each subsequent row waits on D's witness read of the previous one, which D holds until the owner returns.

### Line 3 — agreed sequence, navigation, and the two evidence limits (A and D, 2026-09-17, before any row ran)

**The owner is present** (confirmed to C at 16:20 EDT: "I'm available with the phone and ready for Line 3 under the existing approved scope"). C's setup writes nothing: signed in as the DV seller, Camera location Never, synthetic ticket images prepared, and the sign-in screen heading clears the SANDBOX badge — which closes the last outstanding F-SELL-2 screen.

**Prerequisites, both sides.** A re-verified read-only at 20:17:58.915Z and D at 20:22:33Z; the two match line for line on everything both measure, and both match their own pre-window baselines: PC1–PC8, the seven PC3 bodies, the seven triggers, the five policies, the bucket, the five transfers with dedupe 0/1/1/0/0, folder 0/0, executors false, vault `project_url` only, queue 0, 2xx 0, inbox 41, and the bids ACL back to `{postgres=arwdDxtm/postgres,anon=arwdm/postgres,authenticated=arwdm/postgres,service_role=arwdDxtm/postgres}`.

**Listings behind the tracked transfers** (A, read-only): 3118bd30 → `086dd027` "Device D1" (Club Device) · 92ee5156 → `96799125` "Device D6" (Club Device) · bce07eef → `9c6eecd4` "Device D2" (Club Device) · 8f59d37e → `92f8effe` "Sandbox S8only" (Club) · **untouched** 83b83858 → `c0dd0706` "Sandbox L7" (Club).

**Navigation finding (A, from the Build 19 source; it changes which screen DV-IMG-10 uses).** `my-listings.tsx:150-153` defines `needsTicketSend` as `listing.status = 'sold' AND transfer.status = 'pending'`, and the card-body tap routes to `/transfer/send/<transferId>` only under that condition (`:250-262`). So the three PENDING transfers are reached from My listings → "Send tickets" → card body, while **8f59d37e, already seller_sent, never appears in that tab**: the owner reaches it through My listings → Sold → "Sandbox S8only" → the listing detail screen, whose seller primary button reads **"View transfer"** for a seller_sent transfer (`detailState.ts:266-270`) and routes to the same send screen (`ListingDetailScreen.tsx:1173-1176`), where Add proof renders. The wording the owner taps for DV-IMG-10 is therefore "View transfer", not "Send tickets". "Sandbox S8only" and "Sandbox L7" share the venue label "Club", so the event name is the only on-screen discriminator.

**Two evidence limits, stated by D and adopted for every Line 3 row:**
1. **Prevention and detection are separate and neither substitutes for the other.** Reading the event name before each tap is the prevention; D's read immediately before the S8only row records BOTH that row's and 83b83858's md5, so a wrong tap shows up at once. **83b83858 changing at all is a stop, whatever the screen said.**
...
### Line 3 — the blocker, the fixture write, and the consolidated sweep (2026-09-17)

**Step 0b was BLOCKED on 3118bd30 / "Device D1"** (owner-reported 16:34 EDT, Build 19, with a screenshot): the Send tickets screen showed "Transfer window expired", "Buyer: Unknown", "Buyer must provide delivery info before you can send tickets", Mark as sent disabled with the synthetic image selected. The owner stopped and tapped nothing further. **The no-image "Evidence required" check therefore stays UNTESTED — it never ran**, because the button was already disabled for a different reason.

**A's read-only diagnosis (all four transfers):**
- `delivery_email` and `delivery_phone` are **NULL on all four, and on all 33 transfers on this project** — 0 have delivery info. **Missing test data, not a defect:** the client's gate is `!delivery_email && !delivery_phone`, and a seller genuinely cannot send tickets without a destination.
- `expires_at` is past on all four (3118bd30 2026-09-12T01:05:11Z · 92ee5156 2026-09-12T01:19:01Z · bce07eef 2026-09-11T20:58:47Z · 8f59d37e 2026-09-09T01:20:22Z), but it **gates nothing**: display-only in the client, and **neither `mark_transfer_sent` nor `attach_transfer_evidence` reads it** — their preconditions are caller-is-seller and the status transition. Extending it was proposed and **declined as pointless**.
- `transfer_method` is `mobile_transfer` on all four.
- "Buyer: Unknown" is **null data, not RLS**: `profiles.display_name` is NULL for both DV accounts, and `authenticated` holds column-level SELECT on `profiles.display_name`, so the embed is not filtered. C confirmed the two surfaces differ by source — Profile falls back to the email local part, the Send screen only has the embed — and records it as a minor consistency finding, not a defect.

**The owner's decisions:** run the fixture fix first and keep the originally approved order (so the attach-first subset was **declined as a reordering**); use the **direct fixture write** rather than entering delivery details as the buyer; and, in A's conversation, "use the approved direct delivery-details fixture update; don't repeat buyer data entry."

**FIXTURE WRITE, executed 20:44:31Z (A), one transaction:** `update public.transfers set delivery_email = <the DV buyer's sandbox email> where id in ('3118bd30…','92ee5156…','bce07eef…') and status='pending' and delivery_email is null and delivery_phone is null`, raising unless exactly 3 rows changed. **`delivery_phone` left NULL** deliberately — one column satisfies the gate and is less to undo.
- **Blast radius proved inside the same transaction** by diffing each tracked row's full `to_jsonb` before and after: changed keys `delivery_email` on the three; **`(none)` on 8f59d37e and 83b83858**. Post-write md5s: 3118bd30 `17a9dddb…`, 92ee5156 `198c9918…`, bce07eef `4116ee1a…`; 8f59d37e `c74e9fd0…` and 83b83858 `d1b36045…` **identical to pre-write**.
- No side effects: notifications 105, buyer inbox 41, queue 0, **2xx 0**, executors false, vault `project_url` only.
- **Checked before writing, because an email column could feed an outbound path:** `notify-transfer` is the only reader of these columns and is **NOT deployed** on this project (deployed: confirm-and-release, confirm-payment, create-connect-account, create-payment-intent, delete-account, enforce-transfer-expiry, notify-report, send-push, stripe-webhook), and the DB trigger cannot call anything without the Vault service key. The state-column guard does not cover the delivery fields, so **no bypass GUC was needed**.

**Consolidated blocker sweep — all four CLEAR** (asked for as one list before any tap): 92ee5156 pending/delivery set · bce07eef pending/delivery set · 3118bd30 pending/delivery set · 8f59d37e seller_sent with no path, where delivery does not matter because the Add proof section renders on `status='seller_sent' && !transfer_evidence_path` and its button is `disabled={busy || refreshing}` only. Storage INSERT policy requires the object under the seller's own uid folder; party read requires the object to be referenced by the transfer, which is what makes RT6 pass and U1/RT5-P deny. Bucket private, 10 MiB, allow-list includes both `image/heic` and `image/jpeg`. The append-only guard is inert while every path is NULL.

**Efficiency rulings applied (owner, 2026-09-17):** C reuses step 0a's evidence for DV-IMG-1 and the replace/remove half of DV-IMG-3 — same components (MediaUpload + useImageUpload) — and those are recorded **PASSED on step 0a's evidence, not re-tested**. The remaining handset work is ONE combined pass: DV-IMG-2, DV-IMG-6, DV-IMG-3a, then -4 on "Device D6", -5 on "Device D2", -9+3b on "Device D1", -10 on "Sandbox S8only". **Probe sequencing:** N3 runs before the pass (it must precede DV-IMG-10); N1, N2 and N4 are batched after it with the RT6/U1/RT5-P reads. **Deviation stated:** the approved order interleaved N1/N2 between rows; batching them keeps the owner tapping continuously, which the owner asked for, and changes nothing about what the probes prove.

### Line 3 — the combined pass as narrowed by the owner, and the boundary state it starts from

**The owner narrowed the pass** to the proof-repair changes and their direct regression risks: upload failure and retry, duplicate submission, selecting the correct replacement image, HEIC handling and buyer display, and attaching proof after marking sent. Generic navigation and permission checks are dropped unless tied to a code change or an unresolved release requirement. **The owner also ruled that their remark about the transfer flow working in earlier versions is not evidence about today's state:** "A should confirm current transfer states from the planned pre-checks; don't infer them from my comment."

**A's boundary read, 20:55:11.785Z, immediately before the pass** — the states the pass actually starts from: 92ee5156 pending, delivery set, no evidence, md5 `198c9918…` · bce07eef pending, delivery set, no evidence, md5 `4116ee1a…` · 3118bd30 pending, delivery set, no evidence, md5 `17a9dddb…` · 8f59d37e seller_sent, no evidence, md5 `c74e9fd0…` · 83b83858 untouched, md5 `d1b36045…`. Three transfers carry delivery info and **0 remain eligible for the fixture, so it cannot fire twice**; notifications 105; queue 0; 2xx 0.

**N3 PASSED, 20:50:53Z** (A, seller JWT over the API; never service_role, never psql for the verb): `mark_transfer_sent(8f59d37e, seller, <nonexistent path>)` → **HTTP 400, P0001, `precondition_failed: transfer already sent without evidence — use attach_transfer_evidence`**; row md5 unchanged before and after; notifications 105 → 105. **Disclosure:** A's first attempt used wrong parameter names (`p_seller_id`/`p_evidence_path`) and PostgREST answered 404 `PGRST202` — no function matched, so nothing ran; A read the live names (`p_transfer_id, p_user_id, p_transfer_evidence_path`) and re-ran.

**C's retained and dropped rows, recorded as C's judgement with reasons:** RETAINED into the D1 screen — the permission-denied alert with Open Settings, and selection preservation across leaving the screen, both **new code from the F-IMG-1 repair** (`useImageUpload`'s final-denial Alert with `Linking.openSettings()`; `rememberSelection`/`recallSelection`/`selectionToRecall`) and both on the owner's acceptance list for that repair. DROPPED — the limited-access row (no changed code behind it) and **DV-IMG-3a's no-image gate**, whose early return predates the repair and is unchanged: it stays **UNTESTED with that reason, not a pass**. DV-IMG-1 and the replace/remove half of DV-IMG-3 are **reused evidence from step 0a**, same components, not re-run.

**HEIC decision rule, fixed BEFORE the evidence** (A): stored magic `ftypheic`/`ftypheix`/`ftypmif1` + `.heic` + `image/heic` → a stored HEIC and the conversion half is UNTESTED; magic `ffd8ff` + `.jpg` + `image/jpeg` **with the device on High Efficiency** → conversion PROVED; `ffd8ff` **with the device on Most Compatible** → never HEIC, conversion half UNTESTED. **If the Camera › Formats setting is not captured before the row, the result is UNTESTED, not inferred** — "it was a camera photo" is not evidence of HEIC.

**Stop conditions made observable on the phone** (C, with the owner): a success message while Airplane Mode is on; two success messages for one transfer; the transfer showing as sent without the button's spinner; the no-image tap showing a spinner instead of an instant alert. Wi-Fi off as well as Airplane Mode, so the failure is a real network failure.
**A's verification limit, stated before the pass:** from the database A can prove exactly one pending→seller_sent transition per transfer, exactly one object, the path set once and never replaced, and `buyer_confirmation_needed:<id>` at 1. A **cannot** prove how many client calls were made — `mark_transfer_sent` is idempotent, so a retry writes nothing and leaves no trace, and counting calls needs an API-log read that Line 3's approved scope excludes. "No transfer took two calls" is therefore recorded as **not established**, not as passed.

**Step 5 (buyer display) — an expected write, agreed in advance:** the owner switching to the buyer and opening a transfer writes one `transfer_viewed` notification to the seller per transfer opened. It is accepted as a consequence of a step the owner put in scope, and C names which transfers were opened so each row is matched one for one. **A `transfer_viewed` for a transfer the owner did not open remains a stop.**

### Line 3 — the auto-release hazard behind the standing disposition precondition (D found it; A verified independently, 21:0xZ)

**What D found, and what A measured separately rather than accepting:**
- `enforce-transfer-expiry` is an **ACTIVE** cron job on this project, schedule **`*/2 * * * *`**, whose command posts to the edge function with `Authorization: Bearer <vault service_role_key>` (A: `uses_vault_key=true`, `posts_to_function=true`, `active=true`).
- The Vault holds **no** `service_role_key`, so every tick is unauthorized. A's count across the project's whole history: **189 responses, 188 of them 401, and ZERO 2xx**, newest at 21:06:00Z.
- **Both seller_sent rows are 6.8 days past their `auto_release_at`** (2026-09-11) with `payout_released_at` NULL: `8f59d37e` (a Line 3 target) and **`83b83858`, the row that must not change**.
- **Therefore those two transfers are unreleased only because the call is unauthorized.** The moment a `service_role_key` exists in this project's Vault, the next tick — **at most two minutes later** — would attempt release on both, including 83b83858, **with no one tapping anything**.

**Consequence, recorded as an instruction rather than a caution:** no `service_role_key` (and no push key, which the owner has already deferred) may be added to this project's Vault while Line 3 is open. This is the same lock the owner already has in place, and this is the reason it matters: not prudence, but a live two-minute timer.
**It also makes "net 2xx total 0" load-bearing rather than decorative** — it is the evidence that no tick has ever succeeded, and it is read at every Line 3 boundary.

**This sharpens, and does not replace, the standing disposition precondition in §13:** before any future sandbox activation of a `service_role_key`, `payout.executor_enabled` or `refund.executor_enabled`, or any manual invocation of `enforce-transfer-expiry` with `INTERNAL_CRON_SECRET`, the activation package re-reads these transfers with a plain SELECT using 039's predicate — never through `get_auto_release_candidates()`, which writes risk scores — and carries an owner-approved disposition for each. **Line 3's own writes add to that set:** each transfer marked sent gains `auto_release_at = now + 72 h`, so after the pass the disposition register in package §5a covers three more rows with fresh deadlines.
