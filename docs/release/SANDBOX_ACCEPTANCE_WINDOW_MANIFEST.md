...
- **DV-S2 (seller edit form with the keyboard) on Build 18 — owner's result via C, recorded 2026-09-17:** Save changes bar PASS; text preservation PASS; Event name only MOSTLY visible; **header clearance FAIL at the largest accessibility text size.** Open layout defect, kept alongside the image repair for the next candidate (C owns the fix; device re-check needed on the next build). C records the row.
