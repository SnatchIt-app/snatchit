...
> **NUMBERING NOTE (2026-09-14, Claude A, on the owner's sequencing approval).** The unwritten 086↔112/113 scanning drift fix, referred to throughout this document as migration **122**, is reassigned to migration **125**; its companion pgTAP number **190** is unchanged. Approved merge order: **121 → 123 → 124**, with this fix numbered 125 when it is written. The renumber is required, not cosmetic: `.github/workflows/migrations-guard.yml` §4 rejects any added migration that is not greater than the base branch's highest same-scheme version, so once 124 merges a PR adding 122 would fail CI. Every "122" below is left verbatim as history — the dependency diagram, the §1.6 row, §2.6, the Claude A/C responsibilities row and the remaining-effort row. The authorization phrase "AUTHORIZE PFA-18C MIGRATION 122" in §1.6 is also left verbatim; whether it is re-issued for 125 is the owner's decision. Registry of record: `docs/release/MIGRATION_NUMBER_REGISTRY.md` (branch `fix/122-transfers-profiles-fk`). Sequencing approval only — this note authorizes no migration apply.
