...
| F7 (2026-09-08T03:12Z) | `PutUserPolicy` refused `m2_verifier_policy.json`: `LimitExceeded — maximum user inline policy size 2048` (artifact = 2544 chars compacted; `UserPolicySizeQuota 2048`, managed `PolicySizeQuota 6144`). | **Packaging only:** the identical document is created as customer-managed policy `SnatchIt-KMS-Verifier-ReadOnly` and attached to `snatchit-kms-verifier`. No statement removed, broadened or split. Read-back = `get-policy-version` document diff. |
