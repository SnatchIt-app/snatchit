...
from `docs/venue-dashboard/CLIENT_ONBOARDING_READINESS_20260916.md` @ `70e19af`), **C** (§6, My Tickets end to end — embedded
verbatim from `docs/product-v2/MY_TICKETS_READINESS_20260916.md` @ `ecc58a0`), then the dependency graph (§7) and milestones (§8).
...
|---|---|---|---|
| `door-session` | mint/validate the loginless door session; relays `record_scan`, `reconcile_offline_scans`, manifest reads; the device id is `kernel.assert_door_session`'s return value, never the body (EA-6) | `verify_jwt: false`, Class B: a `venue.door_pin` presented once at `/mint`,`/refresh`, then `Authorization: DoorSession <id>.<secret>`; no `auth.uid()` on the path | **production: deployed dark** (C6 record); held dark by data: no `door_pin`, no `scan_device` rows. 107's bcrypt PIN KDF is applied (tip 120) — the "park" text is stale (D-ONB-1); 125 (manifest sync drift) **unapplied in production**, on the sandbox. Sandbox: not deployed. Local: dark |
| `door-manifest` | KMS-sign the M2 manifest, parity with M1 | `verify_jwt: true`, Class A: staff JWT forwarded; one `service_role` read of `venue.get_manifest_signing_context()`; ES256 pinned; sign-after-verify | **production: deployed dark** (C6 record, v1 from `562fda9`, 0 requests, 0 signatures; KMS D4 live). **Held dark by data and flags, not configuration** (B, 2026-09-17): no staff role, venue, event session or open episode exists; a config change would not light it. Sandbox: not deployed (A's `functions list` read 2026-09-16: nine functions). Local: dark code, `KMS_PROVIDER` unset |
| `credential-sign` | cacheable signed ticket credential (C33) | `verify_jwt: true`; KMS from env | **production: deployed dark** (C6 record, JWT posture; the header's "NOT DEPLOYED / DARK code only" describes the pre-C6 tree and is stale). Held dark by `kernel.tickets` = 0 and `native_issuance_enabled` = false. Sandbox: not deployed. Local: dark |
...
**Environment truth for the trust root and the door edges (B and D, 2026-09-17, from `PHASE2_PRODUCTION_STATE_20260912.md` rows 10–16/21 and the sandbox reads of 2026-09-16):** production — ledger 135, tip 120, 121 NOT applied; trust root LIVE (one `kernel.signing_key` row `…b0`, global/active/ES256; KMS D4; monitor cron armed, last run 2026-09-11 ok, zero invariant alerts; Sign total 3 = 1 proof + 2 denied probes, 0 runtime AssumeRole); 14 edges ACTIVE = 11 legacy + `credential-sign`, `door-manifest`, `door-session` deployed dark; `verify_jwt` true on eight of nine marketplace functions and false on `stripe-webhook`. Sandbox — ledger 141, `signing_key` 0, `kernel.tickets` 0, nine functions all `verify_jwt=false`, the three signer/door edges not deployed, Vault `project_url` only. Local — the rehearsal harness replays the chain with stubs for `net.http_post` and `vault.decrypted_secrets`, runs as superuser with default ACLs: never evidence for production. **Correction to this report's first draft:** "KMS unconfigured" and "credential-sign not deployed" were pre-C6 statements; the edges are deployed and dark, held by data and flags.
...
_Verbatim from C's `docs/product-v2/MY_TICKETS_READINESS_20260916.md` @ `ecc58a0` (`frontend/premium-experience-backlog`),
headings demoted to nest here; report-only, nothing enabled. C's headline: the list can be populated on the sandbox today with
one authorised, recorded fixture row (an owner decision, since native issuance is excluded); production cannot populate it at
all until `20260909000000` is applied, and the only legitimate row producer is the dark native issuance chain; the spec's
offline "show cached tickets" is not implemented anywhere._
...
| **M2 — My Tickets, sandbox verification** | `20260909000000` is already on the sandbox (C §6 §0); the Tickets tab shows a real ticket on a handset from **one authorised, recorded fixture row** in `kernel.tickets` with its joined catalog/venue rows (C §6 §2.1 — the owner's decision, since native issuance stays excluded), on any build carrying the shipped tab incl. Build 17; the fuller path (native issuance ON on the sandbox only, one primary checkout) is the later, separate ruling; QR/detail contract still absent | fixture-row ruling; C's gates | days after M0 with the fixture; 2–3 weeks for the issuance path |
