...
|---|---|---|---|
| NG-1 | Account `652872010073` is still on the **AWS Free plan** and the owner has not authorized a paid-plan upgrade or a paid member account | Free plan closes after 6 months / credit exhaustion (**2027-03-05T17:57:11Z** here), data erased after 90 more days; Object-Lock COMPLIANCE retention would end with account closure; KMS availability on the Free plan is not stated by AWS | **NO-GO** (owner decision: keep Free plan) — **2026-09-08 (dated note):** the owner reports completing the Paid-plan upgrade in the Console (OWNER-CONFIRMED; root was used for billing, `jose-admin` remains the engineering identity); the keep-Free-plan decision is **superseded**. NG-1 is CLEARED only when `aws freetier get-account-plan-state` returns `accountPlanType PAID` — pending: the CLI session must first be re-established with `aws login --profile snatchit-admin` (the CLI reported a password change). Evidence goes in the execution record (session 9+). **2026-09-08T01:50Z — CLEARED:** `aws freetier get-account-plan-state` returned `accountPlanType PAID · accountPlanStatus ACTIVE · remaining credits 100.0 USD` (CLAUDE-OBSERVED, corroborating the owner-returned output); identity `jose-admin`; `AccountAccessKeysPresent 0`, `AccountMFAEnabled 1`. NG-1 no longer stands; C1 authorized 2026-09-08 within the reviewed package. |
...
| NG-3 | Production ledger/tip differ from the recorded state (ledger 124, numeric tip 109) or `kernel.signing_key` is not empty | the bootstrap is a once-only act on a known substrate | verify in §A — **re-baselined 2026-09-08 (dated note):** the owner-approved ops-console release applied migrations **115–120** on 2026-09-08 (`docs/admin-console/DEPLOYMENT_RECORD_2026-09-08.md`); expected state is now **ledger 130 · numeric tip 120 · 110–114 absent (`guard_110_present=false`) · 0 keys · flags dark · census kernel 149 / venue 83 / kernel tables 31** (read 2026-09-08T00:38Z). The signing substrate 093–109 is unchanged. See `PHASE2_PFA18C_EXECUTION_READINESS_PACKET.md` §0/§7 (incl. the 115–120 → 110–114 production-order rehearsal) |
| NG-4 | Any M1/M2/M3 artifact still has a placeholder (`<RETENTION_YEARS>`, `<KMS_SIGNER_EXTERNAL_ID>`, `<PRODUCTION_KMS_KEY_ARN>` except where it is filled after CreateKey) | deny-set must be code, read back by Device 2 | check before §C1 — **2026-09-08 (dated note):** `<RETENTION_YEARS>` filled as the integer **3** (owner-approved COMPLIANCE retention); `<KMS_SIGNER_EXTERNAL_ID>` is filled only in the owner's local copy (never committed); `<PRODUCTION_KMS_KEY_ARN>` is filled after CreateKey by design |
...
> **2026-09-08 (dated note):** NG-1 cleared (PAID verified). Remaining gates before §C1 mutations: NG-2 layout confirmation (R3), region/name confirmation (R1/R2), TOTP enrolment for the ceremony trust (R4), Device 2 for M2 read-back (NG-5). C1 is owner-authorized within `PHASE2_PFA18C_EXECUTION_READINESS_PACKET.md` §5.
...
| **C2** CreateKey + binding proof | `kms create-key` (ECC_NIST_P256 / SIGN_VERIFY / single-region, key policy **v1**); Device 2 `get-public-key`; D5 fingerprints match on both devices; §5.3 binding proof (ceremony role signs Device 2's challenge; Device 2 verifies; altered message + wrong key fail); `put-key-policy` **v2** (ceremony loses Sign; runtime role gets Sign on the exact ARN only) | "AUTHORIZE PFA-18C CREATEKEY" | C1 read back green — **2026-09-09 (dated note):** execution package `PHASE2_PFA18C_C2_CREATEKEY_EXECUTION_PACKAGE.md` (post-110–114 updates U1–U12; the tag key MUST be `snatchit:purpose` — the generic `purpose` key is refused by the ceremony role; v1 travels inside `CreateKey`, so CloudTrail shows `PutKeyPolicy` ×1 for v2). Not authorized; nothing executed. |
...
| **C4** Apply migrations 110–114 (order 110,111,112,113,114) | `supabase db push --include-all` from the merged, CI-green commit; then read-backs: guard trigger present, recovery functions present, venue 87 / five-schema 296, `get_signing_keys_door` service_role-only | "AUTHORIZE PFA-18C MIGRATIONS 110-114" | C3 done; merge rule honoured (`AUTODEPLOY-VERIFIED-OFF`, `git_branch` empty). Note: 110's guard permits the already-present `…b0` row; applying 110 BEFORE C3 is also valid (the artifact satisfies rules 1–11) — either order is safe, but choose and record one — **2026-09-09 (dated note):** executed BEFORE C3 under "AUTHORIZE PFA-18C MIGRATIONS 110-114" (record `PHASE2_PFA18C_C4_EXECUTION_RECORD.md`); the guard is therefore live for the C3 insert, which the §6.1 artifact satisfies (rules 1–11). |
| **C5** Monitor arming | `signing.expected_key_fingerprint` := D5; `signing.expected_max_not_after` only if D6 chose non-NULL; then `signing.monitor_enabled := true`; run `kernel.check_signing_key_invariants()` once | "AUTHORIZE PFA-18C MONITOR ARMING" | C3 read back green — **2026-09-10 (dated note):** since migration 102 `signing.expected_key_fingerprint` is DUAL-CONTROLLED (parks for a second distinct platform_admin, aal2, via `kernel.approve_refund_request`); only `signing.monitor_enabled` is single-admin; `catalog` is not PostgREST-exposed, so the setter runs via an authenticated psql session. Package: `PHASE2_PFA18C_C5_MONITOR_ARMING_EXECUTION_PACKAGE.md`. Ceremony §9.3 "a single platform_admin can pin" is stale. |
| **C6** Runtime secrets + dark deploy of `credential-sign` / `door-manifest` / `door-session` (E2 env: `KMS_PROVIDER=aws`, `AWS_REGION`, `KMS_SIGNER_ROLE_ARN`, `KMS_SIGNER_EXTERNAL_ID`, base credentials of the runtime USER only — never the ceremony principal) | "AUTHORIZE PFA-18C DARK DEPLOY" | C4; O1 adoption decided by the owner — **2026-09-12 (dated note): DONE.** Authorized 2026-09-11 from `snatchit-c6deploy @ 562fda9`; three functions ACTIVE (JWT true/true/false), six E2 secrets by name, one runtime key; 24 h quiet window verified (0 runtime `AssumeRole`/`Sign`, 0 requests, monitor ok). Record `PHASE2_PFA18C_C6_EXECUTION_RECORD.md`. |
| **C7** M5 live proof | one throwaway atom on a non-saleable test event; one `credential-sign` call; one `door-manifest` call on a real open episode; CloudTrail: exactly one `Sign` per call by the runtime role; flags stay false | "AUTHORIZE PFA-18C M5" | C6 — **2026-09-08 (dated note, R5 correction):** this row as written is **not executable under the canonical rules**: (1) no sanctioned path creates an atom while `feature.native_issuance_enabled=false` (`kernel.issue_ticket_atoms` refuses `feature_disabled`; its only callers are `venue.finalize_primary_order` and `venue.issue_comp`); (2) any credential signed by the deployed edge with the production key **is** "a production credential is signed" = T3 (PFA doc, maturity trigger) and therefore requires Model A first. No owner exception exists. C7 stays PENDING until the M5 procedure is clarified (packet §5d); custody rows are never inserted directly, issuance is never flipped for a test, no guard is disabled. The §5.3 **challenge** signature at C2 is a KMS `Sign` over a random nonce, not a credential — permitted by the bootstrap (ratification item 3) and not T3. |
...
| C2 | `kms schedule-key-deletion` (7–30 day window) after removing all Sign grants; record the pending-deletion state — **2026-09-09:** by `jose-admin` only (ceremony/verifier explicitly denied); needs "AUTHORIZE PFA-18C C2 ROLLBACK"; package §4.10 | yes |
...
> **2026-09-08 (dated note):** superseded in part — the owner has since approved and reports completing the direct Paid-plan upgrade (pay-as-you-go; no product, subscription, support plan or credit package purchased; no recurring budget authorized). Until the API read confirms `PAID`, C1–C3 remain unauthorized. The upgrade itself authorizes none of C1–C8. Current status and the C1 package: `PHASE2_PFA18C_EXECUTION_READINESS_PACKET.md`.
