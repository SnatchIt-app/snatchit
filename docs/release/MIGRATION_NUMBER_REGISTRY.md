...
| `129` | 196 | **release integration (A)** | session-bound push bindings — owner O-3 decision (b): password change / sign-out-everywhere invalidate bindings and proofs; old sessions cannot re-register | **allocated 2026-09-15**; design `SESSION_BOUND_PUSH_BINDINGS_129_DESIGN.md` under D review; **production gate, not in the Friday candidate**; branch `fix/129-session-bound-push`; not written |
| `130` | 197 | **Claude B** | checkout supersede serialization — per-(listing, buyer) claim so two concurrent checkouts cannot both obtain a replacement secret (PR #64 residual, pre-existing in `df9e0d3`) | **allocated 2026-09-15**; contract from A, B writes; disposition (candidate or disclosed) to the owner; not written |
