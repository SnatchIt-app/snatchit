...
| `132` | 199 | **Claude B** (proposed) | fresh-mint concurrency — a (listing, buyer, mode) claim that does not depend on an existing pending row (insert the pending row before minting), closing D's OPEN MONEY DEFECT (two captured charges, auto-refunded only by a healthy Phase-0 sweep) | **allocated 2026-09-15, owner decision pending** (this candidate vs the production gate); not written |
