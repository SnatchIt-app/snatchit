...
| `126` | 193 | **release integration** | ops-console partial-refund exactness | **PART 1 written** (`048eeb1`: `ops.refund_facts`, probed against A1–A5/A7/A8); **parts 2–3 NOT written** (rewire `build_daily_summary` + `money_overview`; pgTAP 193 incl. A6); applied nowhere. **Number stays `126`** — see the note below on the unapproved `129` proposal |
...
| `128` | 195 | **release integration** | `public.register_push_token` — device-proof token rebinding (F7); `notify` stays unexposed | contract **v2**; fourth revision after D's independent cold read found **F1 (HIGH: the verb never healed the push channel)** — fixed, plus the three residuals folded in; pgTAP 195 **53/53**, 157 **294/294** ported to the public verb, 162 pins bumped; census +1 table, **+5 functions, +2 triggers** (93/35); `push_tokens` client SELECT is column-scoped; applied nowhere. **D's fold-in review pending; contract freeze target Wed 16 AM** |

> **126 vs. the `129` proposal — kept distinct (2026-09-14).** 126's *progress* is part 1 of 3, committed. Separately, A *proposed* reassigning the refund-exactness work from `126` to `129` so that `127`/`128` (security fixes) would not be held behind it. **That proposal is UNAPPROVED** and the owner has ruled the other way: 126 must be resolved before 127/128 merge, or an explicit registry/sequencing change must be proposed and approved. Until the owner says otherwise the number is `126`, the file is `126_ops_console_refund_exactness.sql`, and the merge order is 126 → 127 → 128 behind 121 → 123 → 124. Nothing claims `129`.
