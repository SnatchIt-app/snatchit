...
| `136` | 203 | **release integration (A)** | `public.get_my_security_notices()` + `public.mark_security_notices_read(uuid[])` — `public` wrappers over `notify.get_inbox` / `notify.mark_read` for the mandatory security types (device rebound now), the 129 pattern, because `notify` is not PostgREST-exposed | **ALLOCATED 2026-09-17, not written** — notification batch 1 item 2 (`NOTIFICATION_BATCH_1_PLAN.md`); awaits the owner's batch approval |
