...
| `129` | 196 | **release integration (A)** | `public.revoke_push_token(text)` — the client's sign-out revoke, reachable through `public` (the `notify` schema is not PostgREST-exposed; contract v2 §2.4 erratum) | **written 2026-09-15**, in the Friday candidate; census +1 function (94); rollback true inverse |
| `130` | 197 | **Claude B** | checkout supersede serialization — per-(listing, buyer) claim so two concurrent checkouts cannot both obtain a replacement secret (PR #64 residual, pre-existing in `df9e0d3`) | **allocated 2026-09-15**; contract from A, B writes (lock order payments → listings); in the candidate if reviewed by Thu AM, else disclosed; not written |
