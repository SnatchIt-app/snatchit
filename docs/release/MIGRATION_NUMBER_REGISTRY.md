...
| `128` | 195 | **release integration** | `public.register_push_token` — device-proof token rebinding (F7); `notify` stays unexposed | contract **v2** (`f5d57c5` → `6383b8f`); pgTAP 195 **41/41**; census +1 table (`push_token_rebind_epoch`), +4 functions, +1 trigger; applied nowhere. **Not integration-ready**: three residual findings dispositioned in the release package, clean re-review outstanding, contract not yet sent to C |
