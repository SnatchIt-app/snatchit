...
| C1-9 MFA test + deny-set proof | **PARTIAL 04:29Z** — T1 (no explicit MFA) **succeeded** (login session is MFA-authenticated per CloudTrail; recorded as such, not as a denial pass); T2 (serial+token) succeeded with the serial logged; live trust identical; simulator: every deny-set action explicitDeny, CreateKey/Sign scoped as reviewed | remaining: owner-run live probes as the role + their CloudTrail `AccessDenied` evidence |
