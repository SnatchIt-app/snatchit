...
| **Production-gate candidate = a NEW pin and a NEW build.** **Combined b2 commit on `release/production-gate-20260918 @ 0e8de77`:** 131 + 132 + 133 + 134 + **135** (contract v3, D Gate 1 PASS) + `send-push` challenge kind (B, D PASS) + client v3 (C, D PASS) + C's K-2/131 delta, seller-form fix and Premium StateView unification — every piece D-passed individually; typecheck 0, vitest 2044/2044, lint 0 errors; SQL 154/154, census 32|105|37|38, pgTAP 5180/5180; CI 35053233744 in progress; **applied nowhere**. Remaining before a pin: D's Gate 3 at this one commit; C's keyed-strings guard test. Then: pin → CI at the pin → the sandbox application package to the owner for approval (131–135 in order, Vault `project_url` ceremony before 133, then `stripe-webhook`, `create-payment-intent`, `enforce-transfer-expiry` after 20260916000000, `send-push` after 135) → the one authorized build from the pin | production readiness |
