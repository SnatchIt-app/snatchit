...
| **Production-gate candidate = a NEW pin and a NEW build.** Server side reviewed and integrated: stack `release/production-gate-20260918 @ d61970b` = 131 + 132 + 133 + 134 — **D PASSED the final stack at one commit** (CI 35046570214; harness 26/0/3 declared; every rollback exact; reverse-order rollback of all four returns the candidate catalog byte-for-byte), applied nowhere; **the stack's server-side sign-out contract has no client carrying it yet** (C's K-2/131 delta is on `frontend/*`, not on this branch). Still to add before a pin: C's K-2 + 131 client delta (`frontend/logout-scope`, `frontend/session-bound-131-r2`, reviewed by A), C's F-FE-1 seller-form fix, and, if the owner chooses b2, migration 135 + `send-push` + client v3. The one-build authorization covers only build 17 | production readiness |
