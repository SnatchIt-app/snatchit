...
| Device acceptance | **IN PROGRESS on build 17 (2026-09-16)** — Block 0 ✔ after the Path B recovery (no mutation); Block 1 rows 1–8 PASS (owner); rows 9–18 pending; **F-FE-1** seller-form keyboard/action-bar gap + heading crowding the banner → next candidate (C); leaves open by ruling: populated Tickets (CFT-801), D9c |
...
| O-3: b1 / b2 / b3 (brief §11 + D's §12 + the b2 scope §13; only b2 closes the completed-redirect path, under D's C1–C6). **Acceptance evidence (brief, end):** the owner's own handset met 128's lock-out on build 17 (Build 16 residue under a staff account); recovery took four guided steps and the previous account's credentials; a real user's only route today is support `unbind_push_token` — the case b2 removes. 131 is integrated on the stack (incl. A-131-K2) | production gate — owner's choice pending |
