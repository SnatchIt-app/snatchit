...
| **Production-gate candidate = a NEW pin and a NEW build.** Server side reviewed and integrated: stack `release/production-gate-20260918 @ d61970b` = 131 + 132 + 133 + 134, all D-passed, CI green, applied nowhere. Still to add before a pin: C's K-2 + 131 client delta (`frontend/logout-scope`, `frontend/session-bound-131-r2`, reviewed by A), C's F-FE-1 seller-form fix, and, if the owner chooses b2, migration 135 + `send-push` + client v3. The one-build authorization covers only build 17 | production readiness |
