...
| **132** — REQUIRED before production (owner 2026-09-15). B's PR #70 @ `ebbd1c0` (CI green, run 34982027295): pre-mint group record keyed (listing, buyer) with mode recorded; edge (`create-payment-intent` only) reads prior attempts across modes, refuses a cross-mode live attempt (409), refuses when any mode succeeded (400), fails closed on another buyer's processing or uncancellable intent (409, consulting Stripe first). D's F-132-1/F-132-2 addressed; **D's full battery pending at this head**; A integrates after D passes. **Behaviour change for the packet:** an entitled buyer sees "checkout is being updated" (409) while another buyer's intent on the listing is processing or cannot be cancelled, until the webhook or sweep resolves it; a buyer with a live attempt in one mode gets 409 in the other. **Residual disclosed by B (A's surface):** a DB-`processing` row whose intent fell back to `requires_payment_method` refuses until `payment_failed` lands; nothing sweeps `processing` rows (`get_unsettled_payments` has no such arm) — A to add a processing arm to `enforce-transfer-expiry` (retrieve; `requires_payment_method` → fail the row) as a separate reviewed change; D says whether it is blocking. **Root cause recorded:** `reserve_buy_now` accepts a hold on an auction the buyer is winning and the finalizer ends an auction under a live hold (pre-existing, baseline / 20260906100000); the money property no longer depends on them; whether both entitlements should coexist is an owner product question | production gate |
