...
- **Two reported items B surfaced to the owner as UNVERIFIED claims, not findings,** because the direction is safety-relevant and should not sit unexamined while attribution is open: the reported `R10` is **0**, and the reported `R7` name list does **not contain `EMAIL_ENABLED`**. If accurate, the check would have done the one thing B's document said it could — **falsify, not confirm**. It must be re-established under a clean authorization and attribution rather than laundered into the record through B's countersignature.
