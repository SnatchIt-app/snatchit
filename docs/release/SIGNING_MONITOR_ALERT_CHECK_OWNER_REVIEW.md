# Signing-monitor alert check — packaged for the owner's review (A, 2026-09-17)

**Status: FOR REVIEW ONLY. No production read is authorized by this document or by the owner's message that requested it
("No production read is authorized by this message"). Nothing below has been run against production by A or B.**

Owner's request (2026-09-17, verbatim): "Bring me B's exact prepared production read scope for review. Distinguish what
secret names and deployed function settings establish from actual alert delivery. Configuration presence alone must not be
reported as proof that an alert reaches a person. Identify what remains unknown without exposing secret values or sending an
alert."

## A's packaging notes (read these with B's document below)
1. **Source:** B's prepared check at `91c97bd` (`docs/release/SIGNING_MONITOR_ALERT_EGRESS_PREFLIGHT_CHECK.md`), reproduced
   verbatim in §B. B has been asked to confirm `91c97bd` is the current head; any later head replaces §B in full.
2. **Two classes of read, to be authorized separately and by name.** R1–R8 are catalog/metadata reads (secret NAMES,
   cron rows, config keys and versions, function list). R9 and R10 are **production data reads** (counts of admin users and
   of their active push tokens) and fall under the standing rule that even an aggregate count needs the owner's authorization
   for that specific read. An authorization that names R1–R8 does not cover R9–R10, and vice versa.
3. **What the reads can and cannot establish — A's summary of B's asymmetry.** Every read can only *falsify* delivery (show
   a blocker) and none can *confirm* it. Presence of a secret name (R1, R7) establishes that a name exists, not its value: an
   `EMAIL_ENABLED` set to `"false"` is indistinguishable from one set to `"true"` without reading the value, and values are
   never read. A deployed `notify-report` (R8) establishes an endpoint exists, not that a POST reaches it (production egress,
   the 133-dependent base URL — 133 is applied on the sandbox and NOT on production — and pg_net's ~6-hour response cache all
   sit between). Active cron runs (R2, R3) establish the monitor runs, not that its alert arm has ever executed: it has not,
   because the invariants have never been violated. **The one read that can decisively falsify is R10: if no admin holds an
   active push token, the push arm cannot reach anyone.**
4. **The unknowns after all ten reads** are listed by B in §3 of the document (email flag value, key validity, mailbox
   readership, egress reachability, base-URL dependency on 133, provider-side token liveness, Sentry transport). A adds none
   and removes none. Closing them requires either reading secret values (not proposed) or an end-to-end test alert with an
   observed receipt (not proposed, not authorized).
5. **The sentence that travels with any result** is B's §4, and A will quote it verbatim in the production preflight
   package if any subset is authorized.
6. **Where it lands if authorized:** the production preflight package (L-1 read, `project_url` production ceremony,
   `verify_jwt` read, AUTODEPLOY attestation), as a named read block executed by A with B witnessing, results recorded as
   values in the execution record, secrets by name and digest only.

## B. B's prepared check, verbatim from `91c97bd`
