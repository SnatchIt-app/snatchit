...
| Why it is expensive | `cron.job_run_details` is **245 MB, about 72% of the 339 MB database**. It holds about 600,000 rows (run IDs 1–599,283), because it is never purged. **Its only index is the run-ID primary key** (none on `start_time`). It has been **sequentially scanned 3,509 times**, about one per detector run. `pg_stat_all_tables` records **no vacuum or analyse** (corrected wording, 2026-09-19: those counters restart at a statistics reset, so this is "none since the last reset", not "never"; pg_cron updates each row 2–3 times, so autovacuum should have run unless disabled on the table — `reloptions` and reset times are unread). `work_mem` is about 2 MB, so the week-long window sort spills to temp files |
