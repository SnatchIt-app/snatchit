...
1. **Migration `supabase/migrations/20260910120000_venue_api_read_views.sql`** (+ rollback in
   `supabase/rollbacks/`): schema `venue_api` with six `security_invoker`, `security_barrier` views
   over the granted columns only — `venues`, `events`, `event_sessions`, `resale_policies`,
   `ticket_types`, `inventory_batches` (remaining only). USAGE + SELECT to `authenticated`; nothing
...
2. **pgTAP `supabase/tests/187_venue_api_read_views.sql`** (40 assertions): shape, projection,
   tenant isolation as four synthetic callers, anon refusal, no-widening. Fixtures are created inside
...
Checks: `npm run typecheck && npm run lint && npm test && npm run build` green; pgTAP 187 → 40/40.
...
Known limitation: the display role defaults to `venue_manager`, so an outsider still sees
preview-only controls (e.g. the Release-hold form) that the database would refuse; deriving the
display role from the caller's real grants needs a self-grants read (next slice).
...
New files only: the migration, its rollback, `supabase/tests/187_*.sql`. No existing migration,
