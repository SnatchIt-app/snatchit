...
|---|---|---|---|
| `venue/read-slice1-fixes` | `2665a20` | F1 session refresh persisted (proxy), F2 event bound to route venue, F3 truthful database-mode presentation (F1–F3 approved by Claude A), F4 route venue bound to route org (F1–F4 approved by Claude A) | typecheck, lint, vitest 89/89, build |
| `venue/light-theme` | `530d35d` | white-background appearance, AA tokens, token contrast test | typecheck, lint, vitest 86/86, build |
| `venue/slice1-acceptance-kit` | `492d641` | automated local/sandbox acceptance runner + hosted runbook (no app code) | kit 135 checks |
| `venue/slice1-integration` | this branch | fixes + light theme + kit, merged with `--no-ff` (Shell.tsx auto-merged) | typecheck, lint, vitest 102/102, build, kit 135/135 |
...
| integration (F4; kit C6 + C7) | 5/5 | 5/5 | 3/3 | 2/2 | 46/46 | 50/50 | 4/4 | 4/4 | 5/5 | 11/11 |
...
## Pending-venue edge (kit C7, 2026-09-14)

F4 makes entry depend on reading the venue row. A pending Venue C (outside the approved-venue policy) stays
enterable for its manager (own-staff policy) and its org owner (org plane), and stays closed to other venues' staff
and grant-less callers — no over-denial. Kit 135/135 on this branch.
