...
| 14 | empty/failed/filtered states | airplane mode ON: Home, Explore, Bids, Tickets; then OFF; filters with no match; the empty-account tab | "couldn't load" offline, "No matches" filtered, empty state only when truly empty, never during load — **CLOSED 2026-09-16, owner-reported: offline PASS (F-OFF-1 review item: Tickets wording), no-match PASS ("NO MATCHES / Try fewer filters."), genuinely empty PASS (Bids › Past "NOTHING HERE YET / Ended auctions and completed purchases show up here.")** |
...
| 17 | DV-607a | sign in again; A invalidates the session server-side while the app is backgrounded; foreground | login shows "Your session expired. Sign in to pick up where you left off." — **PASS 2026-09-16 (post-131, branch (1) single-session global invalidation: server half A + D at 02:38:30Z; client half owner-reported, exact wording seen). Branch (2) two-session case UNTESTED outside D's harness.** |
| 18 | DV-611S — **DEFERRED to the combined build (135 on the sandbox answers challenge_required v3, which Build 17's v2 client rejects); not attempted** | sign in as the DV seller on the same device | A read-back: the row's `user_id` flips to the seller, hash retained, `is_active=true` — that row state is the contract's `rebound` (same device secret, new account); the RPC path is live on this sandbox, so the legacy 23505 case does not apply; Settings › Notifications shows no banner |

Read-back protocol (A, confirmed): C sends A "row N ready" as the owner reaches each read-back row, with the DV account (buyer/seller, the Build 16 matrix accounts) and the local time of the action; A runs the read within minutes and returns the row state. Rows are selected by the DV device's row `id` recorded at the DV-611L baseline (the app never displays the token), so "advanced" and "unchanged" are measured against recorded values. Rows 1–4, 6 and 8 are read-only; DV-607a (row 17) is A deleting that device's `auth.sessions` row on the sandbox as the documented window step, before/after recorded in manifest §10; the DV-106 fixture and the DV-605 deleted-listing id come from A (existing rows, no new deletes).
