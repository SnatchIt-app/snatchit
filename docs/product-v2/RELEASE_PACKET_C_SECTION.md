...
- EAS build: profile `preview` (sandbox), build id **`53e5e98b-dbe9-405d-a7c8-159375c3fbc6`** (submitted 2026-09-15 from the tag with `--message "candidate 2026-09-18 pin aabe029"`; upload 140 MB; credentials: existing distribution certificate and ad-hoc profile, one provisioned device), build number **17** (EAS: status FINISHED, gitCommitHash `aabe029`, profile `preview`, distribution INTERNAL, appVersion 1.0.0, SDK 54.0.0, created 2026-09-15T13:58Z, completed 14:05Z, artifact retained until 2026-09-29), device: the Build 16 handset (`________` model / iOS), tester `________`
- Compiled env — evidence on record (A's decision 2026-09-15; no artifact download): (1) Block 0 — `envGuard` accepts launch against `ofaidukbieeekqaboscm` (A quotes the guard's refusal rule for a wrong ref from source); (2) A's sign-in read-back on the sandbox (`auth.sessions` row for the DV user with the build's user-agent, timestamped) proves the URL + anon key; (3) the first `create-payment-intent` call in Block 2 appears in the sandbox edge logs with the build's headers, proving the functions URL. Results → `________`. **Bundle inspection of the .ipa is the stronger check and is PENDING the owner's permission to download the artifact** (owner item in A's checkpoint); not done.
