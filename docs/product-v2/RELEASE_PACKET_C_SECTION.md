...
- Compiled env (to verify from the built bundle, not `eas.json`): sandbox ref `ofaidukbieeekqaboscm`, sandbox anon key (public class), sandbox functions URL → `________` — PENDING: either (a) inspect the .ipa's JS bundle (download of the EAS artifact needs the owner's permission; not done) or (b) on the handset at Block 0: `envGuard` refuses a wrong ref at launch, and A reads the sign-in back on the sandbox project, which proves URL + anon key; the functions URL is proven by the first edge call in Block 2 (A read-back). Record which path was used.
