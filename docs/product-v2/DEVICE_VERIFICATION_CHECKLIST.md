...
| DV-611C-2 | Cold launch on a slow network, twice (F-611C-1, D 2026-09-16: the composition seam the unit pins cannot prove) | Throttle the network (Settings › Developer › Network Link Conditioner, or a weak Wi-Fi); force-quit and reopen twice, 30 s on Home each time; open Settings › Notifications | Each launch either registers (A read-back: last_used advanced) or Settings shows a failed banner with a specific reason and a Try again — never silence; Try again registers; A read-back after each | device, A |
