...
- Process skills: investigations → `anthropic-skills:systematic-debugging`; review feedback from A, D or a reviewer → `anthropic-skills:receiving-code-review` (verify against the code before accepting; no performative agreement); completion → `anthropic-skills:verification-before-completion`.
