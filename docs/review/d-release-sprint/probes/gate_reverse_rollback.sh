#!/bin/bash
# D: production-gate stack — the release migrations roll back in REVERSE order (135 → 134 → 133 → 132 → 131)
...
for f in $(ls "$TREE"/supabase/migrations/*.sql | LC_ALL=C sort); do
  case "$(basename $f)" in 131_*|132_*|133_*|135_*|20260916000000_*) continue;; esac
  ap "$f" || { echo "APPLY FAIL $(basename $f)"; exit 1; }; n=$((n+1))
done
echo "candidate chain applied: $n files"
ident $OUT/id_candidate.txt
for v in 131 132 133 135 20260916000000; do f=$(ls "$TREE"/supabase/migrations/${v}_*.sql); ap "$f" && echo "applied $v" || { echo "APPLY FAIL $v"; exit 1; }; done
...
for v in 20260916000000 135 133 132 131; do f=$(ls "$TREE"/supabase/rollbacks/${v}_*_rollback.sql); psql -X -q -d $DB -v ON_ERROR_STOP=1 -f "$f" >/dev/null 2>>$OUT/rb.err && echo "rolled back $v" || { echo "ROLLBACK FAIL $v"; tail -3 $OUT/rb.err; exit 1; }; done
