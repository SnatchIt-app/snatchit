...
**Evidence:** `supabase/migrations/012_seller_risk.sql:1+` — `seller_risk_scores` and `seller_flags` tables exist. `src/screens/CreateListingScreen.tsx:43–252` consumes a `RiskTier` system that warns/blocks listing creation. **Phase B Anti-Fraud Design** docs (`PHASE_B_ANTI_FRAUD_DESIGN.md`, `docs/operations/PHASE_B_DETECTION_QUERIES.sql`) describe seller risk scoring.
...
**Evidence:** No admin UI in the iOS app. Manual via Stripe Dashboard + Supabase SQL Editor (docs/operations/DAY5_ADMIN_SQL_PACK.sql exists with playbook queries).
...
**Evidence:** `docs/operations/DAY5_MANUAL_REFUND_PLAYBOOK.md`, `docs/operations/DAY5_ADMIN_DISPUTE_SOP.md`, `docs/product/DAY7_DAILY_MONITORING_CHECKLIST.md`, `docs/product/DAY7_LAUNCH_RISK_REGISTER.md` exist.
