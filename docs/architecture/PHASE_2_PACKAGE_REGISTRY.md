...
**Verified for all nineteen hooks:** `083 → 086`, `085 → 087`, `085 → 088`, `085 → 090`, `086 → 088` (×2),
`087 → 088`, `087 → 090`, the nine `OR-17` deletion pairs — `077 → 079`, `077 → 080`, `077 → 082`,
`077 → 083`, `077 → 085`, `077 → 086`, `077 → 088` (×2), `077 → 090` — the `OR-21` BP-10 predicate pair
`077 → 085` (`has_outstanding_obligations`), and the Q5-release pair `077 → 085`
(`on_deletion_q5_release`, freeze red-team F-2) — nineteen pairs, every one present in all four surfaces
...
> sets enumerate the **same 66 edges** *(the stated "45" went stale across three amendments — corrected 2026-08-30, red-team C-F5; the four-surface parser is a filed CI artifact)*. Every declared dependency **strictly precedes** its dependent by
...
```json
{
  ...
  "seam_rules": {
    ...
    "hooks": [
      ...
      { "name": "kernel.on_deletion_q5_release", "signature": "(p_identity uuid) RETURNS void", "stub_in": "077", "replaced_in": "085", "neutral_result": "no-op (true pre-085: no refund/hold objects exist)", "added_by": "freeze red team F-2, 2026-08-29", "body": "the 17.4 release semantics verbatim: refund_hold overlay release on the Q5-expired request atoms + the refund_request_expired BE notice (R2 row 18); closes OPEN-5 mechanically" }
    ],
    "grants": {
