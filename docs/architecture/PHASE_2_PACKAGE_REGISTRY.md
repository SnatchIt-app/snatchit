...
>
> **No package is added, renumbered or removed; the count stays 16 (`076`–`091`).** Four
...
> once; the count stays 16.** Objects move **between** packages, which rule §6.6 requires whenever
> `SEAM-1` says so.
>
...
>
> **`R2B`-1 (`C105`, `S2-A`) — a composite type used by two routine signatures and created by nothing.**
...
>
> **`R2B`-2 (`C106`, `S2-B`) — `market.on_atom_voided` had two arities and a `CREATE OR REPLACE`.**
...
>
> **`R2B`-3 (`C103`, `V5`) — `kernel.issue_ticket_atoms` reads `kernel.signing_key` (`083`) from `081`.**
...
> rule §6.7 protects.
>
> **`R2B`-4 (`C102`, `V4` + `V6`) — `venue.append_door_manifest_delta` (`086`) is called from three
...
>
> **`R2B`-5 (`C100`, `V2`) — the primary money path forward-referenced two packages, one of them eight
...
>
> **`R2B`-6 (`C101`, `V3`) — a body writing a column a later package adds.** `venue.create_primary_checkout`
...
>
> **`R2B`-7 (`C99`, `V1` + `V7`) — the open-doors drain and its preview reach the native resale rail.**
...
>
> **`R2B`-8 (`C107`) — two pre-existing undeclared direct edges, surfaced not created.** **`087 → 088`**:
...
> all. **`078 → 085`** (applied with `C100`): `085` reads `catalog.platform_config` for the
...
>
> **`R2B`-9 (`C104`) — a twelve-relation GRANT set that no package's Grants row contained.**
...
|---|---|---|---|---|---|
| `076` | `071` | A | A — schema skeleton | `076_create_phase2_schemas_and_grants` | 4 schemas (`kernel`/`catalog`/`venue`/`market`) + GRANT boundary + shared helper functions/triggers · **`R2B` `CREATE ROLE crm_export_builder NOLOGIN` + the column-scoped `auth.users(id,email)` grant, moved from `087` (`C104`)** |
...
| `081` | `076` | E2 | E — inventory | `081_venue_inventory` | `venue.ticket_type`, `inventory_batch`, `inventory_batch_shard`, `inventory_movement`, `inventory_hold` (oversell-safe counter) · **Δ `catalog.publish_event` authored here** · **`R2B` `kernel.issue_ticket_atoms` is NO LONGER authored here — moved to `083` (`C103`)** |
| `082` | `077` | F | F — orders | `082_venue_orders` | `venue.order`, `venue.order_item` (primary-purchase container) · **Δ `kernel.org_contact_consent`, `kernel.org_contact_consent_event` (AO — `K-2`)** · **`R2B` `venue.finalize_primary_order` is NO LONGER authored here — moved to `085` (`C100`); the two `venue.order` attribution-candidate columns and their freeze guard are moved IN from `090` (`C101`), their FKs adopted in `090`** |
| `083` | `078` | G1 | G — credential infrastructure | `083_kernel_credential_infrastructure` | `kernel.signing_key` — public key + KMS handle reference only, **no private key material** · **Δ `pass_type_cert`, `wallet_pass`, `wallet_pass_device`, `wallet_pass_push_log`, `.pkpass` bucket** · **`R2B` `kernel.issue_ticket_atoms` (moved from `081`; `C103`) + the `venue.append_door_manifest_delta` SEAM-2 stub (`C102`)** |
...
| `090` | `085` | 2D | Phase 2D — promoter engine | `090_venue_promoter_engine` | `venue.promoter`, `promoter_link` (**+`status`**), `attribution` (modeled now, activated in the promoter phase) · **Δ commercial-terms columns, `promoter_code(_scope)`, `attribution_review`, the cross-settlement commission unique, `payment_native.instrument_fingerprint`** · **`R2B` ADOPT of the two `venue.order` candidate FKs (`NOT VALID` + `VALIDATE`); their columns are born in `082` (`C101`)** |
...
so the graph is a DAG and is topologically ordered by package number. **Total: 45 edges.**
...
| 8 | `083` | `078`, **+`079`**, **§`081`** |
| 9 | `084` | `079`, `081`, `083` |
| 10 | `085` | `077`, **§`078`**, **+`079`**, **§`081`**, `082`, **§`083`** |
| 11 | `086` | `079`, `080`, `081`, **+`083`** |
| 12 | `087` | `077`, `081`, `085`, **†`086`** |
| 13 | `088` | `078`, `079`, `081`, **+`085`**, **§`086`**, **§`087`** |
| 14 | `089` | `085`, `088` |
...
> at or before N** — which is why `crm_export_builder` moves to `076` (`C104`).
...
```json
{
  ...
  "seam_rules": {
    ...
    "grants": {
      "crm_export_builder": {
        "role_created_in": "076",
        "rule": "SEAM-4 -- a GRANT is authored in the package equal to max(package creating the relation, package creating the grantee role). The role has no dependencies, so placing it in 076 (the GRANT-boundary package) collapses max() to the relation's own package for all twelve and preserves the house convention that a table's grants live with the table.",
        "defect": "R2B/C104: GRANT ... TO crm_export_builder appeared in NO package Grants row anywhere, for a closed twelve-relation set spanning 077-087, while the role was scheduled in 087. Following the house convention writes the first grant into 077, which is a HARD 42704 at replay -- a GRANT resolves its grantee immediately. Consolidating all twelve into 087 instead makes 087's rollback REVOKE on five other packages' relations.",
        "grants_by_package": {
          "076": ["auth.users (id, email) -- column-scoped; precondition relation, max(precondition,076)=076"],
          "077": ["kernel.identity_ext", "kernel.identity_contact_pref", "kernel.identity_contact_pref_event", "kernel.org_customer_key"],
          "078": ["catalog.event", "catalog.event_session"],
          "079": ["kernel.tickets"],
          "082": ["kernel.org_contact_consent", "kernel.org_contact_consent_event", "venue.order", "venue.order_item"],
          "087": ["venue.export_job"]
        },
        "relation_count": 12,
        "count_note": "4 + 2 + 1 + 4 + 1 = 12, matching the closed set of RLS 16.10 exactly. auth.users is a thirteenth GRANT and is NOT one of the twelve; 16.10 flags it as the one GRANT that is not a policy and was missing entirely.",
        "policy_half": "Each of the twelve additionally carries one <schema>_<table>_sel_svc_export policy, USING (current_user = 'crm_export_builder') and nothing else (RLS 16.10 clause 2). A policy does NOT require the role to exist -- it is a bare string comparison -- and it calls no function, so SEAM-3 leaves it with its table. That is why only the GRANT half needed moving. Migration plan section 8 names that policy for four of the twelve; filed to the RLS owner as R2B-2.",
        "edges_added": 0,
        "edges_note": "Every package holding a grant reaches 076 transitively (077 -> 076 directly, the rest through it) and the corpus does not declare transitive edges -- 085 does not declare 076 either, yet needs its schemas.",
        "owner_decision": "CONTINGENT ON RLS MD-2, recorded and NOT taken. RLS 16.10 clause 5: if MD-2 resolves postgres-owned, none of this is built, and shipping the role without the policies is the one combination that is silently wrong. C104 reverts as one unit if so: one CREATE ROLE, thirteen grants, twelve policies, no other package affected."
      }
    },
    "types": [
      { "name": "kernel.settlement_line_candidate", "created_in": "087", "kind": "composite", "columns": "cause text, cause_ref uuid, amount_minor bigint, currency text, payee_kind text, payee_id uuid", "source": "PHASE_2_RPC_FUNCTION_CONTRACTS.md#20.11.1", "r2b_note": "S2-A/C105: declared as the RETURNS SETOF type of settlement_royalty_lines and inherited by settlement_commission_lines, and created by nothing anywhere -- exactly one occurrence in the repository. 087 fails 42704 without it. A named composite rather than two RETURNS TABLE lists because the shape is the return type of four routine definitions across three packages and CREATE OR REPLACE cannot change a return type. Physical definition filed to the schema-spec owner." }
    ]
  },
  "conditionals": [
    {
  ...
  "packages": [
    { "new": "076", "old": "071", "package": "A", "phase": "A", "name": "076_create_phase2_schemas_and_grants", "purpose": "schema skeleton", "scope": "4 schemas + GRANT boundary + shared helper functions/triggers + CREATE ROLE crm_export_builder NOLOGIN and GRANT SELECT (id,email) ON auth.users TO it (R2B/C104)", "depends_on": [], "rollback_posture": "REVERSIBLE" },
    ...
    { "new": "081", "old": "076", "package": "E2", "phase": "E", "name": "081_venue_inventory", "purpose": "inventory (capacity)", "scope": "ticket_type, inventory_batch, inventory_batch_shard, inventory_movement, inventory_hold + catalog.publish_event; kernel.issue_ticket_atoms MOVED OUT to 083 by R2B/C103", "depends_on": ["078", "080"], "rollback_posture": "CLEAN_WHILE_EMPTY" },
    { "new": "082", "old": "077", "package": "F", "phase": "F", "name": "082_venue_orders", "purpose": "orders", "scope": "venue.order (+attribution_candidate_code_id/_link_id, moved IN from 090 by R2B/C101, FKs adopted in 090) + venue.order_item + kernel.org_contact_consent + kernel.org_contact_consent_event (AO); venue.finalize_primary_order MOVED OUT to 085 by R2B/C100", "depends_on": ["077", "078", "081"], "depends_on_added_by_k2_repair": ["077", "078"], "depends_on_note": "077 is FORCED by kernel.org_contact_consent_event.org_id -> kernel.organization; 078 is a co-located pre-existing under-declaration (venue.order.event_session_id -> catalog.event_session). Both were already named in migration plan section 8/082 Dependencies prose while all four declared sets said [081]. Declaration-only: 077 < 078 < 081 < 082, so ordering was never wrong.", "rollback_posture": "CLEAN_WHILE_EMPTY", "delta_added": ["kernel.org_contact_consent"], "k2_added": ["kernel.org_contact_consent_event"] },
    { "new": "083", "old": "078", "package": "G1", "phase": "G", "name": "083_kernel_credential_infrastructure", "purpose": "credential infrastructure", "scope": "kernel.signing_key + pass_type_cert + wallet_pass + wallet_pass_device + wallet_pass_push_log + .pkpass bucket (public key / KMS handle refs only, no key material) + kernel.issue_ticket_atoms (R2B/C103, moved from 081) + the venue.append_door_manifest_delta SEAM-2 stub (R2B/C102)", "depends_on": ["078", "079", "081"], "r2b_added": ["kernel.issue_ticket_atoms", "venue.append_door_manifest_delta (stub)"], "r2b_note": "C103/V5: issue_ticket_atoms reads kernel.signing_key (083) per RPC 7.1 and pins signing_key_id on every minted atom, and writes venue.inventory_batch(_shard)/inventory_movement (081) and kernel.tickets/ticket_ownership_log (079) -> max(078,079,081,083)=083. This is FR-3's decided criterion applied to the other kernel engine. New declared edge 081 -> 083. C102/V4+V6: the append_door_manifest_delta stub lives here because SEAM-2 puts a stub in the package of the earliest caller, and the earliest caller is issue_ticket_atoms in this package.", "rollback_posture": "CLEAN_WHILE_EMPTY", "renamed_from": "083_kernel_signing_key", "delta_added": ["kernel.pass_type_cert", "kernel.wallet_pass", "kernel.wallet_pass_device", "kernel.wallet_pass_push_log"] },
