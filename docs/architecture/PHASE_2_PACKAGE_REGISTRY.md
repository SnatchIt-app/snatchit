...
so the graph is a DAG and is topologically ordered by package number. **Total: 40 edges.**
...
> sets enumerate the **same 40 edges**. Every declared dependency **strictly precedes** its dependent by
