...
> for the registered decision, and `PHASE_2_RATIFICATION_RECORD.md` `O11`. **Do not render surface H's refund
...
**Neither this spec nor the money spec picks a side, deliberately.** Registered as **`D-8`** in the money spec's owner-decision register (§11) with the full statement at §11.1, and as **`O11`** in `PHASE_2_RATIFICATION_RECORD.md`. **`D-4` covers row 37 (settlement) only and does not cover row 35.**
