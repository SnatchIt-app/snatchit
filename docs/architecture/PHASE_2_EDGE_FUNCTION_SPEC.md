...
| `charge.dispute.created` / `.closed` | native | `.created`: resolve PI → `public.payments` → `kernel.payment_native` (rail discriminant), then **`kernel.record_dispute_native`** (upsert + `dispute_hold` overlay on custody-held atoms + payout hold; skip-and-alert on moved custody / occupied overlay — non-fatal, the live handler's own posture); `.closed`: **`kernel.mark_dispute_state`** (forward-only; `not_found` → record at terminal status, no freeze); on `.closed`(lost) where the debited proceeds were already disbursed/retained by an identity, record the loss — **`kernel.record_identity_obligation` (§20.7.10, `F-P2-1`/`OR-21`)**. A failed record/mark returns non-2xx (Stripe retries; §4 boundary invariant) | **`kernel.record_dispute_native` / `kernel.mark_dispute_state` (RPC §20.7.13/§20.7.14, `R-40` — mirrors `freeze_transfer_for_dispute` + MIG 024)** |
