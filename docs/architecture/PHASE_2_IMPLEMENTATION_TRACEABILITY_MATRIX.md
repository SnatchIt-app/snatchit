...
| **RLS / EXEC** | `venue_settlement_sel_org` · `venue_settlement_sel_venue` and the `_line` pair. `venue.export_job` carries **zero policies by design** (`MD-2`/`O17` CLOSED — `OR-1` ruled `postgres`-owned; the former Layer-0 exception is retired and no `_sel_svc_export` policy exists) |
...
| *(vacant — the twelve `_sel_svc_export` policies were retired by owner ruling `OR-1`: `MD-2` resolved `postgres`-owned, so **none of the twelve exists** and the six zero-policy members of the former set stay at zero. The twelve names are preserved in `_governance/O17_RULING_IMPACT_MAP.md` `D-11` and in the `B-3` patch record.)* | **B3** — `OR-1` |
