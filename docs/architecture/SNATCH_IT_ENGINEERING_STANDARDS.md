...
- **Enforcement note — SUPERSEDED 2026-08-27: these rules ARE now machine-enforced.** Ruleset `main-protection` (id `21624091`) is `active` on `~DEFAULT_BRANCH` with **zero bypass actors**, and enforces: PR required, linear history, no deletion, no non-fast-forward, and five required status checks — `Immutability + ordering`, `Migrations apply cleanly (fresh DB)`, `Typecheck / Lint / Unit tests`, `Web build (Next.js)`, `Secret scan (TruffleHog)`. The earlier text here claimed rulesets were "currently NOT enabled" and that a Pro/Team plan was the blocker; both were stale. Note `strict_required_status_checks_policy` is **false**, so a PR is not forced up to date with `main` before merging — see the open recommendation in `PHASE_2_FINAL_PREIMPLEMENTATION_GATE.md`.
