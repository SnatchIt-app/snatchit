...
- **A time-bucket is `30 seconds`, so `± 2 time-buckets` is `± 60 seconds` — `SPEC CORRECTION` (`K-1`).**
...
- **Result shape — `SPEC CORRECTION` (`K-1`).** This contract and door §7.5 described **two different wire
...
- **No open episode ⇒ `{ open: false, status: 'no_open_manifest' }` with empty `entries[]`/`deltas[]`, not an
  error** — a scanner polling before doors is the normal case. **Both keys, `SPEC CORRECTION` (`K-1`):** this
...
- **Test.** `T-RPC-DOOR-17` (above) · `T-RPC-DOOR-18` (a `venue_box_office` and a door session bound to a
  ...
  **`T-RPC-DOOR-33` (structural — the `K-1` acceptance property).** Every field named in the
  ...
  **both are required** — the door spec's group asserts over the tables, this one over what the function
  returns, and `K-1` was a defect in the second with the first already correct.
