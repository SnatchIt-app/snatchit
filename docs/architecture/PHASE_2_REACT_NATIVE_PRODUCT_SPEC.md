...
| `venue.door_manifest` / `door_manifest_entry` (**M2**) | episode `open → closed`; per-atom pinned `credential_version`, `signing_key_id`, `ticket_state`, `resale_state` | Scanner manifest sync + offline verify (§7.1 step 3, §7.3) |
