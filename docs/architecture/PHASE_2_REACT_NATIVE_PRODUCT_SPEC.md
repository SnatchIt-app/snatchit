...
5. **RESOLVED (addenda A2/A3 — schema §2.3, migration 078 — package C, catalog).** Door-freeze = `catalog.event_session.door_open_at` read via the `kernel.is_transfer_frozen(ticket_atom_id)` helper — the UI consumes it as a plain owner-scoped eligibility boolean (Transfer/Sell disabled, copy "Transfers are closed while the event is underway"); RPCs recheck the same helper under lock, so the UI state can never disagree with authorization.
