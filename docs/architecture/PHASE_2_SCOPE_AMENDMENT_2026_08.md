...
| **Venue CRM / export** | **`077`** (`identity_contact_pref`, `org_customer_key` + 2 RPCs) · **`082`** (`org_contact_consent` + 3 RPCs) · **`087`** (`export_job`, `crm-exports` bucket, 8 export RPCs) · `078` (config seeds) · `090` (template bump to `audience_v2`) | `REGISTRY` §2 rows `077`, `082`, `087`, `090` | `CRM` §11.1-20 put its seeds in `087`; consolidated into `078` by `SCHEMA` §13.5-D |
...
| 7 | **Event envelope** | **none.** No demographic event is emitted; the rollup runs on its own cron. Was named in `REGISTRY` §7 as unaffected — **corrected 2026-08-29 (`B-5`): `OR-3` ruled `#31` KEEP, so the registry/plan/schema "unaffected/no-async" prose is struck** by the outbox ruling | — | `REGISTRY` §7 COND-A (`unaffected` list) | — |
...
**Why it is shaped this way:** `CRM` §1.2 (the roster-grain decision), §4 (cross-organization isolation and its four proofs), §5.3 (the three opt-out layers), §10 (the three-layer X-6 enforcement after `OR-1`, plus `_governance/X6_POSTGRES_OWNED_ASSURANCE_PLAN.md`).
...
| 2c | | `venue.export_job` · the private `crm-exports` bucket (zero client policies) | `ADDITIVE SCHEMA CHANGE` | `SCHEMA` §13.1 (`087` rows) · `CRM` §11.2, §10.1 | `087` |
...
| 4 | **RLS** | Matrices for the four new tables; **`T-RLS-CRM-01`: no platform role can call `venue.request_export`** *(the formerly scheduled `_sel_svc_export` policies were retired by `OR-1` — zero policies on the CRM deny-all set)* | `ADDITIVE SCHEMA CHANGE` | `RLS` §16.6, §16.10 · EXEC authority `RLS` §11.6 | `077`,`082`,`087` |
...
| 12 | **Tests** | `T-RPC-CRM-01..07` · `T-RLS-CRM-01` (no platform role may `request_export`) · `T-RLS-CRM-02` (venue-grain vs org-grain marketing) · the pgTAP list in `CRM` §12 · the three X-6 layers in `CRM` §10 (`OR-1` retired Layer 0) plus the `X6` assurance-plan tests, including the **non-vacuity guard** (a grep over a not-yet-existing file set passes vacuously — this repo shipped that exact failure once) | — | `RPC` §18 · `RLS` §16.11 · `CRM` §10, §12 | — |
...
| 14 | **Rollout gate** | `077`/`082` early; `087` after settlement (`PLAN` §3 seq 12). The Layer-0 privilege-wall decision (D-2 / `HG-4`) is **CLOSED — `OR-1` (2026-08-28) ruled B**: the builder is `postgres`-owned; no ownership decision remains | — | `PLAN` §3 · `CRM` §13 D-2 | — |
...
| **HG-4** | **DISCHARGED — owner ruling `OR-1` (2026-08-28) ruled B: the Layer-0 privilege wall is NOT adopted and the builder stays `postgres`-owned.** *(As written, the gate read: "The Layer-0 privilege-wall decision (CRM D-2) must be made before `087` is authored.")* | It changed **who owns** `venue.build_export_rows`; the ruling was taken before `087` was authored, so the gate's purpose is served — the gate is discharged, not dropped | `CRM` §13 D-2 (CLOSED) · `RLS` §15.7 **MD-2** (CLOSED) · `_governance/PHASE_2_RATIFICATION_RECORD.md` **OR-1** |
...
| **CRM export** | `077` seq 2 · `082` seq 7 · `087` seq 12 | **HG-4 discharged (`OR-1`)** · the X-6 **three-layer** check green **plus the `_governance/X6_POSTGRES_OWNED_ASSURANCE_PLAN.md` gates**, including the non-vacuity guard |
