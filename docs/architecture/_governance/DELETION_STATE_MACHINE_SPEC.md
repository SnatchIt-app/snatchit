...
| **BP-8** | **IN-FLIGHT RESERVATION (live rail)** | any `public.listings` row with `reserved_by = :id` (a live buy-now reservation racing a webhook) | payment lands (`mark_listing_sold`) or the reservation is released/expires | INV A#4 :78 + §A.3 :106–111 (*"refusing deletion while a live reservation is held … belongs to 16b"*) · BRIEF-B B1 :44–45 |
...
| **BP-10** | **NEGATIVE SETTLEMENT OBLIGATION** | **Ruled BLOCK; the physical operand is NOT pinned by the corpus.** Phase 2 has *"No reserve. No clawback. No instant payout"* (`PHASE_2_MONEY_AUTHORITY_SPEC.md:1488`), `kernel.payout` CHECKs `amount_minor > 0` (SCHEMA :868), and no identity-scoped balance object exists in any package — so *"an identity owing money"* has no storable representation today. Candidate operands (enumerate, do not choose): (a) a lost dispute (`public.disputes.status='lost'` / `transfers.status='reversed'`, MIG 024 :57–70) whose recovery from `:id` has not completed; (b) an org-rail `venue.settlement` with `net_minor < 0` (waterfall CHECK permits it, SCHEMA :2950–2957) reachable to a natural person only through BP-11's org; (c) an additive per-identity obligation record. **The operand is OPEN-ENG (§6 OPEN-1)** — the predicate exists in the closed list with a defined truth-value of *false until an operand exists*, so it cannot silently block | 16c Q4 (RATREC :584: *"BLOCK on negative settlement obligation"*; PACKET :26) |
...
1. **Predicate** (blocks tombstone entry): INV #13→BP-1 · #19→BP-2 · #20→BP-5/BP-6 · #27→BP-12 ·
   ...
   transfers→BP-7. Q4's ruled predicate BP-10 awaits its operand (OPEN-1).
...
4. **Notification**: the user is told the request is pending **and why** (the current
   ...
   dependent on ODR-3's reduced Gate-P build / `OR-5` (BRIEF-B B4 :72–74).
...
The user is told the deletion completed (BRIEF-B B4 :72–73; same OR-5/Gate-P dependency as §3.1.4). Copy
...
|---|---|---|
| **OPEN-1** | **BP-10's physical operand** — Q4 is ruled BLOCK, and no storable "identity owes money" object exists in Phase 2 (no reserve/clawback, MONEY :1488; `payout.amount_minor > 0`, SCHEMA :868). Candidates §2 BP-10. Engineering + possibly a scope call | 16c Q4 |
