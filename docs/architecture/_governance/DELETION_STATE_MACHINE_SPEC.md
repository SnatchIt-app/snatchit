...
| **BP-3** | **OPEN MARKET SALE** | any `market.market_sale` row with (`buyer_id = :id` or `seller_id = :id`) **and** `terminal_state = 'pending'` **and `sale_state <> 'cancelled'`** *(reconciled with `OR-22` 2026-08-29, red-team F-4: a released/never-paid buy-now checkout is no obligation — zero money, zero custody; its clearing event is the cancel itself, this table's own "…or cancelled" exit)* (enum `pending·completed·compensated`, SCHEMA :3479; compensate-XOR-complete C26, SCHEMA :651–654) | sale completes (custody transfer appends, `terminal_state := 'completed'`) or compensates (`market.sweep_paid_pending_sales` C25, RPC :1858; `terminal_state := 'compensated'`) | INV #41/#42 :206–207 (BLOCK) · BRIEF-B B1 :42, B2 :55 |
...
| **BP-7** | **OPEN OR DISPUTED TRANSFER (live rail)** | any `public.transfers` row `t` with (`t.seller_id = :id` or `t.buyer_id = :id`) **and** ( `t.status ∈ {pending, seller_sent, disputed}` **or** (`t.status = 'expired'` and an open `public.disputes` row exists with `transfer_id = t.id` and `status ∉ {won, lost, warning_closed, charge_refunded}` — the live open-dispute set, MIG `024_disputes.sql:34–35`) ). Status enum: MIG `002_transfers.sql:55–62` extended by `024_disputes.sql:59–70` **NATIVE TWIN (`R-40`, 2026-08-30):** any `kernel.dispute_native` row in the open set (`status NOT IN ('won','lost','warning_closed','charge_refunded')` — the same MIG-024 predicate) whose `payment_id` resolves `public.payments.buyer_id = :id`, or (resale arm, via `kernel.payment_native.sale_id`) whose `market.market_sale.seller_id = :id` — org-side sellers excluded (org matter, BP-11/C31). Evaluated in `kernel.deletion_blockers_market`'s `088` body; clearing event: the dispute reaches the terminal set | buyer confirms / auto-release / clean expiry / reversal (terminal statuses `buyer_confirmed·auto_released·expired·reversed`); dispute closes into the terminal Stripe set — **native twin: the §1.10b terminal set** | 16c Q1 (RATREC :584: *"BLOCK on open/expired-in-dispute transfers"*) + 16d rider *"expired/disputed transfers join the completion blockers"* (RATREC :584). Supersedes the deliberate live gap — `disputed`/`expired` are NOT in PR #28's block list (PR28 :181; BRIEF-B B7 :112; BRIEF-A :189) — and absorbs PR #28's `pending`/`seller_sent` request-time 409 (PR28 :177–192) as a pending-state predicate |
| **BP-8** | **IN-FLIGHT RESERVATION (live rail)** | any `public.listings` row with `reserved_by = :id` (a live buy-now reservation racing a webhook); NATIVE TWIN (R-37/`OR-22`, conjunct corrected by red-team F-2a 2026-08-29): any `market.market_sale` row with `buyer_id = :id AND sale_state IN ('initiated','paid_pending_transfer') AND terminal_state = 'pending'` — in-flight ONLY; a completed or compensated sale is terminal matter for BP-1/BP-3, never this twin | payment lands (`mark_listing_sold`) or the reservation is released/expires | INV A#4 :78 + §A.3 :106–111 (*"refusing deletion while a live reservation is held … belongs to 16b"*) · BRIEF-B B1 :44–45 |
...
2. **Q5 — auto-expire pending approvals naming the deleter**: every `kernel.approval_request` row with
   ...
   individual. Expiring a refund request is a user-visible reversion of a `refund_hold`; **its release side-effects ride
   `kernel.on_deletion_q5_release` (RPC §20.17.5 — SEAM-2 stub `077`, body `085` = the §17.4 release
   semantics verbatim: `refund_hold` overlay release + the `refund_request_expired` BE notice, R2 row 18;
   freeze red-team F-2, 2026-08-29). OPEN-5 is thereby CLOSED mechanically: the §17.4 body already emits,
   so entry-time auto-expiry notifies exactly as sweep-time expiry does — no new type, no silence.**
3. **Pending org invites**: invites **to** the identity (`org_invite.invitee_identity_id = :id`,
   ...
   (enum `pending·accepted·expired·revoked`, SCHEMA :408 — `declined` STRUCK by `OR-18`, 2026-08-29). Invites **sent by** the identity
...
|---|---|---|---|
| F-1 | Primary checkout | `venue.reserve_primary_inventory` (RPC :885) · `venue.create_primary_checkout` (RPC :938) · `venue.finalize_primary_order` (RPC :990) | creates a `venue.order` with `buyer_id = :id` (new BP-12 matter) terminating in `kernel.issue_ticket_atoms` custody (new BP-1 matter). **Refusal semantics (this section's own ruled principle, restated 2026-08-29 — red-team F-3): the refusal tests THE CALLER. The two entry verbs are caller-the-buyer and refuse; `finalize_primary_order`'s caller is `service_role` (the paid-order webhook path), so the finalize limb enumerates the surface but never fires against a mid-flight PAID order — the order completes (16b: money processing preserved; nothing silently discarded), custody lands, and BP-12/BP-1 block the tombstone until disposal. Reading the finalize limb as buyer-tested would strand captured money in a `pending` order no contracted function can terminate — the exact state 16b forbids.** |
...
- **Never-sold listings/offers hard-deleted; sold/completed RETAINED** (16d, RATREC :584; INV #39/#40
  ...
  non-accepted offers (`status ∈ {pending, declined, expired, withdrawn}`, SCHEMA :3421 — **`pending` added 2026-08-29, red-team F-9: 16d's ruled language is "non-accepted", which includes an unexpired pending offer; without it a tombstoned buyer's pending offer would survive and stay acceptable, since ERASED is outside every freeze operand**).
...
**PR #28's physical-delete flow is the live-path DESIGN of record until this architecture ships — the PR itself is OPEN/UNMERGED (RPC §20.15 is the authority on its merge state); pre-merge, production runs the pre-PR delete path.** Its shape:
...
| ~~OPEN-5~~ | **CLOSED 2026-08-29 (mechanical, freeze red-team F-2):** the Q5 release rides `kernel.on_deletion_q5_release`, whose `085` body is the §17.4 release semantics verbatim — including the `refund_request_expired` BE emission (R2 row 18). Entry-time expiry notifies exactly as sweep-time expiry does | 16c Q5 |
