...
- **E-121** — OWNER-DECISION DEFAULTS IMPLEMENTED (not ruled): the register (`PHASE_2_OWNER_DECISION_REGISTER.md`) files ODR-29 (code beats link), ODR-30 (basis = face subtotal), ODR-31 (no redemption cap; opt-in expiry), ODR-32 (the org absorbs a post-close chargeback; no clawback object), ODR-90 (no commission on a resale), ODR-91 (no on-ledger override) as OPEN — OWNER with "silence → SAFE" defaults, and every frozen normative surface (PROMO §2.3/§6.1/§1.1/§5.3/§5.6/§3.4, schema §3.17.1, RPC §17.14/§20.7.2, RLS §11.5) states exactly those defaults. 090 implements the corpus defaults; NOTHING is applied to production; each is reversible pre-go-live by a body-only replacement of the resolver / the seam (SEAM-2 discipline) — precedence and basis live in `venue.resolve_order_attribution`'s body, the reversal posture in the seam's. **OWNER COUNTERSIGNATURE REQUESTED** (packet in the 090 closing report). ODR-113 (enumeration thresholds) is edge-side (§9.4 item 3) and no key is seeded.
...
- **E-126** — eligibility rule E1′: a promoter whose identity is `DELETION_PENDING` or `ERASED` (`kernel.identity_ext.deletion_state`) is ineligible in the resolver, the preview, the binding and `is_promoter_for_event` (E-23 class — 088 refuses erased/deleting counterparties; OR-17 F-7 refuses NEW enrollment of a deleting identity). Fail-to-safe (X-12): the sale completes and the org keeps the commission rather than accruing an obligation to a tombstone. The alternative (accrue-and-HOLD per 16c Q6) is recorded for the owner. The read takes no lock (§17.14: the resolver locks nothing of its own) — the §3.5 benign race extends to the deletion state.
...
- **`COMMISSION_FUNDING_SOURCE`** (E-138 — **OWNER DECISION REQUIRED**) — how a promoter commission is funded and the org's debit collected: (a) carry the negative settlement net forward and net it against the org's later positive settlements (087 carries nothing today), (b) a primary-revenue settlement line source that makes gross non-zero, (c) collect from the org's Stripe balance / a receivable (Gate-M C30/C31), or (d) pay promoters from the platform's balance and absorb. Until ruled, every commission payout is minted HELD `unfunded_settlement`; release is `kernel.release_payout` per payout.
