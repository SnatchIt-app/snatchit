...
- **E-113** — plan §8/089's Grants row ("`GRANT SELECT` on the view to `anon`/`authenticated`") and RLS §10.6's anon `A(discovery)` cell are superseded by the later owner-signed rulings PFA-14 and E-106 (countersigned 2026-09-02: no anon direct access to the market schema, no anon SELECT added "merely to support future discovery", no anon USAGE widening). `market.listing_unified` is granted SELECT to `authenticated` and `service_role` only; anon holds no USAGE on `market` (076) and reaches nothing. Mechanical: a later signed ruling over the same cell.
- **E-114** — the external rail's `rail` discriminator: `external_verified` ⇔ `public.listings.proof_status = 'approved'`, else `external`. Sources: CDM §1 C10 (verified-external is the middle state), 071's `listings_proof_status_check` (`pending_review | approved | rejected` — `approved` is the only verified-positive label). No other column carries verification.
- **E-115** — price units across the two rails: `public.listings.current_bid` / `starting_bid` / `buy_now_price` are WHOLE DOLLARS (the live feed renders `allInFromDollars(listing.current_bid)` → `dollarsToCents`; the Phase-0 CHECKs cap at 25000); `market.listing_native.price_minor` is integer minor units. The bridge projects `price_minor` for both rails, normalizing the external price as `current_bid × 100` in integer arithmetic — a unit conversion, not a pricing rule; `current_bid` is the amount the existing discovery surface already displays for a live external listing (home/explore feeds).
- **E-116** — the native arm of the bridge carries the discovery predicate explicitly — `status IN ('active','reserved')` (the 088 public-arm set, R-37) AND `feature.native_resale_enabled` (X-12: NULL ⇒ false) — rather than relying on base-table RLS alone: `market.listing_native`'s OWNER arm shows a seller their own draft/cancelled rows, which RLS §14.1 ("a native listing appears only when the public-read predicate admits it") and plan §8/089 ("native rows are filtered by the flag") exclude from the bridge. The event title resolves through `catalog.event` under the caller's own catalog policy via LEFT JOIN — a draft event's title is NULL for a fan, never leaked, and no listing row is lost.
- **E-117** — suite 149's A14 (085) asserted the ABSENCE of a `sale_id` FK "— that is 089's adopt step"; 089 adopts it (`fk_payment_native_sale`, RESTRICT, validated), so the negative flips to the positive in the same package that lands the object (the 088/148 pattern for late-binding FKs). 085 §1.8 deferred exactly this FK to the package that creates the target table.
