...
- **`NATIVE_DISCOVERY_PUBLIC_PROJECTION`** (089; E-106 / E-113) — plan §8/089 and RLS §10.6 contemplate anonymous discovery of the unified feed; the owner countersigned that no anon market access exists and that "any future unauthenticated marketplace read surface must be explicitly designed and ratified as a public projection/API". Until that surface is ratified, unauthenticated users discover native inventory nowhere (external inventory stays on its Phase-0 public path); plan §8/089's "checkout routes by rail" has no consumer yet (no client reads `market.listing_unified`). Activation consequence: none today (dark rail); a product decision before native resale goes live. Owner: UNASSIGNED.
...
## ERRATA recorded by package 089 (no amendment needed; frozen bytes / later signed rulings win as stated)

- **E-113** — plan §8/089's Grants row ("`GRANT SELECT` on the view to `anon`/`authenticated`") and RLS §10.6's anon `A(discovery)` cell are superseded by the later owner-signed rulings PFA-14 and E-106 (countersigned 2026-09-02: no anon direct access to the market schema, no anon SELECT added "merely to support future discovery", no anon USAGE widening). `market.listing_unified` is granted SELECT to `authenticated` only; anon holds no USAGE on `market` (076) and reaches nothing. Mechanical: a later signed ruling over the same cell. The unauthenticated discovery surface E-106 requires ("explicitly designed and ratified as a public projection/API") is recorded as `NATIVE_DISCOVERY_PUBLIC_PROJECTION` below.
- **E-114** — the external rail's `rail` discriminator: `external_verified` ⇔ `public.listings.proof_status = 'approved'`, else `external`. Sources: CDM §1 C10 (verified-external is the middle state), migration 033's `listings_proof_status_check` (`pending_review | approved | rejected` — `approved` is the only verified-positive label; 071 references it, 033 authored it). `proof_status` is NOT NULL, so the CASE is total; `guard_proof_status` refuses a client-written `approved`, so the rail cannot be self-asserted.
- **E-115** — price units across the two rails: `public.listings.current_bid` / `starting_bid` / `buy_now_price` are WHOLE DOLLARS (`integer` columns; the live feed renders `allInFromDollars(listing.current_bid)` → `dollarsToCents`; `starting_bid`/`buy_now_price`/`bids.amount` are CHECKed ≤ 25000 while `current_bid` itself carries NO CHECK — only its write paths bound it); `market.listing_native.price_minor` is integer minor units. The bridge projects `price_minor` as BIGINT on both rails, normalizing the external price as `current_bid × 100` in bigint arithmetic with no narrowing cast (a poisoned `current_bid` can never fail every price read across the bridge) — a unit conversion, not a pricing rule. It is the pre-fee BASE amount the feed derives its all-in label from; the bridge never computes fees.
- **E-116** — the native arm of the bridge carries the discovery predicate explicitly — `status IN ('active','reserved')` (the 088 public-arm set, R-37; this supersedes RLS §14.1's literal `status='active'` — the 088 bytes win) AND `feature.native_resale_enabled` (X-12: NULL ⇒ false) — rather than relying on base-table RLS alone: `market.listing_native`'s OWNER arm shows a seller their own draft/cancelled rows, which RLS §14.1 ("a native listing appears only when the public-read predicate admits it") and plan §8/089 ("native rows are filtered by the flag") exclude from the bridge. The external arm carries NO status filter by plan §8/089's parity rule (byte-identical to `public.listings`, whose policy admits every row — sold rows included; a consumer excludes them exactly as it does today). The event title resolves through `catalog.event` under the caller's own catalog policy via LEFT JOIN — a draft event's title is NULL for a fan and no listing row is lost; the row's `event_session_id` is disclosed exactly as 088's public arm already discloses it (inherited, not widened). The flag read under the caller's `platform_config` policy is 088's predicate verbatim (a `restricted`-visibility later version would be invisible to fans; `catalog.set_platform_config` carries visibility forward — inherited, not widened).
- **E-118** — RLS §10.6's `service_role A(machine)` cell for the bridge is undeliverable at the market-schema layer: `service_role` holds no USAGE on `market` or `catalog` (076), and a `security_invoker` view is unreadable without it. No dormant grant is written (the class E-106 warns against); machine reads of discovery, if ever contracted, are a definer path, not this view. Authority impact: none (BYPASSRLS never applied — the view's own predicate governs).
- **E-119** — the bridge projects three columns the frozen discovery set (schema §4.6 / RLS §14.1: id, rail, event/session, price, seller display, status, cover) names implicitly or not at all: `currency` (the unit of `price_minor` — inseparable from a money-valued projection across two rails), `created_at` (the feed's ordering key; PROJ, owns no truth) and `seller_id` as the delivery of "seller display" (already public on both base tables; the display resolves through the 068 profile columns). None is money/custody/PII; none is a cross-rail join.
- **E-117** — suite 149's A14 (085) asserted the ABSENCE of a `sale_id` FK "— that is 089's adopt step"; 089 adopts it (`fk_payment_native_sale`, RESTRICT, validated), so the negative flips to the positive in the same package that lands the object (the 088/148 pattern for late-binding FKs). 085 §1.8 deferred exactly this FK to the package that creates the target table. Two consequences the frozen text does not state: (a) LOCK ORDER — the RI check on an INSERT of `payment_native.sale_id` takes FOR KEY SHARE on the `market_sale` row, so every writer of that column must hold the sale (rank 4) before the insert (088's engine does: sale FOR UPDATE → atom → link); a writer holding Atom(5)/Payment(6) without the sale would create a 5/6→4 back-edge; (b) ROLLBACK ORDER — 088's rollback (`DROP TABLE market.market_sale`) is blocked by this FK while 089 is applied: 089 rolls back BEFORE 088 (stated in 089's rollback header; proven locally: 089 rollback → 088 rollback → re-apply 088+089). Plan §8/089's Lock row ("CREATE VIEW (metadata) + ADD CONSTRAINT") is delivered without a DROP … IF EXISTS so no ACCESS EXCLUSIVE is taken on re-apply (the 084 existence-guard pattern).

*(register maintained per PHASE_2_ARCHITECTURE_FREEZE.md §4)*

## ERRATA recorded by package 090 (no amendment needed; frozen bytes / later signed rulings win as stated)

- **E-120** — the 090 closed world is derived from plan §8/090's Tables/Functions rows (`PACKAGE_OBJECT_PARITY_SPEC.md` is a DEMO seed that defers to plan §8 by its own header): six tables; the seventeen contracted RPCs/reads (`preview_promoter_code`, `get_my_promoter_summary`, `list_my_attributions`, `list_promoter_attributions`, `check_promoter_slug_available` included) + `normalize_promoter_code` + `kernel.is_promoter_for_event` + `kernel.pay_promoter_commission`; the THREE SEAM-2 body-only replacements (`venue.resolve_order_attribution` 085 stub, `kernel.settlement_commission_lines` 087 stub, `kernel.on_identity_erased_promoter` 077 stub — signatures/parameter names/return types/overload count frozen, ACLs preserved, restored verbatim by the rollback); the §3.14.1 partial unique; the two ADOPTED `venue.order` FKs. Two package-local trigger functions (`venue.guard_promoter_engine_immutable`, `venue.assert_promoter_engine_consistency`) follow the 082/086 precedent (trigger fns are outside the parity universe). No cron row, no config key (PFA-9), no edge bytes.
- **E-121** — OWNER-DECISION DEFAULTS IMPLEMENTED (not ruled): the register (`PHASE_2_OWNER_DECISION_REGISTER.md`) files ODR-29 (code beats link), ODR-30 (basis = face subtotal), ODR-31 (no redemption cap; opt-in expiry), ODR-32 (the org absorbs a post-close chargeback; no clawback object), ODR-90 (no commission on a resale), ODR-91 (no on-ledger override) as OPEN — OWNER with "silence → SAFE" defaults, and every frozen normative surface (PROMO §2.3/§6.1/§1.1/§5.3/§5.6/§3.4, schema §3.17.1, RPC §17.14/§20.7.2, RLS §11.5) states exactly those defaults. 090 implements the corpus defaults; NOTHING is applied to production; each is reversible pre-go-live by a body-only replacement of the resolver / the seam (SEAM-2 discipline) — precedence and basis live in `venue.resolve_order_attribution`'s body, the reversal posture in the seam's. ODR-113 (enumeration thresholds) is edge-side (§9.4 item 3) and no key is seeded. **OWNER COUNTERSIGNATURE (2026-09-02): APPROVED — CLOSED.** ODR-29: when a valid explicit CODE and a link-derived candidate are both present THE EXPLICIT CODE WINS (a stale/cached link never overrides a code the purchaser entered; all frozen validation and org/event scope rules preserved). ODR-30: commission basis is FACE SUBTOTAL as implemented — no buyer fees, taxes, unrelated platform fees, resale markup or other amounts absent a later explicit owner amendment. ODR-31: NO general redemption cap (no max redemptions, per-user, daily or global cap) — a valid active code is usable repeatedly within its normal event/org scope; a future cap system is a separate governed change. ODR-32: the ORG economically absorbs valid post-close chargebacks under the frozen next-settlement / compensating accounting model — a historical closed settlement is never rewritten, the platform never permanently absorbs merely because the settlement is closed, and where later-settlement/carry mechanics are incomplete the obligation stays explicitly represented and fails/holds safely rather than silently transferring the loss to the platform. ODR-90: promoter commission does NOT apply to native resale — a primary-ticket-sale feature; historical primary attribution stays historical and a resale creates no new commission. ODR-91: NO ordinary on-ledger override of a ledger-relevant attribution by any normal venue/promoter/user action; any future exceptional administrative correction requires explicit authority, audit, reason, economic compensation semantics and owner ratification — not invented in Phase 2.
- **E-122** — `venue.promoter_link.event_id uuid NOT NULL FK→catalog.event`: schema §3.17's column list omits it, while RPC §20.9.3 takes `p_event_id` and RPC §1.1c reads `l.event_id`; a link is per event under both contracts (the only shape that makes the parameter meaningful). Immutable under PL-1 with `promoter_id`/`slug`.
- **E-123** — the slug "format CHECK" §20.9.3 cites is stated nowhere; 090 pins a conservative URL-slug form `^[a-z0-9](?:[a-z0-9-]{1,62}[a-z0-9])$` (lower-case alnum + inner hyphens, 3–64; inputs are lower-cased/trimmed). Widening is additive.
- **E-124** — the promoter-manager labels (`venue_promoter_manager`, `org_promoter_manager`) hold EXEC on the promoter/code/link RPCs (RLS §11.5) but appear in no SELECT cell of RLS §9.17 / PROMO §8.1; RLS :712 rules the five new labels `D` unless a matrix names them — so they hold NO direct SELECT on `venue.promoter` / `promoter_link` / `promoter_code` / `promoter_code_scope` (deny-by-default), while `venue.list_promoter_attributions` (RLS §11.5) admits them. Recorded as `PROMOTER_MANAGER_CODE_READ` below.
- **E-125** — in-body rate limiting: `venue.preview_promoter_code` (authenticated direct callers; 10/min under the spec's own `promoter-code-preview` action label — the edge wrapper's service_role/anonymous path is not double-counted because `auth.uid()` is NULL there) and `venue.check_promoter_slug_available` (30/min, action label `promoter-slug-check` — named here; §20.9.5 requires "rate-limited per principal, fail-closed" and names no label). The frozen claim "the limiter is unreachable from authenticated" holds for direct PostgREST calls; a postgres-owned definer reaches it as owner, which is what makes the fail-closed in-body limit possible. Both are fail-closed (a limiter error denies).
- **E-126** — eligibility rule E1′: a promoter whose identity is `DELETION_PENDING` or `ERASED` (`kernel.identity_ext.deletion_state`) is ineligible in the resolver, the preview, the binding and `is_promoter_for_event` (E-23 class — 088 refuses erased/deleting counterparties; OR-17 F-7 refuses NEW enrollment of a deleting identity). Fail-to-safe (X-12): the sale completes and the org keeps the commission rather than accruing an obligation to a tombstone. The read takes no lock (§17.14: the resolver locks nothing of its own) — the §3.5 benign race extends to the deletion state. **OWNER COUNTERSIGNATURE (2026-09-02): APPROVED — CLOSED.** WITH CLARIFICATION: DELETION_PENDING or ERASED/TOMBSTONED promoter identities are INELIGIBLE for new attribution, new referral economic activity and new commission accrual from post-deletion activity — do NOT accrue-and-hold brand-new post-erasure commissions; but ALREADY-EARNED commissions are NEVER FORFEITED: any commission validly accrued before the identity became ineligible remains a financial obligation on its normal held/payout lifecycle, and deletion must not erase accrued commission, an existing payout obligation, or required financial/audit history. The dividing line is the canonical economic event/accrual timing of the frozen contract (the attribution's `order_paid_at` freeze). 090 conforms: E1′ gates only NEW attribution; accrued rows, lines and payouts are untouched by the hook (INV #35–#38) and BP-5/BP-6 block erasure while a payout is outstanding.
- **E-127** — `kernel.is_promoter_for_event` carries the code arm with E4–E7 (an UNSCOPED org-wide code is eligible for every org event); RPC §1.1c's sketch joins scope rows only, which would make a code-only org-wide promoter "not a promoter" — the defect class §1.1c(b) itself names. The link arm is verbatim.
- **E-128** — hold reasons beyond the spec's flag hold, all "NO line, slot stays open" (never a zero line): `unreviewed_flag` · `denied` · `basis_zero` (a refunded order / every atom voided) · `payee_unresolvable` (an affiliate with no identity — `kernel.payout` requires an identity payee; the org keeps the money, no silent sink) · `currency_mismatch` (promoter/attribution currency ≠ settlement currency — 087's close would otherwise abort the whole settlement) · `out_of_scope` / `already_lined` (defensive). `held[]` in the seam's audit row explains the arithmetic. `AFFILIATE_PAYOUT_DESTINATION` below.
- **E-129** — `kernel.pay_promoter_commission` is EXEC: DEF with NO client or machine grant (087's treatment of the two seams; §20.7.2's "GRANT EXECUTE TO service_role only" wording would be a dormant grant of the E-106/E-118 class) and asserts its call stack (`kernel.settlement_commission_lines` ← `kernel.close_settlement`) — "called from close_settlement and from nowhere else" is structural, not a comment. Its write set is `kernel.payout` + `kernel.admin_audit` (+ a BE emit); the `promoter_commission` LINES are inserted by 087's immutable `close_settlement` from the candidates the seam returns (087 bytes win over §20.7.2's "writes settlement_line"), under `UNIQUE(settlement_id,cause,cause_ref)` + the 090 partial unique. The seam's advisory lock (E-104) is held to commit, so a sibling close's VOLATILE re-read sees the committed line. `lines_written` in its result counts the candidates it authorized.
- **E-130** — commission lines are NEGATIVE (`−payable`): E-73's sign convention books an org debit into the fees bucket; the promoter's `kernel.payout` carries `+payable`. Money conservation (every line's debit = its payout's credit) is asserted (155 G15). Payable is recomputed at close from SURVIVING (non-voided) atoms per order item — `credited_amount_minor` on the row stays the accrual at freeze (§6.3/§6.4).
- **E-131** — emissions (OR-14 default BEST-EFFORT): `attribution_recorded` (G-25 #31, keyed `attribution_recorded:<order_id>` — §14.5 widens the key to order_id) from the resolver; `promoter_commission_accrued` (G-25 #32, dedup `commission:<attribution_id>` per NOTIF §5) at the payout mint. Neither is REQUIRED (no other invariant depends on the envelope).
- **E-132** — `PROMOTER_COMMISSION_PAYOUT_HOLD` resolution: commission payouts carry `cause_ref = attribution_id` with `idempotency_key = 'promoter_commission:<attribution_id>:<payee_identity_id>'` (schema §1.9 · RPC §20.7.2 · PROMO §4.2(3), byte for byte; OR-6: the physical/money authorities own the shape). 088's dispute payout-hold leg reaches settlement payouts through lines whose `cause_ref` equals the disputed ORDER id — no line of any package carries an order id — so it reaches no commission payout under EITHER shape; "keep the settlement_id shape" would have bought nothing and broken the one-payout-per-attribution proof. The obligation is re-scoped below (owner-signed body-only amendment of `record_dispute_native` adding an attribution arm). Exposure is bounded as PROMO §5.3 states (settle-then-pay; the org absorbs — ODR-32 default). **OWNER COUNTERSIGNATURE (2026-09-02): APPROVED — CLOSED.** `cause_ref = attribution_id` is the canonical identity tying the payout liability to the unique economic attribution; the exactly-once constraints on (attribution, payee) and the existing idempotency model are preserved.
- **E-133** — `venue.check_promoter_slug_available(p_slug)` takes no scope parameter, so "scoped to the promoter's org" resolves to: the caller holds a promoter-program-manager label somewhere (`org_owner`/`org_admin`/`org_promoter_manager` on any org, or `venue_manager`/`venue_promoter_manager` at any venue); fans and any other label are refused (42501). T-RPC-PROMO-14's "org_owner of a DIFFERENT org" cannot be evaluated without a scope; the payload is `{available}` and nothing else (asserted by field-list comparison).
- **E-134** — deploy artifacts, not SQL: the `promoter-code-preview` edge function (§7.10/§17.17, the anonymous-principal derivation is the edge's) and `primary-checkout`'s `code` + `link_slug` parameters (edge §3.1 correction → `venue.bind_order_attribution` while pending). Recorded as `PROMOTER_CODE_PREVIEW_EDGE` / `PRIMARY_CHECKOUT_CODE_PARAMS` below (the 087 `crm-export` precedent).
- **E-135** — `create_promoter_codes_bulk` returns the generated set (`codes[]`, `code_ids[]`) in its result and records it in the ONE audit row's `after` (the "audited export path" download is a dashboard artifact; the issuer must receive the codes it minted). Bulk is `kind=generated` only (a vanity display cannot be generated).
- **E-136** — `venue.attribution.link_id` is the link that PRODUCED the credit (P1: the corroborating link; P5/P6: the winning link); a displaced (P2) or ineligible (P3) link is not stored on `link_id` — P2 records the displaced promoter on `displaced_promoter_id`. `code_id` is the winning code (P1–P4). `attribution_method_ck` binds `method` to the populated column.
- **E-137** — `bind_order_attribution` evaluates the same eligibility rule the resolver does (advisory): an unknown AND an ineligible code both clear the candidate and return the identical `{bound:false, reason:'not_applicable'}` (§9.4 — no existence oracle through the binding path); scalar parameters make M1/M2 (two codes / two links in one request) unpresentable at the DB. Box-office binding is admitted for `source='door'` orders only.

- **E-138** — COMMISSION PAYOUTS ARE MINTED UNDER A SYSTEM HOLD (`hold_state='held'`, `hold_reason_code='unfunded_settlement'`, `held_by NULL`, BE `payout_on_hold` notice — 088's dispute-hold shape). Red-team C1/B3: no package writes a primary-revenue settlement line, so an event settlement's gross is 0, the commission debit drives `net_minor` negative and 087's `close_settlement` mints no org payout (`if v_net > 0`) and carries nothing forward — the org's debit is recorded but never collected — while the promoter's `+payable` payout would be a live liability the moment a payout executor runs; and no contracted path advances a `promoter_commission` payout (`request_org_payout` is `cause='settlement'` only; `venue.on_payout_settled` ignores other causes; `mark_payout_transfer_state` is cause-agnostic but refuses a held payout). X-12 fail-to-safe: the liability is RECORDED (line + held payout, exactly-once), no money can leave, and `kernel.release_payout` (platform_risk / platform_admin, Control-5) is the release once the owner rules the funding source. `COMMISSION_FUNDING_SOURCE` / `COMMISSION_PAYOUT_LIFECYCLE` below; **OWNER DECISION REQUIRED** (packet in the 090 closing report). The bounded exposure PROMO §5.3 accepts for chargebacks (a negative line the org "absorbs") is the same class; for commissions it is 100% of every commission, which is why 090 holds rather than absorbs. **OWNER RULING (2026-09-02) — POLICY CLOSED / OWNER-RATIFIED · IMPLEMENTATION OBLIGATION OPEN:** OPTION B — promoter commissions are funded FROM PRIMARY TICKET REVENUE THROUGH THE VENUE SETTLEMENT ACCOUNTING MODEL (primary ticket revenue → promoter commission liability → venue distributable settlement): a commission is an economic deduction from the venue's primary-sale proceeds before the corresponding distributable venue money leaves the settlement system. NOT chosen: (A) generic carry-forward of an unfunded negative settlement as the primary mechanism, (C) arbitrary collection from the org's Stripe balance as the normal mechanism, (D) platform-funded commissions / platform absorption. FUNDING IS NOT ACTIVATED by this ruling: until the primary-revenue settlement funding leg is mechanically implemented, tested and authorized, ALL promoter commission payouts REMAIN HELD (`unfunded_settlement`) — 090's behaviour stands; nothing is advanced or released on the strength of the policy. See `COMMISSION_FUNDING_SOURCE` below.
- **E-139** — command-key replays are BOUND to their parameters (087's `idempotency_conflict` discipline): `create_promoter`, `create_promoter_link`, `create_promoter_code`, `create_promoter_codes_bulk` and `review_attribution_flag` compare the audited `after` payload with the call and raise `precondition_failed: idempotency_conflict` on a mismatch (red-team B2: a reused key could otherwise report another promoter's code as "created"). The `create_promoter` audit row now records `identity_ref` and `event_id` for the comparison.
- **E-140** — §6.2's currency assertion: the resolver writes an attribution ONLY when `order.currency = promoter.currency` (else no row — never a raise) and stamps the ORDER's currency (red-team B1: an EUR order could otherwise mint a USD commission). The E-128 `currency_mismatch` hold at close remains the second wall.
- **E-141** — `kernel.pay_promoter_commission` takes the per-attribution advisory key `attribution.review:<id>` before reading the effective decision — the SAME key `review_attribution_flag` takes — so a release/deny cannot interleave between the decision read and the line commit (red-team E1: `attribution_settled` was bypassable under READ COMMITTED). Lock order: org seam key → Settlement(6) → attribution keys in id order → Payout(6); the review path takes only the attribution key and no settlement lock, so no cycle.
- **E-142** — `bind_order_attribution` writes ONLY the channel that was presented: a code-only call never wipes a bound link and vice versa (red-team H-1: P3/P5/P9 were unreachable for the natural client call; §7.6 scopes "sets the candidate to NULL" to the failing channel).
- **E-143** — `confusable_with[]` is OWN-ORG only (red-team D: a self-created org's issuer could otherwise harvest every tenant's codes within edit distance 1 — §9.4(4), PROMO §8.1 `A(own-org)`). The issue paths (`create_promoter_code`, `create_promoter_codes_bulk`, `create_promoter_link`) carry the same per-principal fail-closed limiter as the availability check (`promoter-issue`, 30/min) because `code_taken` / `slug_taken` are global-namespace existence signals.
- **E-144** — `review_attribution_flag` authority follows E-76: the venue arm binds only while the venue is still operated by the ATTRIBUTION's org, and the org arm is `has_org_role(attribution.org_id, [org_owner, org_admin])` (never the venue's current operator — a transferred venue cannot release the former org's money); the decider is never the attributed promoter's own identity (separation of duties, AUTHZ-H10's principle). `list_promoter_attributions` admits `platform_support`/`platform_risk`/`platform_admin` (scope-bound): under AUTHZ-M9 / T-RLS-ATTR-06 the ledger carries no table grant, so §9.17's platform `A`/`V` cells and §16.10's `venue_attribution_sel_platform` are delivered by the projection, not a policy (the two texts contradict; the zero-grant form wins as the stricter and the later correction).
- **E-145** — `create_promoter` refuses a `DELETION_PENDING` or `ERASED` identity with ONE message (`precondition_failed: identity_ineligible`): the state of a third party is not disclosed by e-mail (red-team J). `preview_promoter_code` never echoes the caller's input as a display name (an identity-less affiliate previews as `Promoter`).
- **E-146** — the resolver reads the promoter row IN the eligibility statement (one snapshot: status and terms cannot tear between two statements — red-team A1); transient failures (SQLSTATE classes 40/57) land under `transient.<sqlstate>` so they are separable from logic failures in the `attribution.resolver_error` queue (a swallowed transient is a permanently lost commission — §7.11's asymmetry, stated); the seam excludes terminal classes (refunded order, denied at max(seq), identity-less payee, currency mismatch) so a permanently-held attribution is not re-walked under the settlement lock at every close; `pay_promoter_commission` holds an `amount_overflow` rather than raising `22003` out of the close; `kernel.is_promoter_for_event` carries E1′ and the E3 window. The ADOPT step (ACCESS EXCLUSIVE on `venue."order"`) is the LAST statement before COMMIT.
- **E-148** — red-team second pass (money/concurrency lens re-run over the remediated bytes): the eight P0/P1 remediations CLOSED with file:line evidence; P0=0 P1=0; residuals taken in bytes — `create_promoter_codes_bulk`'s replay is bound to all six parameters (kind + window recorded and compared), the rollback locks `venue."order"` FIRST (the candidate-clearing step would otherwise invert the checkout lock order against a concurrent finalize), the seam also excludes `cancelled` orders as a terminal class (a fully-voided non-refunded order remains re-walked — rare, accepted); residuals recorded, not changed — the `hold_payout` noop path and the BP-6 erasure consequence (see `COMMISSION_PAYOUT_LIFECYCLE`).
- **E-147** — the rollback LOCKS `venue.attribution`, `venue.settlement_line`, `kernel.payout` ACCESS EXCLUSIVE before counting (no finalize/close can slip a row between the count and the drop) and CLEARS the two candidate columns under a disabled 082 freeze guard before dropping the promoter tables (a pointer into a dropped table has no meaning; otherwise the next apply's `VALIDATE CONSTRAINT` fails 23503 — the package would be un-re-appliable). Recorded, not re-ruled: `venue.promoter_code_scope.added_by` is a fifth `auth.users` FK the ODR-16 inventory (rows 35–38) does not carry — TOMBSTONED by default, INV #37's disposition; four hardening triggers beyond plan §8/090's Triggers row (`tg_promoter_consistency`, `tg_promoter_link_consistency`, scope-row `raise_append_only` on UPDATE, `set_updated_at` on `promoter_code`); policy names `…_sel_own` / `venue_promoter_code_scope_sel` for §16.10's `…_sel_promoter` trio; `get_my_promoter_summary` / `list_my_attributions` REFUSE (42501) a caller with no live promoter row where §12.52 says "zero rows" (stricter); `resolution_reason` admits `none_eligible` / `no_attribution_presented` that no row ever carries (P7–P10 write nothing); `promoter_code_normalized_pattern_idx` (§10.3) is never read by the equality-probe confusable search; the `pg_context` guard depends on both `settlement_commission_lines` and `close_settlement` staying `plpgsql`.

## Forward obligations opened / re-scoped by package 090 (OWNER: UNASSIGNED unless frozen bytes assign them)
- **`COMMISSION_FUNDING_SOURCE`** (E-138) — **OWNER POLICY: RESOLVED (2026-09-02, OPTION B) · IMPLEMENTATION: OPEN.** TARGET ECONOMICS: commission funded from primary ticket revenue before venue distributable settlement money leaves the system (primary ticket revenue → promoter commission liability → venue distributable settlement). Rejected as the normal mechanism: carry-forward of an unfunded negative net (A), arbitrary org Stripe-balance debit (C), platform advance/absorption (D). The implementation must eventually prove: the exact primary revenue source · the exact settlement-line representation · the commission deducted ONCE · conservation · no duplicate funding · refund behaviour · chargeback behaviour · cancellation behaviour · the held-payout release condition · payout destination readiness · settlement-close concurrency · no platform advance · no arbitrary Stripe-balance debit. Until that leg is implemented, tested and authorized, every commission payout is minted and REMAINS HELD `unfunded_settlement` (no automatic release; `kernel.release_payout` is a per-payout Control-5 action, not a funding rail). NOT assigned to any package unless frozen package bytes assign it.
- **`COMMISSION_PAYOUT_LIFECYCLE`** (E-138) — the advance path for a `promoter_commission` payout and whether §17.7's controls (SoD-1, money-role maturity, AAL2 step-up, destination probation, `dual_control_min_minor`) apply: `request_org_payout` is `cause='settlement'` only, `venue.on_payout_settled` ignores the cause, and the payout executor edge function (RPC §13) is a deploy artifact. Owed with the funding ruling. Two consequences of the E-138 hold to rule with it (re-review NEW-1/NEW-4): (i) `kernel.hold_payout` (085, frozen) returns `noop_replay` on an already-held payout without comparing the reason and without an audit row, so a risk hold placed on a commission payout born `unfunded_settlement` leaves no trace and `kernel.release_payout` (no reason argument) would clear both at once — the funding ruling's release must be reason-scoped (`hold_reason_code = 'unfunded_settlement'`) and an owner-signed body-only amendment of `hold_payout` should make a different reason an overwrite-with-audit; (ii) every promoter with an accrued commission now trips BP-6 (identity payout under hold) in addition to BP-5 (pending identity payout), so an ODR-16 erasure request for an active promoter needs per-payout Control-5 releases until the funding path exists.


- **`PROMOTER_COMMISSION_PAYOUT_HOLD`** (re-scoped by E-132; remains OPEN) — 088's `record_dispute_native` payout leg does not reach `promoter_commission` payouts (their `cause_ref` is the attribution id by the frozen money shape). Owed: an owner-signed body-only amendment of `kernel.record_dispute_native` (088) adding an attribution arm — hold payouts `where cause='promoter_commission' and cause_ref in (select id from venue.attribution where order_id = <the disputed order>)` — or an explicit ruling that a disputed sale's commission payout is not gated. Until then a chargeback that arrives after close does not hold a pending commission payout (ODR-32 default: the org absorbs).
- **`AFFILIATE_PAYOUT_DESTINATION`** — an affiliate (`party_kind='affiliate'`, `identity_id NULL`) accrues attributions but has no payout destination (`kernel.payout` requires an identity or an org payee); the seam holds its commissions `payee_unresolvable` forever. Owed: a ruling on the affiliate payee (an org-side settlement adjustment? an identity binding path? — `update_promoter` cannot set `identity_id` by contract).
- **`PROMOTER_MANAGER_CODE_READ`** (E-124) — the promoter-manager labels can create/manage codes and links but hold no direct SELECT on `venue.promoter_code` / `promoter_link` / `promoter`; the dashboard's codes tab (§11.2) has no read surface for them. Owed: a SELECT cell in RLS §9.17 / PROMO §8.1, or a read RPC.
- **`PROMOTER_CODE_PREVIEW_EDGE`** — the `promoter-code-preview` edge function (verify_jwt false; service-role client for `public.check_rate_limit` only; anonymous principal `uuidv5(NS_PROMOCODE, ip || ':' || sha256(user_agent))`; 5/min anon; burst audit `promoter_code.enumeration_suspected` ≥ 30 `not_applicable` per 5 min; never logs the code string). Deploy artifact.
- **`PRIMARY_CHECKOUT_CODE_PARAMS`** — `primary-checkout` gains optional `code` + `link_slug`, bound through `venue.bind_order_attribution` on the caller's client while the order is pending. Deploy artifact.
- **`CHARGEBACK_CROSS_SETTLEMENT_UNIQUE`** — UNTOUCHED (Gate-M / PFA-29): 090's `attribution_one_commission_line_ever` covers `cause='promoter_commission'` only, exactly as §3.14.1 assigns it.

*(register maintained per PHASE_2_ARCHITECTURE_FREEZE.md §4)*

## ERRATA recorded by package 091 (no amendment needed; frozen bytes win as stated)

- **E-149** — `kernel.reserve` (plan §8/091 · schema §1.11) ships with the minimal six-column shape and nothing more: `org_id` is NOT NULL and `ON DELETE RESTRICT` (the frozen text states the FK to `kernel.organization`, not its nullability or action; an ownerless reserve row has no meaning, RESTRICT is the action every FK to `kernel.organization` carries, and both are additively relaxable — a platform-level reserve is a live Gate-M shape) and `balance_minor` carries NO CHECK (none is stated; a Gate-M receivable posture — a negative balance — is not pre-empted by the stub). `balance_minor integer` is the frozen type (schema §1.11); as an accumulating pool it caps at 2,147,483,647 minor units — int8 widening is a Gate-M precondition, recorded here. No uniqueness on `org_id` ships (plan Indexes row: PK only); the Gate-M writer must state whether a reserve is one-per-org or one-per-(org, currency) and add the unique before its first write — recorded so it is confronted, not inherited. The table is the C29 MONEY reserve (schema §1.11: "funding source for instant payout / cancellation refunds / C25 auto-refund"), not an inventory reservation object — no inventory lock class, TTL or oversell rule exists in this package. No function, RPC, policy, writer, cron row, config key, seed, edge or SEAM hook is created; "stub" is asserted as a checked property (empty · deny-all · no routine references it · writer registry row `NONE-wired-in-MVP` satisfied by a table with zero writers). Rollback DROPs the table behind a guard that refuses a non-empty stub (forward-fix, never drop money state).
- **E-150** — RLS §7.11's `service_role A(machine) / R(def, future)` cells are NOT delivered — a frozen-cell departure recorded here, not a byte-fidelity claim: the stub carries an explicit `REVOKE ALL` for `public`, `anon`, `authenticated` AND `service_role`; 085's four money ledgers issue no `service_role` grant at all (their revoke names anon/authenticated only), so the OBSERVABLE posture is identical — every client and machine role holds nothing on every money ledger, and §7.11's `A(machine)` cell is likewise undelivered on all of them (E-118/E-106 class: no dormant machine grant on a money table). A Gate-M writer will be a definer path. The trigger `tg_reserve_set_updated_at` is the one write-capable object on the table (plan Triggers row); it can never fire (no UPDATE path exists) ; the writer-registry row keeps its frozen form (`-|-|NONE-wired-in-MVP`) — the registry records `CATEGORY:set_updated_at` only on the ten R-35 fence rows by convention, and a one-writer row cannot cite two sections under gate check H.
- **E-151** — 091 red team (two lenses; P0=0): the rollback's empty-guard now runs `set local row_security = off` before counting (the 081–087 house pattern — a deny-all zero-policy table counts 0 for a non-BYPASSRLS, non-owner runner and a guard without it fails OPEN); the 088, 089 and 090 rollbacks omit the same line — recorded as `ROLLBACK_GUARD_ROW_SECURITY` below (owner-signed body-only amendments; exposure today is nil: the tables are owner-run and the guards are exercised as the owner). The guard also refuses when any routine references the table (no pg_depend edge exists for PL/pgSQL bodies; a drop would leave a compiling-but-broken money function). The 090 rollback header's "090 is the chain tip" is now stale (091 is; the two rollbacks commute — 091's only FK targets 077) — a comment in a merged file, left as is. Suite 156 replaced two vacuous assertions (a `pg_depend` probe that can never fail for PL/pgSQL bodies; a hold check over an empty payout table) with the droppability precondition and a byte-pin of 090's hold mint + 085's held-payout refusal, made the cron/config censuses absolute, and asserts the materialized owner-only ACL.
- **091 and the commission funding leg** — the owner's `COMMISSION_FUNDING_SOURCE` policy (Option B) is NOT implemented here: no frozen 091 byte assigns it, and the stub is by contract "always empty, nothing may be added to it". Every `promoter_commission` payout remains HELD `unfunded_settlement` after 091 (asserted).


## Forward obligations opened by package 091 (OWNER: UNASSIGNED unless frozen bytes assign them)

- **`ROLLBACK_GUARD_ROW_SECURITY`** — the CLEAN-WHILE-EMPTY guards of the 088, 089 and 090 rollbacks count deny-all zero-policy tables without `set local row_security = off`; run by a non-owner, non-BYPASSRLS migration role they would report empty and drop money state. 091's guard carries the line. Owed: owner-signed body-only amendments of the three earlier rollback files (no migration bytes change).

*(register maintained per PHASE_2_ARCHITECTURE_FREEZE.md §4)*

## ERRATA recorded by package 092 (no amendment needed; frozen bytes win as stated)

- **E-152** — `GRANT USAGE ON SCHEMA notify TO authenticated` lands in 092: RLS §16.9 (owner-scoped `notify.notification` SELECT + UPDATE(read_at); owner CRUD-minus-delete on `notify.preference`) and RLS §11 / NOTIF §6.3 (nine `EXEC: authenticated` consumer RPCs) are inert without schema USAGE; 076's wall was the state with no client object. `anon` stays walled (suite 140 #8 re-scoped with the dated note; 157 B1–B3, C21, G25–G27). The emit pair and the six internal definers remain `service_role`-only.
- **E-153** — `notify.outbox` gains two ADDITIVE columns, `expand_cursor uuid` and `expanded_count integer NOT NULL DEFAULT 0`: RPC §17.24's bounded-expansion obligation, mechanism (i). The custody expansion (`event_cancelled`) processes ≤ 500 atoms per tick, persists the cursor on the envelope (state stays `pending`, `attempt+1`) and completes only when exhausted (157 E41–E48 drive a 603-atom cancellation across two ticks). Suite 140 #19's fifteen-column pin becomes seventeen with the dated note.
- **E-154** — `notify.delivery_lease_interval` is SEEDED OWNER-UNSET (`'null'::jsonb`, `restricted` — the PFA-22 shape): the corpus names the key (RPC §17.25) and never its value (PFA-9 CLASS A). `notify.claim_deliveries` FAILS CLOSED (`precondition_failed`) while it is unset; the value is an owner decision (`NOTIFY_DELIVERY_LEASE_VALUE`, below). Suite 142 D41 now pins the two door keys and D41b pins this key's NULL.
- **E-155** — two ADDITIVE contract extensions on the §17.25 machine surface, both needed for the edge to act on what the DB returns: (1) `claim_deliveries` returns `rendered_subject`, `rendered_body` after the nine frozen fields — rendering happens at claim (send) time and is frozen into `notify.delivery.rendered_*` (§5.3); the edge needs the wire copy and holds no template read; (2) `record_delivery_result` takes a seventh, defaulted `p_token text` — the delivery row is per (notification, channel) while `device_not_registered` names one device; the token is revoked only when it belongs to the delivery's recipient, and the identity becomes push-unreachable iff no live token remains.
- **E-156** — `notify.resolve_web_link` composes the closed target set onto routes that EXIST in `web/` today (`/listing/[id]`, `/transfer/send|receive`, `/account/{purchases,sales,settings,notifications}`, `/browse`); kinds with no detail route yet (ticket, order, refund, payout, event) resolve to their hub page. Never a URL in a row (§4.4; 157 I5–I7). Adding a detail route is a body-only change.
- **E-157** — §5.4's second locale link ("device locale captured at push-token registration") has NO column: `public.push_tokens` is extended by exactly the four §13.4 columns, and writing a device locale into `kernel.identity_ext.locale` would turn the second link into the first (077's "never a default written into every row"). `register_push_token` VALIDATES `p_locale` (BCP-47 shape) and does not persist it; resolution is `identity_ext.locale → 'en-US'`. Owner item `NOTIFY_DEVICE_LOCALE_COLUMN` below.
- **E-158** — of the CRON_SCHEDULE_REGISTER's three 092 rows only the DB tick `notify-drain-outbox` (`*/2`, `notify.drain_outbox(200)`) is scheduled. The two pg_net edge ticks (`notify-dispatch` 1 min, `notify-receipts` 15 min; EDGE §3.14/§3.15) need a dedicated header NAME and a Vault secret NAME that no frozen byte supplies (the 087 `crm-export` precedent needed both and the E-79 `RESALE_CHECKOUT_SWEEP_TICK` class before it) — PARKED as `NOTIFY_DISPATCH_TICK`; the edge functions themselves are deploy artifacts (`supabase/functions/notify-dispatch`, `notify-receipts`: not authored, not deployed).
- **E-159** — the drainer marks envelopes of event types with no notification consumer (`attribution_recorded` — OUT under ODR-3; `DoorManifestOpened`/`Closed` — scanner consumers) and any unmapped type `done` with zero rows and counts them `unmapped` in its result. An envelope is a fact; a consumer that has nothing to say for it must still retire it, or the outbox never drains. A malformed envelope (unresolvable aggregate) is quarantined `dead` with `last_error`; transient failures (SQLSTATE classes 40/55/57) re-run up to five times; one bad row never blocks the batch (157 E12–E15).
- **E-160** — `notify.notification.created_at` defaults to `clock_timestamp()`, not `now()`: `get_inbox`'s keyset cursor is a single `timestamptz` by frozen signature, and one drain tick writes hundreds of rows in ONE transaction — under `now()` they would share a timestamp, form one keyset tie, and a page boundary would skip rows (157 G4b/G4c). Residual: microsecond ties remain theoretically possible; a composite cursor is `NOTIFY_INBOX_COMPOSITE_CURSOR` below (signature change = amendment, not a body edit).
- **E-161** — PRODUCER PARITY IS PARTIAL BY THE FROZEN PACKAGE ORDER, recorded not implemented: 13 of the 31 IN types have a shipped DB producer (077: `security_org_role_granted/_revoked`, `account_deletion_pending/_completed`; 085: `refund_request_expired/_cancelled`; 087: `payout_request_pending_approval`; 088: `ownership_changed`, `payout_on_hold`, `purchase_confirmed`, `refund_requested`, `event_cancelled`; 090: `promoter_commission_accrued`, `payout_on_hold`). The drainer consumes exactly those envelope shapes (aggregate kind + payload as shipped; 157 section E). The other 18 are (a) edge producers not yet authored (`purchase_failed`, `refund_submitted`, `refund_completed`, `refund_failed`, `wallet_pass_available`, `security_payout_destination_changed`, `security_payout_method_added`) or (b) DB producers whose IMMUTABLE package shipped without the emit clause (`ticket_ready` 083, `payout_released`/`payout_failed`/`staff_payout_failed` 085, the request-arm `refund_requested` + `refund_request_parked/approved/denied` 085, `event_time_changed/venue_changed/postponed` 079, `security_password_changed` via the legacy 0600 sweep, the staff half of `security_org_role_*` 080). Their registry rows and templates are seeded so a body-only amendment on the producer side needs no 092 change. `NOTIFY_PRODUCER_PARITY` below.
- **E-162** — 092 red team (lenses A–M, two passes; P0=0 P1=0). Remediated in bytes: (C) a delivery whose lease expires five times is dead-lettered on the row, not re-claimed forever; (K) a delivery with no template for its channel is dead-lettered `template_missing` and never handed to the edge as an empty message; (B/E) the custody expansion takes the holder from the ledger's `refund_void` entry (085's cascade moves custody to SN-VOID `0000…00f0`, which is never a recipient); (D) `actor_identity` never reaches a notification, push copy for money/custody types carries no amount and no name (157 A34, F9–F10, I1–I5). Residuals recorded as obligations: `NOTIFY_INBOX_COMPOSITE_CURSOR` (P3), `NOTIFY_DRAIN_THROUGHPUT` (P3: 200 envelopes / 2 min and 500 atoms per envelope-tick — a 10 000-atom cancellation notifies over ~40 minutes; the constants are body-only).

## Forward obligations opened / discharged by package 092 (OWNER: UNASSIGNED unless frozen bytes assign them)

- **`NOTIFY_PRODUCER_PARITY`** (E-161) — IMPLEMENTATION REQUIRED · BLOCKS FEATURE ACTIVATION (per rail: a rail whose money/custody/viability type has no producer must not go live with the registry promising it). Owed: body-only `CREATE OR REPLACE` amendments adding the OR-14-classified emit clause to the 079/080/083/085 producers named in E-161(b), and the edge producers of E-161(a) as deploy artifacts. No 092 byte changes.
- **`NOTIFY_DISPATCH_TICK`** (E-158) — EXTERNAL OPERATIONAL REQUIREMENT + IMPLEMENTATION REQUIRED · BLOCKS FEATURE ACTIVATION (notifications rail). Owed: the two edge functions, their Vault secret name and dedicated header name (owner-named), then two `cron.schedule` rows in the 087 pg_net shape (fail-closed on a missing Vault row).
- **`NOTIFY_DELIVERY_LEASE_VALUE`** (E-154) — OWNER POLICY REQUIRED · BLOCKS FEATURE ACTIVATION. A `catalog.set_platform_config` write of `notify.delivery_lease_interval` (the corpus offers no number; a lease shorter than the edge's send timeout double-sends, longer strands a crashed sender's rows for its length).
- **`EMAIL_GO_LIVE`** (NOTIF §10 O-N3 / ODR-3 §6 N1) — OWNER POLICY REQUIRED + EXTERNAL OPERATIONAL REQUIREMENT · BLOCKS FEATURE ACTIVATION (the 17 mandatory types whose redundant channel is email). Every E delivery row is born `suppressed / channel_unavailable`; go-live is: a sending domain with DMARC settled (ODR-3 §6), `RESEND_API_KEY`/`EMAIL_ENABLED`, email templates (registry migration), and a body-only change to `notify.enqueue`'s email arm.
- **`NOTIFY_DEVICE_LOCALE_COLUMN`** (E-157) — OWNER POLICY REQUIRED · OPTIONAL-FUTURE (MVP is en-US only). Whether §5.4's second link gets a column on `public.push_tokens` (schema §13.4 would need amending) or is dropped from the chain.
- **`NOTIFY_INBOX_COMPOSITE_CURSOR`** (E-160) — IMPLEMENTATION REQUIRED · OPTIONAL-FUTURE. A `(created_at, notification_id)` cursor for `get_inbox` (signature amendment).
- **`NOTIFY_DRAIN_THROUGHPUT`** (E-162) — IMPLEMENTATION REQUIRED · OPTIONAL-FUTURE. Tick size / chunk size / cadence are body-only constants; raise once a real event size is known.
- **DISCHARGED by 092:** RPC §17.24's bounded-expansion obligation (E-153, mechanism (i)); PFA-9's `notify.delivery_lease_interval` authoring item (E-154 — seeded owner-unset, value still owed); NOTIF §9's N-A1…N-A44 minus the OUT rows (announcements H, N-A12/N-A13 schedule) are asserted by suite 157; the 077/085/087/088/090 emit clauses now have a consumer (E-161).
- **UNTOUCHED:** `ROLLBACK_GUARD_ROW_SECURITY` (092's own rollback follows the 091 pattern: `set local row_security = off`, tables locked, external-reference scan, append-only trigger lifted for the config delete); every 088/090/091 obligation above stands as written; `COMMISSION_FUNDING_SOURCE` remains OWNER POLICY RESOLVED / IMPLEMENTATION OPEN and every `promoter_commission` payout remains HELD (157 K4).

*(register maintained per PHASE_2_ARCHITECTURE_FREEZE.md §4)*

## RELEASE-READINESS RECORD — 2026-09-02 pre-deployment audit (no production mutation; no frozen migration byte changed)

- **E-163 — `ROLLBACK_GUARD_ROW_SECURITY` CLOSED.** The seven rollback guards that counted RLS-enabled zero-policy tables without `set local row_security = off` (076 · 077 · 078 · 079 · 080 · 088 · 090) now carry it, placed before the first count (088 additionally locks the four tables it drops, the 090/091/092 house pattern). Proof: the 17-package rollback battery (`scripts/release/local_rollback_battery.sh`) passes with catalog-identity diff 0 at every position, and a dirtied 077 world REFUSES under the owner runner and ERRORS (fail-closed, never open) under a non-BYPASSRLS runner. Rollback files are release tooling, not frozen migration bytes — no immutability boundary was crossed.
- **E-164 — production ledger drift found and repaired in-repo.** Production carried `20260902003623_admin_relist_listing_rpc` (applied 2026-09-02; a narrow SECURITY DEFINER admin relist verb, EXECUTE service_role only) with no repo commit. Reconstructed byte-verified (md5 `3919213d09300127d199313d95200971` over the ledger's recorded statement) into `supabase/migrations/`; Gate-2 `EXPECT_FUNCS` 69→70 with a dated note. The repo's version set now equals production's plus exactly 076–092. 043's absence from production and from this chain is CONFIRMED DELIBERATE from production's own recorded 042 header (staged mobile adoption; the `mobile/profile-rpc-compat` track owns it).
- **E-165 — the 077 release-train artifacts are AUTHORED (not deployed).** Per the standing 077 RELEASE-TRAIN GATE: `delete-account` rewritten to the §1.8a OR-17 cutover (Class A caller-JWT → `kernel.request_account_deletion`; `action:'withdraw'` route; 409s retired; `auth.admin.deleteUser` called by nothing; deployed-client compatible); F-5 live-rail guards authored at the edge layer (`create-payment-intent` — the funding chokepoint; `confirm-and-release` — inbound-transfer confirm with the initiated-after comparison via the caller's own `kernel.identity_ext` row) and at the RN layer for direct-insert bids (`src/screens/PlaceBidScreen.tsx`). Guards fail OPEN on probe error by design — the DB sweep's BP wall is the enforcement layer; edge/RN is the frozen UX layer (FR-9). REMAINING NAMED CELLS (block product-completeness, not the DB apply): the Settings withdraw-deletion UI (entangled with PR #28's file — see the readiness report §6) and the tombstone-flow storage reaper.
- **E-166 — PostgREST exposure is a discovered release-train requirement no census had named.** The Class A edge calls and the RN own-row deletion-state reads travel through PostgREST; the API's exposed schemas must gain `kernel` on the 077 train (Dashboard config — an external operational step, not SQL). `catalog`/`venue`/`market`/`notify` exposure rides each rail's activation. Recorded in the runbook as a deploy-order step with its verification probe.
- **E-167 — PFA-1's production witness.** The compensating control's production-side sweep is `scripts/release/phase2_postapply_verify.sql` V13 (zero PUBLIC/anon EXECUTE across kernel/venue/market/notify), mandated at +5M/+2H/+24H in the runbook. PFA-1 is thereby closed for the dark apply.
- **Proof artifacts added under `scripts/release/`:** `phase2_preflight.sql` (21 read-only checks — run against LIVE production 2026-09-02: 21/21 PASS), `phase2_postapply_verify.sql` (18 checks — 18/18 PASS on the production-order rehearsal), `local_rollback_battery.sh`, `local_prod_order_rehearsal.sh` (production-order apply: 1 s wall clock; per-table data checksums prove ZERO live-row mutation outside 078's two sentinel identities; suites 2 622/2 622). Runbook + rollback decision tree + this pass's full census: `docs/release/`.
...
## PFA-33 — operators never become members of customer organisations: platform-assisted bootstrap (A1–A3) and the acceptance refusal (A4) (migration 138, unapplied) — SIGNED 2026-09-17 (owner, written approval in the A conversation)

**Placement (A, 2026-09-17):** the text below is placed VERBATIM from D's contract, `docs/venue-dashboard/OPERATOR_ONBOARDING_CONTRACT_D_20260917.md` on `review/d-release-sprint @ dd86f77` (amendment block md5 `2da381a1667b0c9e873b709ff3f1d7ce`, 5731 bytes, 59 lines; extraction rule: the contents of the fenced code block under this PFA-33 heading, excluding both fence lines and with no trailing newline, UTF-8; D reproduced the same value independently). It names the implementation it amends by: `ops/138-operator-onboarding @ 0cfa8ba` (CI 35249486531 green on all five jobs; pgTAP Tests=5403, Result: PASS). **A's independent reviews:** 8ecc929 (no defect found; the membership paths (a)–(f) are verified in source, and (f) is added as a LIMIT) and the hardening delta 0cfa8ba (the fixed-text outcome map covers all 11 messages A2 raises; 206 140/140 on A's local replay). **The quoted rulings** match, word for word (apostrophe style aside), the owner's message of 2026-09-17 in the original A conversation; the owner states that D received them directly as well. **Signing method (owner, 2026-09-17, to A):** "use my explicit written approval in this conversation as the signing method. Prepare the final amendment with its exact revision and commit, then bring it to me for that approval. My earlier direction authorizes development; do not record an unseen final amendment as signed." **OWNER SIGNATURE: RECORDED (A, 2026-09-17).** The owner wrote, in the A conversation: "I approve and sign PFA-33 at governance commit bf7fd66, amendment checksum 2da381a1… . Record the signed block exactly as defined in the corrected placement note." The signed block is the fenced block below, byte-identical to bf7fd66 (md5 `2da381a1667b0c9e873b709ff3f1d7ce`, 5731 bytes, 59 lines under the extraction rule above; re-verified at this commit). Its words "PFA-33 (proposed)" and "A records how the owner signs" are part of the signed bytes and stay unchanged; this note is the signature record. The same approval also reached D in D's conversation. This record rests on the approval given to A, not on D's relay. **Signing does not apply migration 138.**

**Owner rulings given with the signature (2026-09-17, verbatim, outside this amendment).** Rulings 1–3 change or settle behaviour beyond PFA-33 as signed, so they go into a follow-on amendment: D drafts the text naming the new 138 commit; A places it and assigns the ID.
1. "A suspended organization may receive a recovery owner invite only when it has no current owner, using the same two-person approval, identity checks and audit controls. No invite may replace an existing owner." (Answers A2's "Owner to confirm". A verifies at the new head that 206 covers both suspended cases: no owner is accepted, an existing owner is refused.)
2. "Approve A5: prevent a platform-authority identity from directly creating an organization and becoming its owner. Route creation through the approved platform bootstrap flow." (Changes frozen RPC §2.1 `kernel.create_organization`.)
3. "Approve the names-only detection read for platform identities who also hold organization membership. It must be read-only, scoped, and reported without exposing unrelated personal data." (The path-(d) detector. Before any run, the exact query text and target project go to the owner; a production run stays under the owner-named production-read rule.)
4. "Do not apply migration 138 yet. Integrate A5 and the approved amendment, rerun the front-door tests and review, then bring me a new apply package."
...
**Placement (A, 2026-09-17):** the text below is placed from D's contract, `docs/venue-dashboard/OPERATOR_ONBOARDING_CONTRACT_D_20260917.md` on `review/d-release-sprint @ 5b501b8`, with ONE deviation, which D's own text invites: the ID line read "(assigned by A at placement)" and now reads "PFA-34 (proposed)". Source block md5 `ea1c2999dc1099f869e23eb227c1671f`; **placed block md5 `026cb858319bc7c0181e1dad01e23ef1`, 2726 bytes, 27 lines** (extraction rule: the contents of the fenced code block under this PFA-34 heading, excluding both fence lines and with no trailing newline, UTF-8). **D verified this independently (D, 2026-09-17), reading governance `b7895bb` itself rather than A's message:** the placed block hashes `026cb858319bc7c0181e1dad01e23ef1`, 2726 bytes, 27 lines, and a line diff against D's source block shows exactly one difference, the ID line. D's contract at `review/d-release-sprint 9e21e81` now mirrors the placed text and recomputes to the same value, so both records converge. D also re-checked that PFA-33's block still hashes `2da381a1…`, 5731 bytes, 59 lines. **Implementation:** `ops/138-operator-onboarding @ a9aa34e` (CI 35257712408 green on five jobs; pgTAP Files=88, Tests=5441, Result: PASS). **A's independent review at a9aa34e:** replay 157/157, Gate-2 32|107|37|38, local 5435/5435 with a strict TAP::Parser pass, the migration and rollback byte-identical to the reviewed 5960b51, all seven pre-apply prosrc md5s recomputed from source, A's mutants killing exactly the predicted tests (the over-broad A6 mutant is now killed by I134/I135/I135b/I136, closing A's F-A6-TEST), and A's own run of `scripts/rehearsal_138_a6_concurrency.sh`: S1, S2, S3, C1 and C2 ALL PASS — including the control in which, without A6, two concurrent acceptances commit and leave the organisation with ZERO owners. **The owner's approval of A6 (2026-09-17, in the A conversation):** "Approve A6 for the follow-on amendment: accepting an invite must never overwrite or demote an existing organization owner. Keep the existing maturity-clock and audit behavior unless a separate change is approved. Add tests for a sole owner, multiple owners, lower-role invites and concurrent acceptance, with rollback evidence." **OWNER SIGNATURE: NOT YET RECORDED.** Signing this amendment does NOT apply migration 138: the owner ruled "applying 138 still requires a separate authorization, including the prerequisite 115–120 sandbox chain and any hosted-project detection read."

**SPRINT CLOSE-OUT — the owner's ruling, 2026-09-17, and it resolves BOTH open conflicts on this amendment.** The owner's words: *"PFA-34 remains proposed and unsigned. Do not treat it as approved or apply migration 138."* and *"Defer the venue-staff detector extension. Keep the detection scope limited to organization membership; leave the local extension unpushed."*

1. **Signature conflict — RESOLVED as NOT SIGNED.** Two owner statements had conflicted: one relayed through D ("I sign PFA-34 at governance commit b7895bb, checksum 026cb858…") and one to A ("Keep PFA-34 proposed and unsigned until I explicitly approve its checksum"). Neither session acted on the other's relay; both held the conservative state, and this file was never changed to SIGNED. **The owner's own statement above now settles it: PROPOSED, NOT SIGNED.** The heading is therefore correct as it stands and nothing in the block changes. D's contract records the same conflict at `review/d-release-sprint` §"PFA-34's signature status is UNRESOLVED" — that section is superseded by this ruling.
2. **D1 scope conflict — RESOLVED as organisation membership only.** D was told to widen the detector to venue staff (ruling 6 of the earlier set) and implemented it locally as `1cacdf5`; A's instruction was not to expand the detector. **The owner has now deferred the extension**, so the placed D1 clause — "platform identities holding organisation membership" — **stands as written**. The replacement clause D proposed in its contract is NOT placed, the block is unchanged, and **the placed checksum `026cb858319bc7c0181e1dad01e23ef1` (2726 bytes, 27 lines) remains the checksum awaiting the owner's approval**. The documentation divergence D correctly flagged is closed by the amendment and the reviewed implementation agreeing again, not by an erratum.
3. **The reviewed 138 head stays `a9aa34e`** (= `origin/ops/138-operator-onboarding`). D's `1cacdf5` carrying the venue-staff detector and the I136 split is **local, unpushed, no CI, reviewed by nobody**, and by this ruling it stays that way. Venue-plane membership at a customer venue remains reachable by the same reverse path A inventoried; that is a known, recorded, undetected gap in the detector's coverage — deferred, not closed.
4. **Applying 138 remains separately barred** and this amendment's signature would not change that.
