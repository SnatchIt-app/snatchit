...
**Standing flags/config that stay OFF until their gate:** `feature.native_issuance_enabled` (Gate P / 15.A) · `feature.native_scanning_enabled` (2B door gate) · `feature.native_resale_enabled` (Gate M + 2C). Flags are runtime config: a migration may seed them OFF; **a migration must never flip them ON**. ~~Supabase automatic production deploy remains **OFF** until migration-history repair is completed and verified~~ — **superseded 2026-08-27 (AUTODEPLOY-1).** That sentence was wrong twice over: the automatic deploy was never verified OFF (it was in fact active and applied migration `071` to production on the merge of PR #14, with no approval gate), and its condition — migration-history repair — has since been satisfied, so as written it read as permission to enable. **The prohibition is now unconditional** until an owner visually confirms the control in the Supabase dashboard. Until then **no migration-bearing PR may merge to `main`**. Canonical: `docs/operations/DEPLOYMENT_PATHS.md`.
...
10. **Production gate** — no production apply without: staging PASS, migration verification, rollback/recovery plan in hand, and **explicit owner/human approval for anything touching money, custody, auth, or RLS**.
    - **10a. Deploy-path precondition (AUTODEPLOY-1, mandatory for every migration-bearing PR).** Before the merge, the **owner must confirm — visually, in the Supabase dashboard — that production auto-deploy is OFF**. Record the confirmation in the PR. Never infer it from check names, preview behaviour, timestamps, or from this document. If it is not confirmed, **do not merge**: on this repository the merge *is* the apply, and step 10b will already have happened without approval.
    - **10b. Explicit apply** — owner-authorized, by an approved path in `docs/operations/DEPLOYMENT_PATHS.md`. The Supabase GitHub integration is **not** an approved apply path; it is the thing being gated.
    - The full required order is: PR reviewed → staging verified → **owner confirms auto-deploy OFF** → merge → explicit owner-authorized apply → live verification.
...
- **Toolchain pin (binding for every migration-sensitive step):** **Supabase CLI `2.115.0`**, and nothing else. A replay is only evidence if it ran on the pinned tool, so "fresh replay green" claimed from a different CLI does not satisfy Gate-2. The version lives in one place — job-level `env.SUPABASE_CLI_VERSION` in the `db` job of `.github/workflows/ci.yml` — and is asserted there before any migration step; drift fails the build. Local replays (step 7) must use the same version. `2.116.0` is available upstream and deliberately not adopted; bumping the pin is its own PR (change the env value → fresh replay green → Gate-2 parity green → merge), never a side effect of another package. Rationale and history: `docs/architecture/SNATCH_IT_ENGINEERING_STANDARDS.md` §5.
...
- **Default-ACL discipline (SEC-2) — binding on every Phase-2 package that creates a table or function in `public`.** Production's `pg_default_acl` on `public` has TABLE and FUNCTION entries (`public/r → anon=arwdm, authenticated=arwdm`; `public/f → anon=X, authenticated=X`; verified 2026-08-27), so **a new table is auto-granted full client DML and a new function is auto anon-EXECUTE the moment it is created**. Every such migration MUST therefore end with an explicit `REVOKE ALL ... FROM PUBLIC, anon, authenticated` (for a function: `REVOKE EXECUTE ON FUNCTION public.<fn>(<argtypes>) FROM PUBLIC, anon, authenticated`, then `GRANT EXECUTE` to the intended roles — a bare `REVOKE FROM PUBLIC` is insufficient once explicit role grants exist) plus only the grants it needs, and MUST add the object to the manifest in `supabase/ci/assert_public_table_grant_decisions.sql` (which covers tables **and** functions despite its name, and is wired into the required `db` job). **Functions are the more exposed half**: a new table still has RLS in front of it, a new function is an unauthenticated `POST /rest/v1/rpc/<name>`. Do **not** attempt to delete the default ACLs — they are vendor-managed and removing them breaks new-object provisioning; and never write an *existence-guarded* privilege statement, which succeeds on production and silently skips on every rebuild. Two traps this closes: the fresh CI replay has **no** TABLE entries in `pg_default_acl`, so a missing REVOKE looks fine there and is wide open in production; and the `expected_grants.txt` parity diff passes vacuously for a table absent from both replay and fixture. This is not theoretical — it is exactly how `public.webhook_retries` came to grant `anon`/`authenticated` full DML in production (SEC-1) while migration `069` revoked them and the ledger recorded `069` as executed. Reconciled by migration `074`. Canonical rule text: `SNATCH_IT_ENGINEERING_STANDARDS.md` §5.
