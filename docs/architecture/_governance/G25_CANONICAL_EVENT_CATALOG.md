...
| 29 | `DisputeOpened (chargeback)` | **REMOVE** (Gate M) | — | **`C30`** — fan-side chargeback/clawback liability is `RATIFIED-MODELED-ONLY(GATE-M)`, MVP `NO` (ratification record). **Producer does not exist:** TRACE §8 `:750` — *"**no dispute or chargeback table exists in any of the sixteen packages**"* *(justification superseded 2026-08-30: `kernel.dispute_native` exists in `088` under `OR-24`; the REMOVE disposition itself STANDS — the dispute writers are `OR-14` non-producers for the EVENT rows, while the `payout_on_hold` NOTICE rides R2 row 21a)*. `A-SSCAS` #9 restores it at Gate M. **Note the residual tension in §6.4.** | `VERIFIED` |
