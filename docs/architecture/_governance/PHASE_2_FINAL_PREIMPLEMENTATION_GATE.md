...
> **HISTORICAL RECORD — DO NOT UPDATE THE FIGURES BELOW. Header added 2026-08-27 on salvage.**
>
> This document records the state of the repository and production **as measured at the time it was
> written**: `main` = **`bddad8f`**, **85** repository migrations, production ledger **85** rows, with
> `072` and `073` authored but unapplied. It is preserved as the baseline evidence record for the
> pre-Phase-2 stabilization pass, and its numbers are only meaningful as a snapshot of that moment.
>
> **Current state at the time of salvage:** `main` = **`23a8a3e`**, **89** repository migrations,
> production ledger **89** rows, source ↔ ledger exact, after `071`–`075` were authored and applied to
> production. The `073` referenced throughout this document is *not* the `073` on `main`: the
> drift-reconciliation migration described here never landed, and the work it covered landed instead as
> `073_storage_bucket_upload_constraints.sql`, `074_privilege_cleanup.sql` and
> `075_replay_parity_storage_policies_and_cron.sql` — `074` deliberately doing less than the `073`
> described here (see its header). Likewise the pgTAP figures here (15 files / 267 assertions) predate
> the current ratchets of **16 files / 287 assertions**.
>
> It was carried over from the superseded PR #22 (`repo/preimpl-integration`), where it existed at the
> repository root; it was moved to `docs/architecture/_governance/` to match the docs map in `CLAUDE.md`.
> Read it as history. For current state, read the migration chain, the CI ratchets, and
> `docs/architecture/SNATCH_IT_ENGINEERING_STANDARDS.md`.
