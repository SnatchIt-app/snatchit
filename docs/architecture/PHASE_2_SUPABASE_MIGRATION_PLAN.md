...
SEAM-2 is used **nineteen** times in the MVP chain (eight pre-`OR-17` + the nine deletion hooks + the `OR-21` BP-10 predicate + the F-2 Q5-release hook) — the enumeration is `kernel.settlement_royalty_lines`,
...
| `kernel.deletion_blockers_custody(p_identity uuid) RETURNS text` | `077` — NULL (no blocker) | `kernel.tickets` does not exist before `079`, so "no live custody" is the true value over an empty world, not an approximation; `079` asserts the real body is live (`OR-17`) | `079` |
| `kernel.deletion_blockers_orders(p_identity uuid) RETURNS text` | `077` — NULL | no `pending` order can exist before `venue.create_primary_checkout` (`082`) — the arm is split from the money hook precisely to close the `082`–`084` window (`OR-17`) | `082` |
| `kernel.deletion_blockers_wallet(p_identity uuid) RETURNS text` | `077` — NULL | no `issued` pass exists before `kernel.wallet_pass` (`083`) (`OR-17`) | `083` |
| `kernel.deletion_blockers_money(p_identity uuid) RETURNS text` | `077` — NULL | no `kernel.payout`/`kernel.refund` row and no `paid` order exists before `085`; the live `public.transfers` analog is evaluated directly in the sweep body, not here (`OR-17`) | `085` |
| `kernel.deletion_blockers_market(p_identity uuid) RETURNS text` | `077` — NULL | no market row exists before `088` (`OR-17`) | `088` |
| `kernel.on_identity_erased_staff(p_identity uuid) RETURNS void` | `077` — no-op | terminal cleanup INV #23/#24; misses fail soft (hygiene on retained rows) (`OR-17`) | `080` |
| `kernel.on_identity_erased_door(p_identity uuid) RETURNS void` | `077` — no-op | terminal cleanup INV #29–#31 (`OR-17`) | `086` |
| `kernel.on_identity_erased_market(p_identity uuid) RETURNS void` | `077` — no-op | the 16d hard-delete allowance ONLY: draft/cancelled listings + non-accepted offers; sold/completed retained (`OR-17`) | `088` |
| `kernel.on_identity_erased_promoter(p_identity uuid) RETURNS void` | `077` — no-op | INV #36 `status_changed_by` SET NULL; the `venue.promoter` row itself SURVIVES (16d) (`OR-17`) | `090` |
| `kernel.has_outstanding_obligations(p_identity_id uuid) RETURNS boolean` | `077` — `false` | the BP-10 operand read (`OR-21`): no `kernel.identity_obligation` origin object exists before `085`, so zero outstanding is the correct answer (`C110` class) | `085` |
| `kernel.on_deletion_q5_release(p_identity uuid) RETURNS void` | `077` — no-op | the Q5 auto-expiry's release side-effects (§17.4 semantics: `refund_hold` release + `refund_request_expired` BE notice, R2 row 18); no refund/hold object exists before `085`, so nothing can need release (freeze red-team F-2) | `085` |
...
- **Additive-only: YES** for all MVP packages (076–092 — band extended by `OR-12`; F-13 recount). No `public.*` semantic change; no destructive edit.
...
**Edges added by the deletion-machine fold (`OR-17`, F-P0-1 Option A)** — all declaration-only for ordering:

| Edge | Because |
|---|---|
| **`077 → 081`** | F-1 clause in `venue.reserve_primary_inventory` calls `kernel.is_deletion_pending` (`077`) |
| **`077 → 083`** | `083` replaces the `kernel.deletion_blockers_wallet` stub (`077`) — stub-replacement edges must be declared |
| **`077 → 086`** | `086` replaces the `kernel.on_identity_erased_door` stub (`077`) |
| **`077 → 088`** | `088` replaces the deletion-market stubs (`077`) and hosts the F-2/F-3/F-4 clauses calling the `077` predicate |
...
| 6 | 081 | E | **077** *(OR-17)*, 078,080 | Y | N | new-table only | none | s | — |
| 7 | 082 | F | **077**,**078**,081 | Y | N | new-table only | none | s | issuance gated by 15.A |
| 8 | 083 | G | **076** *(OR-12)*, **077** *(OR-17)*, 078,**079**,**§081** | Y | N | new-table only | none | s | wallet gated (`wallet.apple.enabled=false`) |
...
| 11 | 086 | H | **076** *(OR-12)*, **077** *(OR-17)*, **078**,079,080,081,**083** | Y | N | new-table only | none | s | scanning gated (2B door gate) — `078` applied 2026-08-29 (`B-4`: `door_session` FKs → `event_session`/`venue`, both `078`; was recorded-not-declared) |
| 12 | 087 | I | 077,081,085,**086** | Y | N | new-table only + 1 storage bucket | none | s | — |
| 13 | 088 | J | **076** *(OR-12)*, **077** *(OR-17)*, 078,079,081,**085**,**§086**,**§087** | Y | N | new-table only | none | s | **native resale gated** (Gate-M+2C) |
...
**Per-package "does marketplace behavior change?" = NO for all 16.** **Additive-only = YES for all 16.** *(recounted `OR-12`/F-P2-5: **all 17** — the `092` row was added below this line)*
...
|---|---|---|---|
| `feature.native_issuance_enabled` | `kernel.issue_ticket_atoms` (authored `083` after `R2B`/`C114`; issue path `079`/`083`/`085`); `venue.reserve_primary_inventory` (canonical name, A4 — alias `reserve_inventory`) real draws; `venue.create_inventory_hold` staff holds | **15.A gate** cleared (C1,C2,C3,C4,C5,C6-model,C9,C10) **+ `088` APPLIED and the `stripe-webhook` §4 `charge.dispute` native branch DEPLOYED (`R-40` completion, red-team E-F1: a native payment must not be reachable before its ratified dispute substrate — the flip-ordering condition is forced by `OR-24`'s own "already-reachable MVP-native payment dispute path" clause, not chosen)** — end of Phase 2A | audited `catalog.set_platform_config` (dual-control seam, C11) — a runtime op, **never a migration** |
...
## 5. Migration packages (076–092) — full specification
...
> **`OR-17` DELETION MACHINE (F-P0-1 Option A — ships IN this package; cutover ≤ 077 apply, same release train).**
> `kernel.identity_ext` gains `deletion_state`, `deletion_requested_at`, `deletion_block_reason` (+ the OPEN-3
> erased-marker cell) and a partial index over open requests. New RPCs: `kernel.request_account_deletion`
> (always-accepts into DELETION_PENDING; lazy `identity_ext` create; Q5 auto-expiry of pending
> `approval_request` rows naming the deleter, decided rows immutable; BE-emits `account_deletion_pending`) ·
> `kernel.withdraw_account_deletion` (PENDING → ACTIVE) · `kernel.is_deletion_pending` (the single freeze
> predicate every F-clause calls) · `kernel.sweep_deletion_pending` + its own `cron.schedule` (2 min, P0-1).
> The sweep evaluates BP-1…BP-12 in order — BP-11/Q5 direct (077), BP-6-live/BP-7-live/BP-8-live/BP-9 direct over the
> live `public.*` rail (precondition; no edge), BP-10 via `kernel.has_outstanding_obligations` (`OR-21` —
> stubbed here returning `false`, true-not-inert; real body `085`), the rest via the deletion SEAM-2 stubs
> authored here (bodies: 079/080/082/083/085/086/088/090) — records the first true
> predicate in `deletion_block_reason`, and at all-false executes terminal entry: erased marker, retained
> request metadata, 077-plane role/invite clears, PR#28-minus-repointing live clears (CUSTODY-DEL-1 untouched;
> `auth.admin.deleteUser` called by nothing), the four `on_identity_erased_*` hooks, BE-emit
> `account_deletion_completed`. F-6 refusal clauses land on `accept_org_invite`/`create_organization` here.
> Deploy artifacts on this package's release train: the delete-account edge switch (PR#28 409s retired) and
> the F-5 live-rail guards (edge/RN — FR-9). OPEN cells ride unresolved: OPEN-2/3/4/5/6/7/9/10 (OPEN-1 closed
> by `OR-21`). Physical column/index definitions filed to the schema-spec owner (§1.1/§13 companion), per
> house convention.
...
- **Objects created** (schema spec §1.1–1.4, §1.12):
  ...
  - **`kernel.org_invite` (ADDENDUM A1 — schema §1.3b):** `invite_id` PK; `org_id` FK; `invitee_ref` +
    ...
    `pending/accepted/expired/revoked` (*`declined` STRUCK — `OR-18`, R-36 = B, 2026-08-29*); `invited_by` FK; `expires_at`; partial
...
> **`OR-17` rider:** replaces `kernel.deletion_blockers_custody` (BP-1) — asserts the stub body is no longer live.
...
> **`OR-17` rider:** replaces `kernel.on_identity_erased_staff` (terminal cleanup INV #23/#24).
...
> **`OR-17` rider:** `venue.reserve_primary_inventory` gains the F-1 refusal clause (`kernel.is_deletion_pending`; new edge `077 → 081`).
...
> **`OR-17` rider:** replaces `kernel.deletion_blockers_orders` (BP-12 order arm); `venue.create_primary_checkout` gains F-1.
...
> **`OR-17` rider:** replaces `kernel.deletion_blockers_wallet` (BP-2; new edge `077 → 083`). `mint_wallet_pass` stays un-guarded (OPEN-4 default ALLOW).
...
> **`OR-17` rider:** replaces `kernel.deletion_blockers_money` (BP-5/BP-6 kernel arm/BP-12 refund arm) **and `kernel.has_outstanding_obligations` (`OR-21` — the BP-10 operand predicate)**; `venue.finalize_primary_order` gains F-1.
...
- **Objects created** (schema spec §1.8–1.10):
  ...
  - `kernel.identity_obligation` (`F-P2-1`/`OR-21`, schema §1.10a: `debtor_identity_id` FK→auth.users RESTRICT; `origin_kind` CHECK in `chargeback/refund_clawback`; `origin_ref` soft ref [the `cause_ref` no-FK discipline]; `stripe_dispute_ref` write-once partial UNIQUE; `amount_minor>0`; `status` CHECK in `outstanding/recovered/written_off` forward-only; the resolution pairing CHECK; `UNIQUE(origin_kind, origin_ref)`; partial `(debtor_identity_id) WHERE status='outstanding'` — the BP-10 operand index; `REVOKE DELETE` outright).
  - All four: money-custody-RPC-only (deny-all RLS + `REVOKE ALL`); payee/buyer reads own via scoped RPC (`identity_obligation`: no client read at all).
...
- **Rollback (`085_*`):** drop identity_obligation, payout, refund, payment_native. Clean while empty. Post-go-live: forward-fix
...
> **`OR-17` rider:** replaces `kernel.on_identity_erased_door` (INV #29–#31; new edge `077 → 086`).
...
- **Rollback (`087_*`):** drop settlement_line then settlement; **`CREATE OR REPLACE` `venue.on_payout_settled` back to its `085` stub body (red-team C-F2 — a rolled-back replacer must never leave a body writing dropped `venue.settlement` live under `kernel.mark_payout_transfer_state`'s webhook path: the `settlement_royalty_lines` 42P01 class).** Clean while empty; forward-fix once used.
...
> **`OR-17` rider:** replaces `kernel.deletion_blockers_market` (BP-3 · BP-4 · **the BP-7 and BP-8 NATIVE TWINS** — deletion-spec routing, harmonized 2026-08-30) + `kernel.on_identity_erased_market`; `make_offer` (F-3), `respond_offer` (F-2 — guards the OFFER's buyer), `accept_p2p_transfer` (F-4) gain refusal clauses (new edge `077 → 088`).
...
> **`OR-17` rider:** replaces `kernel.on_identity_erased_promoter` (INV #36 only; `venue.promoter` row SURVIVES, 16d); `venue.create_promoter` gains F-7.
...
## 8. THE FINAL PACKAGE TABLE — `076`–`092`, post-delta-integration
...
| Field | Value |
|---|---|
| **Purpose** | Stand up the **five** MVP schemas (`notify` added by `OR-12`) and the modular-monolith GRANT boundary, plus the shared helper functions every later package attaches. No product tables. |
...
| **Indexes** | `notify.outbox`: **`UNIQUE(event_type, event_key)`** + **`UNIQUE(aggregate_kind, aggregate_id, sequence)`** (the C12 envelope constraints, born with the table — §17.24). *(Was "none" pre-`OR-12`.)* |
| **Grants** | `REVOKE ALL ON SCHEMA kernel, venue, market FROM PUBLIC, anon, authenticated`; `GRANT USAGE ON SCHEMA catalog TO anon, authenticated`; `GRANT USAGE ON SCHEMA kernel, venue, market TO authenticated` **for function EXECUTE only**. `service_role` is a machine identity, never a human grant target. **Plus (`OR-12`, red-team F-P1-7): `REVOKE ALL ON SCHEMA notify FROM PUBLIC, anon, authenticated` + the same `ALTER DEFAULT PRIVILEGES` belt — the notify plane is inside the deny-by-default wall from birth.** |
...
| **Tables** | `kernel.identity_ext` (+`locale`, Δ-N2) · `organization` (+`payout_destination_set_by`) · `org_member` (**+`granted_at`**, **six** org labels) · `org_invite` (**six** org labels) · `platform_role` · `admin_audit` (AO) · **`approval_request`** (**+`required_approver_class`**) · `identity_demographic` · `identity_demographic_erasure` (AO) · `identity_contact_pref` · **`identity_contact_pref_event` (AO)** · `org_customer_key`. **Twelve tables, not eleven — `identity_contact_pref_event` was asserted by SPEC_FOUNDATION §6 (which even assigned it this package), by CRM §5.1/§11.1-5b, by RLS §6/§16.6/§16.10 and by RPC §20, and defined by nothing anywhere (defect `K-2`; schema §1.15.1).** **`OR-17` (F-P0-1/A): `kernel.identity_ext` gains `deletion_state`, `deletion_requested_at`, `deletion_block_reason` (+ the OPEN-3 erased-marker cell) — the deletion-machine substrate; columns filed to schema §1.1/§13.** |
| **Functions** | `has_org_role`, `is_platform`, `is_org_affiliate`; `get_my_demographics`, `set_my_demographics`, `clear_my_demographics`; `get_my_contact_prefs`, `set_my_contact_prefs`; **`kernel.create_organization`/`update_organization`, `set_org_status`, `set_org_connect_ref` (EDGE-FRONTED — the precondition for every payout in the system, wrapped by `connect-onboarding` and contracted nowhere until RPC §20.1.1), `upsert_identity_ext`, `grant_platform_role`, `revoke_platform_role`, `invite_org_member`, `accept_org_invite`, `grant_org_role`/`revoke_org_role`** (RPC §20.1, §2.2–§2.5). **`set_my_contact_prefs` appends one `kernel.identity_contact_pref_event` row in the SAME transaction as its current-state upsert — that append is the function's contract, not an optimization** (`AUTHZ-CRM1`; RPC §20.14 `R-7`). SEAM-1: `max(077, 077) = 077`. **Plus `kernel.revoke_org_invite`, `kernel.sweep_expired_org_invites` (RPC §20.1.6/§20.1.7, authored 2026-08-29 — SEAM-1: both touch only `kernel.org_invite` (`077`) [+`kernel.admin_audit` `077`]; the sweep carries its own explicit `cron.schedule` entry per `P0-1`), and `kernel.write_demographic_erasure_tombstone()` (§17.20a — the J-12 trigger function).** **Plus the `OR-17` deletion machine (F-P0-1 Option A, §20.17): `kernel.request_account_deletion` (always-accepts, Q5 auto-expiry in-txn, BE-emits `account_deletion_pending`), `kernel.withdraw_account_deletion`, `kernel.is_deletion_pending` (the single freeze predicate), `kernel.sweep_deletion_pending` (+ its own `cron.schedule`, 2 min, `P0-1`; terminal entry BE-emits `account_deletion_completed`), and the ELEVEN SEAM-2 stubs whose bodies land in their operands' birth packages — `kernel.deletion_blockers_custody` (→`079`), `kernel.deletion_blockers_orders` (→`082`), `kernel.deletion_blockers_wallet` (→`083`), `kernel.deletion_blockers_money` (→`085`), `kernel.deletion_blockers_market` (→`088`), `kernel.on_identity_erased_staff` (→`080`), `kernel.on_identity_erased_door` (→`086`), `kernel.on_identity_erased_market` (→`088`), `kernel.on_identity_erased_promoter` (→`090`), `kernel.has_outstanding_obligations` (→`085`, the `OR-21` BP-10 operand), `kernel.on_deletion_q5_release` (→`085`, the Q5-release side-effects — freeze red-team F-2). F-6 clauses land on `accept_org_invite`/`create_organization` here; F-5 + the delete-account edge switch are deploy artifacts on this package's release train (FR-9).** |
...
| **Rollback** | **FORWARD-FIX ONLY (from first row).** The custody ledger is permanent and is never dropped once it holds real atoms. The DROP script serves only the pre-go-live window. **`OR-17` rider (F-5): the pre-go-live DROP also `CREATE OR REPLACE`s `kernel.deletion_blockers_custody` back to its `077` stub body — a rolled-back replacer must never leave a body referencing dropped tables live under the 2-minute sweep (the `settlement_royalty_lines` 42P01 class).** |
...
| **Rollback** | **CLEAN-WHILE-EMPTY.** Drop the four predicates, then `venue.staff_role`; **restore `kernel.on_identity_erased_staff` to its `077` stub body (F-5).** |
...
| **Rollback** | **CLEAN-WHILE-EMPTY.** Drop the candidate-freeze trigger and the two `venue.order` candidate columns, then `org_contact_consent_event`, `org_contact_consent`, `order_item`, `order`; **restore `kernel.deletion_blockers_orders` to its `077` stub body (F-5).** **`090`'s rollback must run first** — it drops the two FK constraints it adopted onto these columns. |
...
| **Tables** | `kernel.payment_native` **(incl. `instrument_fingerprint text NULL` — moved from `090` 2026-08-29 under the `C112` column-follows-writer discipline: `venue.finalize_primary_order`, authored HERE, writes it; RPC §6.3, schema §1.8)** · `kernel.refund` · `kernel.payout` · **`kernel.identity_obligation` (`F-P2-1`/`OR-21`; schema §1.10a)**. |
...
|| **Scheduled ticks** | **Five explicit `cron.schedule` entries created BY THIS PACKAGE (`P0-1`; register rows — red-team F-12):** door-session expiry (`venue.sweep_expired_door_sessions`, 2 min) · door-override expiry (`kernel.sweep_expired_door_overrides`, 2 min) · implicit door freezes (`catalog.sweep_implicit_door_freezes`, 2 min) · holder-mix refresh (`venue.refresh_holder_mix`, nightly) · holder-mix reconcile (`venue.reconcile_holder_mix`, nightly). |
...
| **Tables** | `market.listing_native` · `auction` · `offer` · `market_sale` · `p2p_transfer`. `market.bid` is **EXT, not created**. **`R-40` additive: `kernel.dispute_native` (schema §1.10b — the native chargeback record; forward-fix from first row) + the fifth `resale_state` label `dispute_hold` on `kernel.tickets` and on `venue.door_manifest_entry`'s CHECK (DROP/ADD-CONSTRAINT pair, the ADDITIVE-2 mechanics).** R-37/`OR-22` additive: `listing_native.status` gains `reserved`; `market_sale.sale_state` gains `cancelled`; `market_sale.reservation_expires_at`, `market_sale.payment_intent_ref` (write-once). |
| **Functions** | `market.create_listing`, `cancel_listing`, `create_auction`, **`market.place_bid`** (RPC §20.13 names it — RLS §11 granted it as the unnamed *"bid RPC"*; **its ledger's home was CLOSED by `OR-11` (R-9 = Option A, 2026-08-29: no native-rail auctions in MVP — the EXT `market.bid` ledger is NOT scheduled; the function ships feature-dark)** — either native-only auctions are not offered in MVP, or the EXT `market.bid` ledger is scheduled into `088`. An implementer facing that silence creates a table no package specifies, and a bid ledger invented at build time is a money surface with no review), `make_offer`, `respond_offer`, `create_/accept_/cancel_p2p_transfer`, `sweep_expired_p2p_transfers`, `sweep_paid_pending_sales`; **`kernel.transfer_ticket_ownership` (SEAM-1: it reads `listing_native` and writes `payment_native`)**; **`catalog.cancel_event` (SEAM-1: it writes across `079`/`081`/`085`/`088`)**; **`CREATE OR REPLACE kernel.settlement_royalty_lines`** (adds the `market_sale` royalty arm) and **`CREATE OR REPLACE market.on_atom_voided`** (sets `market_sale.terminal_state := 'compensated'`); **plus `CREATE OR REPLACE market.on_door_freeze_engaged` and `CREATE OR REPLACE market.door_freeze_drain_preview`** — the two door-drain hooks stubbed in `086` (`C110`, `R2B`). **`on_door_freeze_engaged`'s real body is the drain `venue.open_door_manifest` performs before it takes the snapshot**: `initiated` `market.p2p_transfer` rows → `cancelled` with `reason_code='door_freeze'` (atom unlocked back to the **sender**, which C43 exempts because owner and `credential_version` do not change) and `active` `market.listing_native` rows → `cancelled`, **excluding any listing whose sale is `paid_pending_transfer`** — money is already taken and the C25 sweep owns that row (RPC §12.4c, `T-RPC-DOOR-12`). It unlocks through `kernel.unlock_ticket` (`079`), **appends no ownership-log row and bumps no `credential_version`**, so the Door Safety Theorem is unaffected. **`door_freeze_drain_preview`'s real body is the same predicate as a count**, and `excluded_paid_pending` is returned separately so the preview's numbers and the open's returned counts reconcile. **All four replacements are body-only (SEAM-2a).** `settlement_royalty_lines` keeps `(p_settlement_id uuid) RETURNS SETOF kernel.settlement_line_candidate` verbatim from `087`; `on_atom_voided` keeps `(p_atom_id uuid, p_refund_id uuid, p_cause text) RETURNS void` verbatim from `085`. **A changed parameter list here does not fail — it creates an overload and leaves the `085` stub bound to every existing call site (`C117`).** **Plus `market.mark_sale_paid_state` (§20.8.7, authored 2026-08-29 — SEAM-1 `max(077,088)=088`, no edge added; LIVE under R-37/`OR-22`). Plus the R-37/`OR-22` buy-now set (§20.8.8–§20.8.12): `market.checkout_buy_now`, `market.bind_checkout_payment_ref`, `market.finalize_market_sale` (SSCAS member #2's "market checkout" caller), `market.cancel_buy_now_sale`, `market.list_lapsed_checkouts` — every one SEAM-1 `max(…,088)=088`, no edge added (the §20.8.7 arithmetic).** **Plus the `R-40` dispute surface (§20.7.13–§20.7.15): `kernel.record_dispute_native`, `kernel.mark_dispute_state`, `kernel.resolve_dispute_native` — SEAM-1 `max(077,079,085,087,088)=088`, every operand package already in `depends_on`, no hook, no edge. `kernel.deletion_blockers_market`'s `088` body additionally evaluates the BP-7 native twin over `kernel.dispute_native`.** |
| **RLS** | Public-read for active + reserved listings/auctions (flag-gated); owner-scoped for offers/sales/transfers; money-custody-RPC-only writes. **No `market` object mutates a `public.*` money/custody row** — it only references `public.payments` by id. |
...
| **Rollback** | **CLEAN-WHILE-EMPTY**, then forward-fix for `attribution`. Restore `kernel.settlement_commission_lines` (`087` stub), **`venue.resolve_order_attribution` (`085` stub — red-team C-F1: `085`'s own cell asserts this restore; without it `venue.finalize_primary_order` 42P01s on every primary purchase after a `090` rollback)** **and `kernel.on_identity_erased_promoter` (`077` stub, F-5)**; drop the settlement-line index, **the two adopted `venue.order` FK constraints** (the columns themselves now belong to `082` and are dropped by *its* rollback — `R2B`/`C112`), the `payment_native` column, `attribution_review`, `promoter_code_scope`, `promoter_code`, `attribution`, `promoter_link`, `promoter`, `normalize_promoter_code`; **restore `settlement_commission_lines` to its `087` stub body.** The promoter spec's insistence that the feature not be split across packages is honoured: it reverts as one unit. |
...
| **Functions** | the **15 in-package RPCs** *(the emit pair lives in `076` — recounted, F-P2-5)*, named (§17.24/§17.25 — announcements + `sweep_scheduled` OUT): `notify.get_inbox`, `get_unread_count`, `mark_read`, `mark_all_read`, `dismiss`, `get_preference_matrix`, `set_preference`, `register_push_token`, `revoke_push_token`, `channel_enabled`, `enqueue`, `resolve_web_link`, `claim_deliveries`, `record_delivery_result`, plus `notify.drain_outbox` (bounded expansion per §17.24's obligation) and the consumer read pair as contracted. Producer emit clauses are classified per `OR-14` (R2). |
| **Seeds** | **31 `notify.notification_type` rows** (N3 closed + `OR-15` `refund_request_cancelled` + `OR-17` `account_deletion_pending`/`account_deletion_completed` — both BEST-EFFORT emitters in `077`; the 29→31 count is the owner-stamped `OR-15` corrigendum signed with `OR-17`). |
...
| **Tests** | Replay green; seed-count = 31 asserted by enumeration; dedupe-key uniqueness; the R2 class split (best-effort never raises · required raises and aborts); `pg_roles` contains no `crm_export_builder` (chain-final re-assertion, `OR-12` choice 9). |
