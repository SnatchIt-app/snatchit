...
> package `N` must be created at or before `N`** — see §8/`076` and `C115`. (`SEAM-3`, the RLS-policy
...
| *(2D)* promoter engine | `090` | `venue.promoter`, `promoter_link`, `attribution` **+Δ commercial-terms columns, `promoter_code`, `promoter_code_scope`, `attribution_review`, the cross-settlement commission unique, `payment_native.instrument_fingerprint`** **+`R2B` ADOPT of the two `venue.order` candidate FKs, whose columns are now born in `082` (`C112`)** (roadmap Phase 2D; modeled now, activated in the promoter phase) |
...
| Field | Value |
|---|---|
| **Purpose** | Stand up the four MVP schemas and the modular-monolith GRANT boundary, plus the shared helper functions every later package attaches, **and the one non-superuser role the boundary needs (`crm_export_builder`, `R2B`/`C115`)**. No product tables. |
...
| **Grants** | `REVOKE ALL` on all **twelve** from `PUBLIC, anon, authenticated`; **`REVOKE UPDATE, DELETE` additionally on `identity_contact_pref_event` (AO)**; EXECUTE granted only to the intended role per RPC. **Plus `GRANT SELECT … TO crm_export_builder` on FOUR of this package's tables — `identity_ext`, `identity_contact_pref`, `identity_contact_pref_event`, `org_customer_key` — authored HERE by `SEAM-4` (`R2B`/`C115`), which is possible only because the role is created in `076`.** |
...
| **Grants** | `GRANT SELECT` to `anon`/`authenticated` on the public-read projections; writes revoked. **Plus `GRANT SELECT … TO crm_export_builder` on TWO of this package's tables — `catalog.event`, `catalog.event_session` (`R2B`/`C115`, `SEAM-4`).** |
...
| **Grants** | `REVOKE UPDATE, DELETE` on both AO tables; `REVOKE ALL` from `anon`/`authenticated` on all three. **Plus `GRANT SELECT … TO crm_export_builder` on ONE of this package's tables — `kernel.tickets` (`R2B`/`C115`, `SEAM-4`).** |
...
| **Grants** | `REVOKE ALL` on all **four**; **`REVOKE UPDATE, DELETE` additionally on `org_contact_consent_event` (AO)**; EXECUTE per RPC. **Plus `GRANT SELECT … TO crm_export_builder` on FOUR of this package's tables — `org_contact_consent`, `org_contact_consent_event`, `venue.order`, `venue.order_item` (`R2B`/`C115`, `SEAM-4`).** |
