...
> package `N` must be created at or before `N`** — see §8/`076` and `C104`. (`SEAM-3`, the RLS-policy
...
**`venue.append_door_manifest_delta`** (`R2B`, `C102`), **`venue.resolve_order_attribution`**
(`R2B`, `C100`), **`market.on_door_freeze_engaged`** and **`market.door_freeze_drain_preview`**
(`R2B`, `C99`). **It was four before `R2B` and the two
...
nothing anywhere in the corpus**, which is `42704` at `087` (`C105`).
...
**silent production failure** — see `C106` and §8/`085`.
...
| **F** orders | `082` | `venue.order`, `venue.order_item` **+Δ `kernel.org_contact_consent`** **+`R2B` the two `venue.order` attribution-candidate columns and their freeze guard, moved from `090` (`C101`); `venue.finalize_primary_order` moved OUT to `085` (`C100`)** |
| **G** credential infrastructure | `083`, `084` | `083`: `kernel.signing_key` (key-ref, NO secret) **+Δ `kernel.pass_type_cert`, `wallet_pass`, `wallet_pass_device`, `wallet_pass_push_log`, the `.pkpass` bucket** **+`R2B` `kernel.issue_ticket_atoms` (moved from `081` — it reads `kernel.signing_key`; `C103`) + the `venue.append_door_manifest_delta` SEAM-2 stub (`C102`)** · `084`: **adopt** — `kernel.tickets` late-binding FKs → `ticket_type` + `signing_key`, **and nothing else** |
...
| *(2D)* promoter engine | `090` | `venue.promoter`, `promoter_link`, `attribution` **+Δ commercial-terms columns, `promoter_code`, `promoter_code_scope`, `attribution_review`, the cross-settlement commission unique, `payment_native.instrument_fingerprint`** **+`R2B` ADOPT of the two `venue.order` candidate FKs, whose columns are now born in `082` (`C101`)** (roadmap Phase 2D; modeled now, activated in the promoter phase) |
...
| Edge | Because |
|---|---|
| **`081 → 083` `§`** | **`kernel.issue_ticket_atoms` moves from `081` to `083` (`C103`) and takes its inventory reads and writes with it.** The mint reads `kernel.signing_key` (`083`) as a precondition — *"an `active` `kernel.signing_key` resolves for the event scope"* (RPC §7.1) — and writes `venue.inventory_batch(_shard)` and `venue.inventory_movement` (`081`) and `kernel.tickets`/`ticket_ownership_log` (`079`), so `max(079, 081, 083) = 083`. **This is `FR-3`'s criterion, applied to the other kernel engine.** `FR-3` moved `kernel.transfer_ticket_ownership` out of `079` for reading the same table; the sibling engine was left where it was. `081 < 083`, so the edge is satisfied on the day it is written down. |
| **`083 → 085` `§`** | **`venue.finalize_primary_order` moves from `082` to `085` (`C100`) and calls `kernel.issue_ticket_atoms`, authored in `083` by `C103`.** A called routine contributes its own package to the `max()` in its own right (corrected SEAM-1, §0.4b) — this is the edge the uncorrected rule could not see, because `085`'s statement list names no `083` *table*. |
| **`081 → 085` `§`** | **`venue.finalize_primary_order` writes `venue.inventory_batch(_shard)` (`held -= q; sold += q`) and `venue.inventory_movement` (`issue`) — RPC §6.3's Writes row — and both are `081`.** They came with the function when `C100` moved it. |
| **`087 → 088` `§`** | **`088` `CREATE OR REPLACE`s `kernel.settlement_royalty_lines`, whose stub is created in `087` — and the edge was never declared** (`C107`). Pre-existing, surfaced by `R2B` rather than created by it, and it is the **sixth** instance of the under-declaration shape. **The consequence of leaving a stub-replacement edge undeclared is not a missing name, it is a silent inversion:** run `088` before `087` and the replacement is created first, then `087`'s `CREATE OR REPLACE … AS $$ zero rows $$` **overwrites the real royalty arm with the neutral one**, replay stays green, and every settlement closes with no royalty lines. **Declaration-only:** `087 < 088` was always true. |
| **`086 → 088` `§`** | **`088` `CREATE OR REPLACE`s two hooks whose names and signatures `086` creates** — `market.on_door_freeze_engaged` and `market.door_freeze_drain_preview` (`C99`). A replacement that runs **before** its stub does not fail; it is created first and the stub's `CREATE OR REPLACE` then **overwrites the real body with the neutral one**, silently. That is why the edge must be declared rather than left true by numbering. **The same shape already existed undeclared for `kernel.settlement_royalty_lines`** (stub `087`, replaced `088`) — see `087 → 088`, `C107`. |
...
### §8 acceptance property — **placement is re-derived, not inherited** (`R2B`/`D22`)
...
| Field | Value |
|---|---|
| **Purpose** | Stand up the four MVP schemas and the modular-monolith GRANT boundary, plus the shared helper functions every later package attaches, **and the one non-superuser role the boundary needs (`crm_export_builder`, `R2B`/`C104`)**. No product tables. |
...
| **Indexes** | none. |
| **Grants** | `REVOKE ALL ON SCHEMA kernel, venue, market FROM PUBLIC, anon, authenticated`; `GRANT USAGE ON SCHEMA catalog TO anon, authenticated`; `GRANT USAGE ON SCHEMA kernel, venue, market TO authenticated` **for function EXECUTE only**. `service_role` is a machine identity, never a human grant target. **Plus `CREATE ROLE crm_export_builder NOLOGIN` and `GRANT SELECT (id, email) ON auth.users TO crm_export_builder`, column-scoped — MOVED HERE from `087` by `R2B`/`C104` (defect `V-GRANT`).** See the enumeration below. |
| **`R2B`/`C104` — the `crm_export_builder` grant set, enumerated, with the package each grant is authored in** | **The defect: `GRANT … TO crm_export_builder` appeared in NO package's Grants row, anywhere, for a set of twelve relations spanning `077`–`087`, while the role itself was scheduled in `087`.** RLS **§16.10** closes the set and names it — *"twelve, not ten"* — and rules that each relation additionally carries one `<schema>_<table>_sel_svc_export` policy, `USING (current_user = 'crm_export_builder')`. **The house convention puts a table's grants in the table's package** (`077`'s Grants row grants on `077`'s twelve tables, `082`'s on `082`'s four), so an implementer following the convention writes `GRANT SELECT ON kernel.identity_ext TO crm_export_builder` **into `077`** — and the role does not exist until `087`. **That is `42704 role "crm_export_builder" does not exist`, and unlike the rest of this pass it is a HARD REPLAY FAILURE at `077`**, because a `GRANT` resolves its grantee immediately. The alternative an implementer reaches for when it fails — consolidating all twelve grants into `087` — is worse than it looks: `087`'s rollback would then have to `REVOKE` on relations belonging to five other packages, and rolling back `077` while `087` stands would leave the builder holding grants on nothing. **Repair, and it is derived rather than chosen: `SEAM-4`, the GRANT corollary of `SEAM-1`.** *A `GRANT` is authored in the package equal to `max()` of the package creating the **relation** and the package creating the **grantee role**.* The role has no dependencies of its own, so moving it to `076` — *"the modular-monolith GRANT boundary"* package, whose purpose row already says exactly that — makes `max()` collapse to the relation's own package for all twelve, and the house convention holds unchanged. **The role is `NOLOGIN` and holds no membership, so between `076` and `087` it is a name with privileges and no way to use them.** **No dependency edge is added by any of this:** every package below already reaches `076` transitively (`077 → 076` directly, everything else through it), and the corpus does not declare transitive edges (`085` does not declare `076` either, yet needs its schemas). |
| **The twelve, plus `auth.users`** | `076` — **`auth.users`** *(precondition relation; `max(precondition, 076) = 076)*, column-scoped `SELECT (id, email)` so the builder cannot reach `encrypted_password`, `raw_user_meta_data` or the recovery tokens · `077` — **`kernel.identity_ext`**, **`kernel.identity_contact_pref`**, **`kernel.identity_contact_pref_event`**, **`kernel.org_customer_key`** *(4)* · `078` — **`catalog.event`**, **`catalog.event_session`** *(2)* · `079` — **`kernel.tickets`** *(1)* · `082` — **`kernel.org_contact_consent`**, **`kernel.org_contact_consent_event`**, **`venue.order`**, **`venue.order_item`** *(4)* · `087` — **`venue.export_job`** *(1)*. **Four plus two plus one plus four plus one is twelve**, matching RLS §16.10's closed set exactly; `auth.users` is the thirteenth grant and the **first** relation, not a member of the twelve, and §16.10 flags it as *"one GRANT that is not a policy and was missing entirely"*. **No demographic relation appears and none may be added** — the property `T-RPC-CRM-06`'s reader enumeration asserts. **The `_sel_svc_export` POLICY half stays with its table in every case and moves nowhere:** its `USING` clause is a bare `current_user` string comparison, so it calls no function and `SEAM-3` leaves it where the table is — **and, unlike the GRANT, it does not require the role to exist**, which is exactly why the two halves failed differently and why only one of them needed moving. **§8's RLS rows name that policy for the four `077`/`082` consent/preference tables and not for the other eight** (`identity_ext`, `org_customer_key`, `catalog.event`, `event_session`, `kernel.tickets`, `venue.order`, `venue.order_item`, `venue.export_job`); RLS §16.10 clause 2 is the authority and states all twelve — **filed to the RLS owner as `R2B-2` so the two documents enumerate the same set rather than one enumerating a third of it.** |
| **OWNER DECISION, recorded and NOT taken** | **The entire grant set is contingent on RLS open decision `MD-2`** — whether `venue.build_export_rows` runs as the narrow `crm_export_builder` owner or stays `postgres`-owned. §16.10 clause 5 is explicit: *"If `MD-2` resolves the other way … none of 1–4 is built"*, and *"shipping the role without the policies is the one combination that is silently wrong."* **This pass places the role and derives the grants; it does not rule on `MD-2`**, and if `MD-2` resolves `postgres`-owned, `C104` reverts as one unit — `CREATE ROLE` in `076`, thirteen grants, twelve policies — with no other package affected. `MD-2` is already an open RLS decision and is **not** re-numbered here. |
| **Feature flags** | none. |
| **Dependencies** | precondition only (phase0 chain `000` + `046`–`070`, plus applied `071`–`075`). |
| **Rollback** | **REVERSIBLE** — `DROP SCHEMA … CASCADE` ×3 + drop helpers + **`DROP ROLE crm_export_builder`** (`R2B`). The role drop is valid only once every package holding a grant to it has been rolled back — reverse-order rollout guarantees that, and `DROP ROLE` raises rather than silently orphaning if it is not. |
| **Tests** | Replay `000→076` green. `\dn` shows four schemas. `has_schema_privilege('anon','kernel','USAGE') = false`; `catalog` USAGE `= true`. Helpers owned by `postgres` with pinned `search_path`. No default table privilege for `anon`/`authenticated` in the three private schemas. **`T-SCHEMA-GRANT-01` (`C104`, `R2B`): the role `crm_export_builder` exists after THIS package's replay** — `pg_roles`, not a grep — **and is `NOLOGIN` with no membership**, so the twelve grants it will accumulate are unusable by anyone until `MD-2` wires it to `build_export_rows`. **`-02`: it holds `SELECT` on exactly `(id, email)` of `auth.users` and on no other column of it** — asserted column-by-column, because a whole-table grant here reaches `encrypted_password` and the recovery tokens. **`-03` (the assertion that fails against the pre-fix chain): replay `000 → 077` and every `GRANT … TO crm_export_builder` in `077` succeeds** — run against a chain that creates the role in `087`, `077` aborts at `42704`. |
...
| **RLS** | `identity_ext` owner-scoped read · `organization`/`org_member`/`org_invite` org-scoped · `platform_role`/`admin_audit` audit-only · **`approval_request` money-custody-RPC-only** (deny-all + `REVOKE ALL`; org read only via `list_approval_requests`) · the five demographic/CRM tables **deny-all with an EMPTY grant set — not a reduced one**. **`identity_contact_pref_event` joins the §16.10 zero-policy register and carries exactly ONE policy — `kernel_identity_contact_pref_event_sel_svc_export`, `USING (current_user = 'crm_export_builder')` and nothing else. `INFERENCE:` it inherits the current-state table's posture rather than a relaxed one on the grounds that it is 'just a log' — a column grant here to `authenticated` would publish a timestamped history of when a person changed their mind about being emailed, which is strictly worse than the row it derives from.** |
| **Triggers** | `raise_append_only` on `admin_audit`, `identity_demographic_erasure` **and `identity_contact_pref_event`**; `set_updated_at` on the MUT tables. **`identity_demographic` carries exactly one trigger — the `updated_at` maintainer — and nothing else** (no prior-value capture). |
...
| **Grants** | `REVOKE ALL` on all **twelve** from `PUBLIC, anon, authenticated`; **`REVOKE UPDATE, DELETE` additionally on `identity_contact_pref_event` (AO)**; EXECUTE granted only to the intended role per RPC. **Plus `GRANT SELECT … TO crm_export_builder` on FOUR of this package's tables — `identity_ext`, `identity_contact_pref`, `identity_contact_pref_event`, `org_customer_key` — authored HERE by `SEAM-4` (`R2B`/`C104`), which is possible only because the role is created in `076`.** |
...
| **Grants** | `GRANT SELECT` to `anon`/`authenticated` on the public-read projections; writes revoked. **Plus `GRANT SELECT … TO crm_export_builder` on TWO of this package's tables — `catalog.event`, `catalog.event_session` (`R2B`/`C104`, `SEAM-4`).** |
...
| **Grants** | `REVOKE UPDATE, DELETE` on both AO tables; `REVOKE ALL` from `anon`/`authenticated` on all three. **Plus `GRANT SELECT … TO crm_export_builder` on ONE of this package's tables — `kernel.tickets` (`R2B`/`C104`, `SEAM-4`).** |
...
| **Grants** | `REVOKE ALL` on all **four**; **`REVOKE UPDATE, DELETE` additionally on `org_contact_consent_event` (AO)**; EXECUTE per RPC. **Plus `GRANT SELECT … TO crm_export_builder` on FOUR of this package's tables — `org_contact_consent`, `org_contact_consent_event`, `venue.order`, `venue.order_item` (`R2B`/`C104`, `SEAM-4`).** |
...
| **Dependencies** | `078` (catalog event/venue, config), **`079`** (`wallet_pass.ticket_atom_id` FK), **`081` `§`** (`kernel.issue_ticket_atoms`, authored here by `C103`, writes `venue.inventory_batch(_shard)` and `venue.inventory_movement`). Unblocks `079`'s deferred `signing_key_id` FK, adopted in `084`. |
...
| **Grants** | `REVOKE ALL` on both new tables; the bucket is created **with its constraints in the same statement and a raising `DO $$ … $$` self-verification block — no `ON CONFLICT DO NOTHING`**: `public=false`, `file_size_limit=33554432`, `allowed_mime_types={text/csv}` exactly. **Plus `GRANT SELECT ON venue.export_job TO crm_export_builder` — the twelfth and last of the `SEAM-4` set (`R2B`/`C104`); `build_export_rows` reads its own job row as that role. The ROLE itself is no longer created here — it moves to `076`, because a grant to it is owed from `077` onward and `CREATE ROLE` in `087` makes those grants `42704`.** |
...
| **Dependencies** | `078` (resale_policy), `079` (`kernel.tickets`), `081` (venue context), **`085`** (`sweep_paid_pending_sales` writes `kernel.refund`; `transfer_ticket_ownership` writes `payment_native`; and this package `CREATE OR REPLACE`s `market.on_atom_voided`, stubbed there), **`§086`** (`CREATE OR REPLACE` of `market.on_door_freeze_engaged` and `market.door_freeze_drain_preview`, stubbed there — `C99`), **`§087`** (`CREATE OR REPLACE` of `kernel.settlement_royalty_lines`, stubbed there — `C107`; **pre-existing and undeclared before `R2B`**). Also calls `venue.append_door_manifest_delta` (name from `083`, body from `086`) on `catalog.cancel_event` and the C25 compensate branch. |
