...
> The sweep evaluates BP-1…BP-12 in order — BP-11/Q5 direct (077), BP-6-live/BP-7/BP-8/BP-9 direct over the
...
> **`OR-17` rider:** replaces `kernel.deletion_blockers_market` (BP-3/BP-4) + `kernel.on_identity_erased_market`; `make_offer` (F-3), `respond_offer` (F-2 — guards the OFFER's buyer), `accept_p2p_transfer` (F-4) gain refusal clauses (new edge `077 → 088`).
