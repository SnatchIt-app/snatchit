...
SEAM-2 is used **four** times in the MVP chain, not three — the enumeration is
`kernel.settlement_royalty_lines`, `kernel.settlement_commission_lines`, `market.on_atom_voided` and
**`venue.on_payout_settled`**. The fourth was added by the unwritable-control pass (`MB-2b`, ratified row
...
and `086 → 087` — and they are resolved identically. Edge count `36 → 38`; **still 16 packages, still
