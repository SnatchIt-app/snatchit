...
SEAM-2 is used **six** times in the MVP chain — the enumeration is `kernel.settlement_royalty_lines`,
`kernel.settlement_commission_lines`, `market.on_atom_voided`, **`venue.on_payout_settled`**,
**`venue.append_door_manifest_delta`** (`R2B`, `C102`) and **`venue.resolve_order_attribution`**
(`R2B`, `C100`). **It was four before `R2B` and the two
