...
**Two of twelve, unchanged for the third revision running.** Both YES rows are the two A8 ruled
require nothing: DRAFT and PUBLISHABLE. That is the ratification working exactly as written — a venue
can build its whole event before Stripe exists, and cannot take a dollar until the rail does.
...
**The PAYABLE tail, after a first sale — three of its five items were discharged this train.**
0a signed **and 0a′ attested** → apply 094/095 → deploy `payout-execute` → only then may
`payout.dual_control_min_minor` be set. **Struck as done:** *"build the receivable object"* (the
...
mutation, no remote, no Stripe call. Migrations 093, 094 and 095 exist only as files and as a local,
disposable rehearsal database (`snatchit_rehears_matrix`, [V] 110/110, GATE-2 at the CI baseline).
It records what is true at the repo tip and in production — and, where the two disagree, says so.**
